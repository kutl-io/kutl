...
async fn space_feed_online_shows_edits_and_signals() {
    ...
    // Init the space pointing at the real relay and authorize the human
    // identity so the daemon can sync to the relay and so signal create is
    // accepted by the relay's auth gate.
    init_and_authorize(&relay, &home, space, ONLINE_SPACE_NAME).await;

    // Boot the daemon AFTER init (supervisor reads spaces.toml at start, so the
...
async fn space_feed_offline_fallback_shows_local_signals() {
    ...
    // and authorize so `signal create` can transmit to the relay.
    init_and_authorize(&relay, &home, space, OFFLINE_SPACE_NAME).await;
