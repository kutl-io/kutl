...
use kutl_cli_uxr::harness::journey::{init_and_authorize, init_space};
...
async fn space_bootstrap_journey() {
    ...
    let init = init_space(&relay, &home, space.path(), "acme").await;
...
async fn space_outside_kutl_home_fails_closed() {
    ...
    init_space(&relay, &home, space.path(), "beyond").await;
...
async fn space_status_resolves_from_a_subdirectory() {
    ...
    init_space(&relay, &home, space.path(), "nested-acme").await;
...
async fn space_leave_forgets_locally_and_join_reattaches() {
    ...
    init_space(&relay, &home, space.path(), "leavable").await;
    ...
    assert!(
        kutl_client::SpaceConfig::is_joined(space.path()),
...
async fn sync_stdout_is_clean() {
    ...
    // Init and authorize the human identity so the one-shot sync can push.
...
/// exit non-zero, name the bad URL, and leave no `.kutl/space.toml` behind.
#[tokio::test]
async fn init_rejects_malformed_relay_url() {
    ...
    assert!(
        !kutl_client::SpaceConfig::is_joined(space.path()),
        "no space config should be written when the relay URL is rejected"
    );
}
...
async fn status_flips_space_unhealthy_when_relay_dies() {
    ...
    init_space(&relay, &home, space.path(), "acme").await;
