...
//! The token store/logout arc with its status renders
...
async fn auth_token_status_logout_journey() {
    ...
    // The credentials landed in $KUTL_HOME/auth.toml, owner-only (0600 —
    // the bearer must never be world-readable).
    let creds_path = home.path().join("auth.toml");
    let creds = kutl_client::StoredCredentials::load(&creds_path)
        .expect("auth.toml readable")
        .expect("auth.toml written");
    assert_eq!(creds.token, STORED_TOKEN);
    assert_eq!(creds.relay_url, STORED_RELAY_URL);
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        let mode = std::fs::metadata(&creds_path).unwrap().permissions().mode() & 0o777;
        assert_eq!(mode, 0o600, "auth.toml must be owner-only");
    }
    ...
    assert!(v["did"].is_null(), "no identity.toml → no did: {v}");
    ...
    assert!(!creds_path.exists(), "auth.toml must be gone after logout");
...
async fn auth_login_device_flow_journey() {
    ...
    let creds = kutl_client::StoredCredentials::load(&home.path().join("auth.toml"))
        .expect("auth.toml readable")
        .expect("auth.toml written");
    assert_eq!(creds.token, "kutl_uxr_device_tok_1");
    assert_eq!(creds.account_id, "acct-uxr-1");
    assert_eq!(creds.display_name, "Journey User");
    assert_eq!(creds.relay_url, relay.ws_url());
