//! `kutl daemon start/status/stop` lifecycle journeys —
...
use kutl_cli_uxr::harness::journey::unreachable_relay_url;
...
async fn init_local_space(home: &TestHome, space: &Path) {
    let relay_url = unreachable_relay_url();
...
/// pins the load-bearing line of `render_daemon_status`),
...
async fn start_status_stop_lifecycle() {
    ...
    // Status flips to running — JSON and the human line.
...
/// signalled — `status` does not report it as running, `stop` reports the
/// recycled PID, clears the stale state, and leaves the unrelated process
/// untouched (`live_daemon_pid_in` / identity_permits_signal).
#[tokio::test]
async fn recycled_pid_is_never_signalled() {
    ...
    // `daemon status` reads the same liveness answer `stop` acts on: alive
    // AND the recorded process instance, so the recycled PID is not a
    // running daemon here either.
    let (running, status_pid) = daemon_status(&home, cwd.path()).await;
    assert!(
        !running,
        "status must not report a recycled PID as a running daemon"
    );
