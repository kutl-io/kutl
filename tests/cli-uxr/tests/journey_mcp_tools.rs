...
async fn mcp_document_tools_journey() {
    ...
    // status → the loaded-slot count, WS doc subscriptions, and exactly one
    // MCP session (the proxy holds one relay session for the whole serve
    // lifetime).
    //
    // `document_count` is 1 and agrees with `list_documents`: a
    // `SubscribeSignals` frame is how the session joins signal delivery, and
    // it holds no doc slot and no doc subscription — the serve session itself
    ...
    assert_eq!(status["subscriber_count"], serde_json::json!(0), "{status}");
    assert_eq!(
    ...
    // list_participants → the agent, exactly once, as the MCP session.
    // `SubscribeSignals` holds no doc subscription, so the MCP session is the
    // serve session's only lane — which is also the truest answer to "what is
    // this participant".
    ...
    assert_eq!(
        ...
        serde_json::json!("mcp"),
        "the MCP session is the serve session's only lane: {participants:?}"
    );
