...
async fn document_blame_journey() {
    ...
    // so the daemon's sync to the relay is accepted (and so its authored ops
    // carry that durable DID).
    let human_did = init_and_authorize(&relay, &home, space, "blame-space").await;
