...
/// Env var the CLI's token resolution chain reads first. Every invocation
/// scrubs it so a developer shell can't leak a stale token into a journey;
/// a journey that wants an env token sets it explicitly per call.
