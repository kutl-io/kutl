...
//! `$KUTL_HOME/spaces.toml`, spawns a watcher/sync worker per registered space,
...
//! `spaces.toml` at boot (new ones arrive via SIGHUP), the caller must
...
impl DaemonProcess {
    ...
    /// signing identity and the `spaces.toml` registry). The space must already
