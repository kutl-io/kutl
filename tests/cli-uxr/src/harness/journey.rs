...
pub const UNREACHABLE_RELAY_PORT: u16 = 19999;

/// A relay URL on [`UNREACHABLE_RELAY_PORT`] — `kutl init` against it falls
/// back to local-only mode.
pub fn unreachable_relay_url() -> String {
    format!("ws://127.0.0.1:{UNREACHABLE_RELAY_PORT}")
}
...
/// Returns the init `Output` so callers can also assert on init's stdout.
...
/// [`init_space`], then authorize the human identity (from `status --format
/// json`) on the relay's live allowlist. Returns the human DID. Does not
/// fetch the space id — callers that need it pair `space list` with
/// [`space_id_of`].
pub async fn init_and_authorize(
    relay: &RelayProcess,
    home: &TestHome,
    space: &Path,
    name: &str,
) -> String {
    init_space(relay, home, space, name).await;
    let human_did =
...
/// returning the change ids newest-first (so the OLDEST change is
/// `ids.last()`). Panics on timeout with the last observed log so a flake is
/// diagnosable rather than mysterious.
pub async fn await_change_count(
    home: &TestHome,
    space: &Path,
    file: &Path,
    want: usize,
...
/// empty-vec mystery.
fn document_log_change_ids(json_text: &str) -> Vec<String> {
