...
and kutlhub). Each flavor's `instructions_template.md` (the OSS
relay's is `oss/crates/kutl-relay/src/instructions_template.md`)
pulls this file in via `include_str!` at compile time, sandwiched
between its flavor-specific top (intro / identifier shapes /
primitives) and tail (default-visibility, deployment-specific
cookbooks). Edit here when the change applies to both flavors; edit
a per-flavor template when the change is flavor-specific. The KFM
dialect spec is concatenated separately and lives at
`oss/docs/kutl-markdown.md`.
-->
...
- **Signal stream** reaches only viewers whose tool surfaces signals.
  ...
  `get_signal_detail`. **Query these before task-shaped work, and keep
  querying while you do it** — not only when you are blocked.
  Participants address you mid-task and nothing delivers what they
  said; you are the only one who will go and get it. A signal need
  have no in-doc form, so re-reading the document is not a substitute.
  Someone who addressed you is waiting on an answer — `create_reply`
  gives one.
  Whether *humans* see signals depends on the client they're using —
  see the deployment-specific default-visibility note below for the
  assumption this relay flavor makes.
...
`edit_document` requires `base_version` — the `version` your
`read_document` of that document returned — alongside the full
content you want and a short `intent`. The relay compares your
content against the text you read and applies only what you
changed, so whatever a participant added meanwhile survives. The
`intent` says what your edit accomplishes; agents and humans
querying `read_log` rely on it.

If a participant rewrote a region you also changed, or that
region now sits in more than one place, the region comes back in
`hunks_refused` and the rest of your edit lands. Changes close
together in the text travel as one region, so contesting one of
them refuses both. Read the document again and reapply that
region.

An edit returns no version. To edit again, read again.
