...
//! `~/.kutl/spaces.toml` to discover registered spaces and spawning a
//! [`kutl_daemon::SpaceWorker`] for each one. The PID files
//! (`daemon.pid` / `desktop.pid`) are a reporting surface for `kutl status`;
//! actual single-producer exclusion is the per-space flock the worker takes
//! at construction, released by the OS if the process dies.
...
/// Minimum time between restart attempts for a worker that exited with an
/// error. Long enough that the error stays visible in the tray between
/// attempts, short enough that a transient conflict (e.g. a one-shot
/// `kutl sync` holding the producer lock) heals without an app restart.
const WORKER_RETRY_INTERVAL: std::time::Duration =
...
pub struct SpaceStatus {
    /// Space identifier from `.kutl/space.toml`.
    pub space_id: String,
...
struct WorkerHandle {
    ...
    space_root: PathBuf,
    relay_url: String,
    /// The running worker task. `None` after the task finished and was
    /// reaped (joined for its error message); a reaped worker is a restart
    /// candidate for [`DaemonSupervisor::maintain`].
    task: Option<JoinHandle<Result<(), String>>>,
    cancel: CancellationToken,
    state: SyncState,
    /// When this worker (attempt) was started. Gates both the synced-promotion
    /// heuristic and the retry interval after a failure.
...
pub struct DaemonSupervisor {
    ...
    /// Identity used to (re)build worker configs; recorded on registry load
    /// so failed workers can be restarted without re-reading the identity file.
...
/// Load the identity from `~/.kutl/identity.toml`.
///
...
/// Path to the global space registry (`$KUTL_HOME/spaces.toml`).
...
/// The worker config for a space directory, paired with the human-readable
/// space name the supervisor shows. `None` (with a warning) when the path
/// cannot be resolved or its config cannot be read.
pub(crate) fn build_worker_config(
    space_path: &str,
    identity: &LoadedIdentity,
) -> Option<(kutl_daemon::SpaceWorkerConfig, String)> {
    let config = kutl_daemon::SpaceWorkerConfig::for_space(
    ...
    .ok()?;
    let space_name =
        kutl_client::KutlspaceConfig::display_name(&config.space_root, &config.space_id);
    Some((config, space_name))
}
...
impl DaemonSupervisor {
    ...
    /// the human-readable name from `space.toml` (distinct from the
    ...
    pub async fn start_space(
        ...
        let task = tokio::spawn(async move {
            ...
            match kutl_daemon::run(config).await {
                Ok(()) => {
                    info!(space_id = %worker_space_id, "space worker stopped");
                    Ok(())
                }
                Err(e) => {
                    error!(space_id = %worker_space_id, error = %e, "space worker failed");
                    Err(e.to_string())
                }
            }
        });
    ...
    pub async fn stop_space(&mut self, space_id: &str) -> Result<()> {
        if let Some(mut handle) = self.workers.remove(space_id) {
            ...
            if let Some(task) = handle.task.take() {
                match task.await {
                    ...
                    Ok(Err(e)) => {
                        info!(space_id = %space_id, error = %e, "worker reported an error while stopping");
                    }
                    Ok(Ok(())) => {}
                }
            }
    ...
    /// Harvest finished worker tasks: join each one and record its own error
    /// message, so status surfaces show the actual reason rather than a
    /// placeholder. A reaped worker keeps its registry entry and becomes a
    /// restart candidate for [`Self::maintain`].
    pub async fn reap_finished_workers(&mut self) {
        for (space_id, handle) in &mut self.workers {
            if !handle.task.as_ref().is_some_and(JoinHandle::is_finished) {
                continue;
            }
            let task = handle.task.take().expect("finished task checked above");
            let msg = match task.await {
                Ok(Ok(())) => "worker exited".to_owned(),
                Ok(Err(msg)) => msg,
                Err(join_err) => format!("worker panicked: {join_err}"),
            };
            warn!(space_id = %space_id, error = %msg, "worker exited, retry pending");
            handle.state = SyncState::Error(msg);
        }
    }

    /// One periodic maintenance pass: harvest exits, restart failed workers
    /// whose retry interval has elapsed, and promote connecting workers that
    /// have survived the heuristic delay.
    pub async fn maintain(&mut self) {
        self.reap_finished_workers().await;
        self.restart_due_workers().await;
        self.promote_connecting_workers();
    }

    /// Restart reaped workers whose retry interval has elapsed.
    ///
    /// The config is rebuilt from disk on every attempt, so a space whose
    /// `.kutl/space.toml` changed between attempts picks up the new values.
    async fn restart_due_workers(&mut self) {
        ...
        let due: Vec<(String, String)> = self
            ...
            .filter(|(_, h)| {
                h.task.is_none()
                    && matches!(h.state, SyncState::Error(_))
                    && h.started_at.elapsed() >= WORKER_RETRY_INTERVAL
            })
            .filter_map(|(id, h)| {
                h.space_root
                    .to_str()
                    .map(|root| (id.clone(), root.to_owned()))
            })
            .collect();

        for (space_id, root) in due {
            info!(space_id = %space_id, "retrying failed space worker");
            if let Some((config, space_name)) = build_worker_config(&root, &identity)
                && let Err(e) = self.start_space(config, space_name).await
            {
                error!(space_id = %space_id, error = %e, "failed to restart space worker");
            }
        }
    }
    ...
    pub fn space_statuses(&self) -> Vec<SpaceStatus> {
        self.workers
            .iter()
            .map(|(space_id, handle)| {
                // A finished-but-not-yet-reaped task is an error; the next
                // maintenance pass replaces this placeholder with the
                // worker's own message.
                let state = match &handle.task {
                    Some(task)
                        if task.is_finished() && !matches!(handle.state, SyncState::Error(_)) =>
                    {
                        SyncState::Error("worker exited unexpectedly".into())
                    }
                    _ => handle.state.clone(),
                };
    ...
    /// Reload the space list from `~/.kutl/spaces.toml` and reconcile workers.
    ///
    ...
    /// Promote workers that have survived [`SYNC_HEURISTIC_DELAY`] from
    /// `Connecting` to `Synced` — the v1 heuristic: a worker that hasn't
    /// exited by then is assumed connected. A worker that DID exit is left
    /// for [`Self::reap_finished_workers`] to record with its own message.
    pub fn promote_connecting_workers(&mut self) {
        for handle in self.workers.values_mut() {
            if matches!(handle.state, SyncState::Connecting)
                && handle.started_at.elapsed() >= SYNC_HEURISTIC_DELAY
...
mod tests {
    ...
    fn test_overall_status_all_synced() {
        let statuses = vec![SpaceStatus {
            space_id: "ca978112-ca1b-4dca-8ac2-31b39a23dc4d".into(),
    ...
    fn test_overall_status_connecting() {
        let statuses = vec![
            ...
            SpaceStatus {
                space_id: "3e23e816-0039-494a-8389-4f6564e1b134".into(),
    ...
    /// Test identity with no signing key — enough to build worker configs.
    fn test_identity() -> LoadedIdentity {
        LoadedIdentity {
            did: "did:key:z6MkTestOnly".into(),
            signing_key: None,
            display_name: String::new(),
        }
    }

    /// Write a minimal on-disk space (`.kutl/space.toml`) under `dir` and
    /// return its root. The relay URL points at a closed port — these tests
    /// exercise worker lifecycle, not sync.
    fn write_test_space(dir: &std::path::Path) -> PathBuf {
        let space = dir.join("space");
        std::fs::create_dir_all(space.join(".kutl")).expect("create space dirs");
    ...
    /// Start a worker for `space` and wait until its task finishes.
    async fn start_and_await_exit(sup: &mut DaemonSupervisor, space: &std::path::Path) {
        let (config, name) =
            build_worker_config(space.to_str().expect("utf8 path"), &test_identity())
                .expect("worker config builds");
        sup.start_space(config, name).await.expect("start worker");
        ...
        loop {
            ...
            assert!(
                ...
                "worker did not exit within the deadline"
            );
    ...
    async fn test_reap_carries_the_real_worker_error() {
        let dir = tempfile::tempdir().expect("tempdir");
        let space = write_test_space(dir.path());
        let _lock = kutl_client::try_lock_exclusive(&space.join(".kutl").join("producer"))
            .expect("lock call succeeds")
            .expect("test holds the producer lock");

        let mut sup = DaemonSupervisor::new();
        start_and_await_exit(&mut sup, &space).await;
        sup.reap_finished_workers().await;

        let statuses = sup.space_statuses();
        let SyncState::Error(msg) = &statuses[0].state else {
            panic!("expected error state, got {:?}", statuses[0].state);
        };
        assert!(
            msg.contains("already syncing"),
            "state must carry the worker's own message, got: {msg}"
        );
    }

    #[tokio::test(flavor = "multi_thread")]
    async fn test_maintain_restarts_a_failed_worker() {
        let dir = tempfile::tempdir().expect("tempdir");
        let space = write_test_space(dir.path());
        let lock = kutl_client::try_lock_exclusive(&space.join(".kutl").join("producer"))
            .expect("lock call succeeds")
            .expect("test holds the producer lock");

        let mut sup = DaemonSupervisor::new();
        sup.identity = Some(test_identity());
        start_and_await_exit(&mut sup, &space).await;
        sup.reap_finished_workers().await;

        // Release the conflicting producer and make the retry due.
        drop(lock);
        for handle in sup.workers.values_mut() {
            handle.started_at = std::time::Instant::now()
                .checked_sub(WORKER_RETRY_INTERVAL + std::time::Duration::from_secs(1))
                .expect("test instant math");
        }

        sup.maintain().await;

        let handle = sup.workers.values().next().expect("one worker");
        assert!(
            matches!(handle.state, SyncState::Connecting),
            "failed worker must be restarted once the retry interval elapsed, got {:?}",
            handle.state
        );
        sup.shutdown().await;
    }
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn test_promote_waits_for_the_heuristic_delay() {
        let mut sup = DaemonSupervisor::new();
        sup.workers.insert(
            "ca978112-ca1b-4dca-8ac2-31b39a23dc4d".into(),
            WorkerHandle {
                space_name: "alpha".into(),
                space_root: PathBuf::from("/tmp/a"),
                relay_url: "ws://127.0.0.1:1/ws".into(),
        ...
        assert!(
            matches!(handle.state, SyncState::Connecting),
            "a just-started worker must not be promoted yet, got {:?}",
            handle.state
        );

        for handle in sup.workers.values_mut() {
            handle.started_at = std::time::Instant::now()
                .checked_sub(SYNC_HEURISTIC_DELAY + std::time::Duration::from_secs(1))
                .expect("test instant math");
        }
        sup.promote_connecting_workers();
        let handle = sup.workers.values().next().expect("one worker");
        assert!(
            matches!(handle.state, SyncState::Synced),
            "a surviving worker must be promoted after the delay, got {:?}",
            handle.state
        );

        // The placeholder task ignores the cancel token; abort it directly.
        for handle in sup.workers.values_mut() {
            if let Some(task) = handle.task.take() {
                task.abort();
            }
