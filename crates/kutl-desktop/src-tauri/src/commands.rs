...
pub async fn complete_join(
    ...
    let join = prepare_join_state(&relay_url, token.as_deref(), &identity.display_name);

    // 2. If token provided, store to ~/.kutl/auth.toml.
    if let Some(creds) = join.credentials {
    ...
    // 3. Create folder and write .kutl/space.toml.
    ...
    // 4. Update ~/.kutl/spaces.toml registry (flock-protected).
    ...
    Ok(statuses
        ...
        .map(|s| {
            let state_str = match &s.state {
                daemon::SyncState::Connecting => "connecting".to_owned(),
                daemon::SyncState::Synced => "synced".to_owned(),
                daemon::SyncState::Error(_) => "error".to_owned(),
            };
            SpaceInfoResponse {
                space_id: s.space_id,
...
pub async fn stop_space(
    ...
    kutl_client::SpaceRegistry::update(|registry| {
        registry.spaces.retain(|path| {
            // `update` prunes entries whose config is missing; the only call
            // this closure makes is dropping the space being stopped.
            !kutl_client::SpaceConfig::load(std::path::Path::new(path))
                .is_ok_and(|config| config.space_id == target_space_id)
        });
    })
    .map_err(|e| format!("failed to update space registry: {e}"))?;
...
///
/// If any synced space is connected to the hosted relay, returns the
/// hosted-deployment URL. Otherwise returns the project homepage.
#[tauri::command]
...
/// The relay URL and credentials a join writes to disk, derived in one place.
struct JoinState {
    /// ws(s)-scheme relay URL written to the space's `.kutl/space.toml`.
    ws_url: String,
    /// Credentials to store when the join carried a token.
    credentials: Option<kutl_client::StoredCredentials>,
}

/// Derive everything a join persists from the incoming relay URL.
///
/// The wizard hands over the relay's http(s) base; the deep link hands over a
/// ws(s) URL. Both must land as one ws(s)-form URL used for the space config
/// AND the stored credentials: the daemon presents a stored token only when
/// the credential's `relay_url` is byte-identical to the space config's, so a
/// scheme or path mismatch silently drops auth for every future session.
fn prepare_join_state(relay_url: &str, token: Option<&str>, display_name: &str) -> JoinState {
    let ws_url = kutl_client::http_url_to_ws(relay_url);
    let credentials = token.map(|tok| kutl_client::StoredCredentials {
        token: tok.to_owned(),
...
mod tests {
    ...
    #[test]
    fn test_prepare_join_state_http_base_becomes_ws_everywhere() {
        let state = prepare_join_state("http://127.0.0.1:9100", Some("kutl_tok"), "boris");
        assert_eq!(state.ws_url, "ws://127.0.0.1:9100/ws");
        let creds = state.credentials.expect("token was provided");
        assert_eq!(
            creds.relay_url, state.ws_url,
            "credentials must store the exact URL the space config holds"
        );
    }

    #[test]
    fn test_prepare_join_state_ws_input_passes_through() {
        let state = prepare_join_state("wss://relay.example.com/ws", None, "boris");
        assert_eq!(state.ws_url, "wss://relay.example.com/ws");
