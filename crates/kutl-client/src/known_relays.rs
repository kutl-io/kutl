...
//! `$KUTL_HOME/known_relays.toml`. A `known_relays.json` written before the
//! TOML move still loads and is rewritten on first read (see
//! [`crate::text_file`]).
//!
...
impl KnownRelays {
    ...
    pub fn observe_at(
        ...
        if outcome != RelayPinOutcome::Known {
            crate::text_file::save(&path, &known, crate::file_lock::write_atomic)?;
        }
    ...
    /// Load from `path` (a `.toml` path; its `.json` sibling is the
    /// never-expiring fallback), or return an empty set when neither exists.
    ///
    ...
    pub fn load(path: &Path) -> Result<Self> {
        Ok(crate::text_file::load(path, crate::file_lock::write_atomic)?.unwrap_or_default())
    }
...
/// Path to the known-relays file: `$KUTL_HOME/known_relays.toml`.
///
/// # Errors
///
/// Returns an error if `$KUTL_HOME` cannot be resolved.
pub fn known_relays_path() -> Result<PathBuf> {
    Ok(crate::dirs::kutl_home()?.join("known_relays.toml"))
}
...
mod tests {
    ...
    /// A pre-move `known_relays.json` with more than one pinned relay loads
    /// through the fallback with every pin intact, is rewritten as TOML, and
    /// the JSON is gone.
    #[test]
    fn test_json_pins_load_and_are_rewritten() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("known_relays.toml");
        let json_path = dir.path().join("known_relays.json");
        let pinned = KnownRelays {
            relays: vec![
                KnownRelay {
                    url: "ws://one.example/ws".into(),
                    dids: vec!["did:key:zOne".into(), "did:key:zOneRotated".into()],
                },
                KnownRelay {
                    url: "ws://two.example/ws".into(),
                    dids: vec!["did:key:zTwo".into()],
                },
            ],
        };
        std::fs::write(&json_path, serde_json::to_string(&pinned).unwrap()).unwrap();

        let loaded = KnownRelays::load(&path).unwrap();
        assert_eq!(
            loaded.dids_for("ws://one.example/ws"),
            Some(["did:key:zOne".to_owned(), "did:key:zOneRotated".to_owned()].as_slice())
        );
        assert_eq!(
            loaded.dids_for("ws://two.example/ws"),
            Some(["did:key:zTwo".to_owned()].as_slice())
        );
        assert!(path.exists(), "rewritten as toml");
        assert!(!json_path.exists(), "json original removed");
        assert_eq!(KnownRelays::load(&path).unwrap().relays.len(), 2);
    }
