...
/// (`$KUTL_HOME/agents/<name>.toml`; a pre-move `<name>.json` still loads).
///
...
pub fn agent_identity_path(name: &str) -> Result<PathBuf> {
    ...
    Ok(kutl_home()?.join("agents").join(format!("{name}.toml")))
}
...
mod tests {
    ...
    fn test_agent_identity_path_shape() {
        ...
        assert_eq!(path, dir.path().join("agents").join("claude-laptop.toml"));
    }
