...
/// The sentinel text, as a macro so other crates can splice it into their
/// own `concat!`-built compile-time strings instead of hand-copying it.
/// `#[macro_export]` places this at the crate root: downstream callers
/// invoke it as `kutl_client::surface_sentinel_header!()` regardless of
/// which module they're in.
#[macro_export]
macro_rules! surface_sentinel_header {
    () => {
...
/// Agents read this line and recognize that the file is a mirror, not a
/// source of truth.
pub const SURFACE_SENTINEL_HEADER: &str = surface_sentinel_header!();
