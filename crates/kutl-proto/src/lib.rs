...
/// Messages the daemon persists on disk inside the kutl envelope. Never
/// sent over the wire, so they are kept out of the TypeScript codegen.
pub mod daemon {
    #![allow(clippy::doc_markdown)]
    include!(concat!(env!("OUT_DIR"), "/kutl.daemon.v1.rs"));
}
