...
/// Capability advertised in `HandshakeAck.features` by a relay that answers
/// the `Barrier` frame. A client closes through a barrier only when the
/// relay advertises this; against an older relay it sends the close frame
/// directly, as it always did.
pub const BARRIER_CAPABILITY: &str = "barrier";
/// Capability advertised in `HandshakeAck.features` by a relay that accepts
/// client-pushed signal history (the `SignalReseed` frame). Serving history
/// needs no capability: every relay answers `SubscribeSignals` with a page,
/// empty when it keeps no records. Accepting history is the relay's policy;
/// the string is shared here so no client carries a copy.
pub const SIGNAL_RESEED_CAPABILITY: &str = "signal-reseed";

/// Absolute upper bound on a single blob's raw bytes. This is the POLICY
/// cap, enforced against the blob bytes themselves on both sides (the daemon
/// refuses to sync a larger file; the relay rejects a larger upload); real
/// per-tier limits are smaller and enforced via `QuotaBackend::load_space_quota`.
pub const ABSOLUTE_BLOB_MAX: usize = 25 * 1024 * 1024;

/// Allowance above [`ABSOLUTE_BLOB_MAX`] for the protocol envelope a blob
/// rides in (message type tag, ids, metadata — hundreds of bytes in
/// practice). 64 KiB dwarfs any real envelope while staying negligible
/// against the blob cap. The bound is enforced by the codec itself: a
/// message whose envelope (a client-supplied snippet included) pushes it
/// past [`WS_MESSAGE_MAX`] is rejected at the frame cap before any
/// application code sees it, so nothing has to validate the envelope's
/// size separately.
const WS_ENVELOPE_HEADROOM: usize = 64 * 1024;

/// The WebSocket codec-layer message/frame cap: the blob policy cap plus
/// envelope headroom. Every WS codec on the sync path (daemon client,
/// library client, relay accept, and any proxy in between) must use THIS
/// value, not [`ABSOLUTE_BLOB_MAX`]: a blob at exactly the policy cap
/// serializes into a message slightly larger than the cap, and a codec
/// capped at the blob cap alone rejects it on delivery — the catch-up
/// replay then re-serves the same message forever and the subscription
/// silently wedges. The headroom loosens no policy: blob size is checked
/// against the raw bytes at the application layer on both sides.
pub const WS_MESSAGE_MAX: usize = ABSOLUTE_BLOB_MAX + WS_ENVELOPE_HEADROOM;
...
/// A relay's refusal, read from the `Error` its typed reply carries (the
/// refused `HandshakeAck`, a refused `SubscribeStatus`, a `SyncOpsRejected`).
///
...
/// subscribing client.
///
/// `Some(reason)` when no catch-up `SyncOps` will follow: the relay refused
/// the subscribe itself (`error` set — no slot, no subscription), or it
/// classified the document's read as compromised (`Inconsistent` /
/// `Unavailable`) and short-circuited. A subscriber that ignores the frame
/// cannot distinguish "empty document" from "refused" or "wounded slot" and
/// silently treats both as emptiness. `None` for Found/Empty/Unspecified
/// without an error, where catch-up (or genuine emptiness) follows normally.
/// One classifier for all consumers: per-client readings of this frame
/// drift, and a consumer with no reading fails open. A consumer that needs
/// the refusal's verdict (`auth_failed`) reads `error` through
/// [`handshake_refusal`] first; this classifier only says whether to wait.
pub fn subscribe_status_failure(s: &sync::SubscribeStatus) -> Option<String> {
    if let Some(e) = &s.error {
        return Some(format!(
            "relay refused the subscribe ({})",
            handshake_refusal(e).message
        ));
    }
...
/// Build a `SyncOpsRejected` envelope: the relay's refusal of one `SyncOps`
/// frame, scoped to its document by the fields.
pub fn sync_ops_rejected_envelope(
    space_id: &str,
    document_id: &str,
    error: sync::Error,
) -> SyncEnvelope {
    SyncEnvelope {
        payload: Some(Payload::SyncOpsRejected(sync::SyncOpsRejected {
            space_id: space_id.to_owned(),
            document_id: document_id.to_owned(),
            error: Some(error),
        })),
...
///
/// On failure, callers pass `entry = None` and populate `error` with the
/// refusal record every typed reply carries.
...
mod tests {
    ...
    /// A refused subscribe arrives with `error` set and `load_status`
    /// unspecified — the shape a Found/Empty pass-through would wave past.
    /// The classifier must answer on the error, not the status.
    #[test]
    fn test_subscribe_status_failure_fails_closed_on_refusal() {
        let refused = sync::SubscribeStatus {
            space_id: "s".to_owned(),
            document_id: "d".to_owned(),
            load_status: sync::LoadStatus::Unspecified.into(),
            error: Some(sync::Error {
                code: sync::ErrorCode::AuthFailed.into(),
                message: "not a member".to_owned(),
                ..Default::default()
            }),
            ..Default::default()
        };
        let reason = subscribe_status_failure(&refused).expect("refusal fails closed");
        assert!(
            reason.contains("not a member"),
            "reason carries the relay's text: {reason}"
        );

        let found = sync::SubscribeStatus {
            load_status: sync::LoadStatus::Found.into(),
            ..refused.clone()
        };
        assert!(
            subscribe_status_failure(&found).is_some(),
            "an error outranks a Found status"
        );
    }

    /// Without an error the reading is by status: compromised reads fail
    /// closed, Found/Empty/Unspecified let catch-up follow.
    #[test]
    fn test_subscribe_status_failure_reads_the_load_status() {
        let with_status = |status: sync::LoadStatus| sync::SubscribeStatus {
            load_status: status.into(),
            ..Default::default()
        };
        assert!(subscribe_status_failure(&with_status(sync::LoadStatus::Inconsistent)).is_some());
        assert!(subscribe_status_failure(&with_status(sync::LoadStatus::Unavailable)).is_some());
        assert!(subscribe_status_failure(&with_status(sync::LoadStatus::Found)).is_none());
        assert!(subscribe_status_failure(&with_status(sync::LoadStatus::Empty)).is_none());
        assert!(subscribe_status_failure(&with_status(sync::LoadStatus::Unspecified)).is_none());
    }
    ...
    fn test_constants_match_contract_file() {
        ...
        assert_eq!(
            contract["ws_message_max"],
            serde_json::json!(WS_MESSAGE_MAX)
        );
    }
}
