...
/// `RegistryBackend::save_entry` mirror have committed. On success, `entry`
/// carries the persisted snapshot whose `path` is the post-arbitration
/// effective path — the renamer reconciles its local file to it (a rename that
/// lost a path collision is conflict-copied, so the effective path differs from
/// the requested one).
