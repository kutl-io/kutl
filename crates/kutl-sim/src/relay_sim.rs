...
use kutl_proto::protocol::{decode_envelope, encode_envelope, sync_ops_envelope};
...
/// Capacity of the modeled relay→daemon data lane (the shipped one is
/// `RelayConfig::outbound_capacity`, default 512; the sim keeps a small
/// lane so slow-subscriber eviction is reachable in a short run).
...
pub struct RelaySimulation {
    ...
    /// Receivers for relay → daemon control messages (broadcasts: stale
    /// notices, lifecycle fan-out).
    daemon_ctrl_rx: Vec<mpsc::Receiver<Vec<u8>>>,
    /// Receivers for relay → daemon own-acks (handshake/register/rename acks,
    /// `SubscribeStatus`, errors). Drained but not forwarded — this sim drives
    /// daemon docs by direct merge, not by replaying acks.
    daemon_ack_rx: Vec<mpsc::UnboundedReceiver<Vec<u8>>>,
...
/// Identity every simulated daemon connection authenticates as.
///
/// Authentication is mandatory. The simulation has no HTTP transport
/// to run the real challenge-response flow, so it authorizes this DID via an
/// `authorized_keys` file and attaches it to every daemon connection with
/// [`Relay::test_set_authenticated`]. Attribution is irrelevant to the sim
/// (which checks CRDT content convergence, driven by per-op agent ids), so a
/// single shared identity is sufficient.
const SIM_DID: &str = "did:key:zSimDaemon";

impl RelaySimulation {
    /// Create a new relay-mediated simulation with the given PRNG seed.
    pub fn new(seed: u64) -> Self {
        // Authentication is mandatory; authorize `SIM_DID` for every
        // space via a (uniquely-named, leaked) temp `authorized_keys` file.
        let keys_path = std::env::temp_dir().join(format!("kutl-sim-authorized-keys-{seed}"));
        std::fs::write(&keys_path, format!("{SIM_DID}\n")).expect("write sim authorized_keys file");
    ...
    pub fn add_daemon(&mut self, name: &str, space_id: &str, doc_id: &str) -> Result<DaemonId> {
        ...
        // Create mpsc channels for relay → daemon (data + control + own-ack).
        ...
        self.daemon_ack_rx.push(ack_rx);
        ...
        // Authentication is mandatory. The sim has no HTTP transport
        // for the real handshake token flow, so attach the authorized identity
        // directly rather than sending a (rejected) empty-token Handshake.
        self.relay.test_set_authenticated(conn_id, SIM_DID);
    ...
    pub fn reconnect_daemon(&mut self, daemon: DaemonId) {
        ...
        // New channels for relay → daemon (data + control + own-ack).
        let (tx, rx) = mpsc::channel(RELAY_OUTBOUND_CAPACITY);
        let (ctrl_tx, ctrl_rx) = mpsc::channel(DAEMON_CTRL_CAPACITY);
        let (ack_tx, ack_rx) = mpsc::unbounded_channel();
        self.daemon_rx[daemon] = rx;
        self.daemon_ctrl_rx[daemon] = ctrl_rx;
        self.daemon_ack_rx[daemon] = ack_rx;

        let conn_id = self.next_conn_id;
        self.next_conn_id += 1;
        self.conn_ids[daemon] = conn_id;

        // Register + authenticate + subscribe (same as add_daemon).
        pollster::block_on(self.relay.process_command(RelayCommand::Connect {
            conn_id,
            tx,
            ctrl_tx,
        ...
        // Authentication is mandatory; attach the authorized identity
        // directly (the sim has no HTTP transport for the real handshake flow).
        self.relay.test_set_authenticated(conn_id, SIM_DID);
    ...
    fn send_daemon_ops(&mut self, daemon: DaemonId) {
        ...
        // Sim broadcast carries no author snapshot (only catch-up carries one).
    ...
    fn drain_relay_outbound(&mut self) {
        ...
        for daemon in 0..self.daemon_rx.len() {
            ...
            // Drain control + own-ack channels (broadcasts / acks) — not
            // forwarded via network; this sim merges docs directly.
            while self.daemon_ctrl_rx[daemon].try_recv().is_ok() {}
            while self.daemon_ack_rx[daemon].try_recv().is_ok() {}
        }
    }
