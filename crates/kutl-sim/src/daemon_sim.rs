...
pub struct DaemonSim {
    ...
    /// registered them.
    ///
    /// The order is load-bearing (the rename-race class, TDD case 11): a rename
    /// registers the removal half `(old, None)` BEFORE the write half
    /// `(new, Some(hash))` — the `rename_doc` funnel order (daemon/effects.rs).
    /// A local create consults this set via `consume_write_echo` semantics: a
    /// Create/Modify whose bytes hash matches a `Some(hash)` entry at its path
    /// is the daemon's OWN write echo (suppressed); a different-bytes create —
    /// or a create at a `None` removal half — is a GENUINE edit and is never
    /// swallowed (`consume_write_echo`, watcher/suppression.rs).
...
impl DaemonSim {
    ...
    /// (the ghost-resurrection class is structural, graft 2).
    #[must_use]
    ...
    pub fn scan_mints_uuid_for(&mut self, rel: &str) -> String {
        ...
        // idle-heal rule forbids. The assert keeps the model honest.
        assert!(
            self.state
                .shadow
                .shadow_occupant
    ...
    /// `(new, Some(hash))` — the same order `rename_doc` uses. Reversed, a
    /// concurrent watcher event between the halves attributes the write to
    /// the wrong document (the rename-race corruption class). The single
    /// owner of that fact: every rename-shaped disk effect routes through
    /// here so the two halves cannot drift apart at one site while the
    /// suppression-order test keeps passing against the other.
    ...
    /// rename-race §1.3/§4.2 class depends on.
    #[must_use]
    ...
    /// next reconcile re-derives and retries), the write
    /// echo-suppression the funnel computes from the content (`write_doc`),
    /// the disk mutation,
    ...
    /// stat fails).
    fn apply_write_file(&mut self, rel: PathBuf, content: Vec<u8>) {
        if self.fail_next_fs_effect {
            self.fail_next_fs_effect = false;
            return;
        }
        // Write suppression keyed by the content hash, computed here exactly
        // as the real driver's funnel does (`write_doc`) — never carried.
        self.suppressions
            .push((rel.clone(), Some(kutl_daemon::sha256_bytes(&content))));
        self.write_seq += 1;
    ...
    ///   case), `register_hlc` reloads per entry (the dropped-persistence/A1
    ...
    fn apply_guarded_place(
        ...
        if let kutl_daemon::core::PlaceKind::Rename { old_rel } = place_kind {
            ...
            // load-bearing — the rename-race §1.3/§4.2 class (TDD case 11).
    ...
    /// live inode; `record_local_register_inode`) — the recorded view
...
mod tests {
    ...
    /// intake is non-empty the `SaveState` stays a dirty flag (the
    /// save-coalescing rule), so a crash before the flush restarts from the
    /// STALE snapshot and the just-minted identity is gone — the exact
    /// precondition of the crash-mid-bulk cluster-delete bug.
    #[test]
