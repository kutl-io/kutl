//! Integration tests: the flush task writes the durability witness
//! after successful backend save, and does NOT write it when the save
//! fails. Also verifies that `FlushCompleted` is suppressed on
//! witness-update failure so the slot stays dirty and the next cycle
//! retries.
//!
//! These tests drive `run_flush_task` directly with stub backends and a
//! stub "actor" (an `mpsc::channel` we control). No axum, no real
//! database — the invariant under test is narrow and the stubs give
...
/// `ContentBackend::save` fails → witness NOT advanced.
...
/// Save succeeds but `update_witness` fails. The
...
/// a later cold-load that genuinely has bytes.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
