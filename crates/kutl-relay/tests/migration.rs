...
const TEST_SPACE: &str = "5171e0a1-0000-4000-8000-000000000007";
...
/// Build a relay config that authorizes [`TEST_DID`] for every space via a
/// (uniquely-named, leaked) temp `authorized_keys` file — required because
/// authentication is mandatory.
fn test_config() -> RelayConfig {
    use std::sync::atomic::{AtomicU64, Ordering};
    static COUNTER: AtomicU64 = AtomicU64::new(0);
    let keys_path = std::env::temp_dir().join(format!(
        "kutl-relay-migration-keys-{}-{}",
        std::process::id(),
