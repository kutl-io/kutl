...
async fn test_http_create_invite_unknown_space_returns_404() {
    ...
    let resp = client
        .post(format!("{base_url}/invites"))
        .json(&serde_json::json!({"space_id": "0a0b534c-4dd0-4e6e-8d77-58e7813a943e"}))
        .send()
        .await
        .unwrap();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_http_invite_routes_absent_without_invite_backend() {
    use kutl_relay::config::RelayConfig;

    // Invite-route mounting is gated on the invite backend: a
    // host that manages invites elsewhere (kutlhub) passes `invite_backend =
    // None`, and the /invites routes are then not mounted. The OSS binary always
    // has an invite backend, so construct the storeless (no-backend) shape
    // explicitly via the host seam to exercise the absent-route path.
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap().to_string();

    // Authentication is mandatory; seed an (empty) authorized_keys
    // file so the boot assert passes. This test is about invite-route mounting,
    // not auth.
    ...
    let resp = client
        ...
        .json(&serde_json::json!({"space_id": "d6a7cd2a-7371-41a1-8d54-3196979ff74f"}))
        .send()
        .await
        .unwrap();
    ...
    assert!(
        ...
        "invite routes should not be mounted without an invite backend, got: {}",
        resp.status()
    );
}
