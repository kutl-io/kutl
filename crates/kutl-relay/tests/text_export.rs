...
/// Regression test: the "register → `send_ops`" pattern (no prior
/// subscribe) must persist content. Before
/// `register_document_internal` materialized an empty `DocSlot`,
/// `handle_sync_ops` would `NotFound` the immediately-following ops
/// because the in-memory slot only got created by `subscribe`. The
/// registry entry was still written by `register`, so the symptom
/// was a registered document whose content reads back empty.
#[tokio::test]
async fn test_register_then_sync_ops_without_subscribe_persists_content() {
    ...
    // Handshake.
    let envelope = handshake_envelope_with_token("worker-test", &token, "");
    let bytes = encode_envelope(&envelope);
    ws.send(tokio_tungstenite::tungstenite::Message::Binary(
        bytes.into(),
    ))
    ...
    // Send content ops immediately after register. Pre-fix this would
    // be dropped by `handle_sync_ops` with a NotFound error envelope.
    let mut doc = kutl_core::Document::new();
    ...
    let ops = doc.encode_since(&[]);
    let metadata = doc.changes_since(&[]);
    let envelope = sync_ops_envelope("space1", "doc1", ops, metadata);
    let bytes = encode_envelope(&envelope);
    ws.send(tokio_tungstenite::tungstenite::Message::Binary(
        bytes.into(),
    ))
