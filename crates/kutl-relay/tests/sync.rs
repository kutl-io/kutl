...
/// Connect and handshake a client. Drains the handshake ack from the own-ack
/// lane so subsequent assertions see only later frames.
async fn connect_client(
    ...
    // Drain the handshake ack from the own-ack lane.
...
/// that evicts also drops it (the permanent-strand bug the lane move fixed).
...
async fn test_relay_forwards_blob() {
    ...
    client_a
        .send_blob("space", "img.png", blob.clone(), forged_hash, Some(meta))
        .await;
...
async fn test_blob_catch_up() {
    ...
    client_a
        .send_blob("space", "doc.pdf", blob.clone(), forged_hash, Some(meta))
        .await;
...
/// a completely FULL ctrl lane cannot drop it (on the old bounded-ctrl path
/// the very storm that evicted also dropped the only re-subscribe signal —
/// the permanent-strand bug). Proves ctrl backpressure neither crashes the
...
/// Drain a `Signal` broadcast from a subscriber's ctrl channel. Tolerates a
...
async fn recv_signal(ctrl_rx: &mut mpsc::Receiver<Vec<u8>>) -> sync::Signal {
    ...
    panic!("too many non-Signal envelopes before Signal");
}
...
async fn test_signal_flag_space_sentinel() {
    ...
    // typed as uuid in production, space-wide signals
...
async fn test_live_peer_document_registration_broadcast() {
    ...
    c1.send_envelope(&reg_a).await;
    // RFD 0042 amendment 2026-05-24: sender receives a
    // RegisterDocumentAck on its own connection. Drain it so the
    // subsequent `recv_envelope` calls land on the broadcast for
    // c2's `doc-b` register.
