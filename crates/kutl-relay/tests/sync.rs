...
use kutl_proto::protocol::{
    PROTOCOL_VERSION_MAJOR, PROTOCOL_VERSION_MINOR, decode_envelope, handshake_envelope,
    handshake_envelope_with_token, presence_update_envelope, subscribe_envelope,
};
...
/// transparently inside [`handshake`], so most tests need no
...
async fn start_relay() -> String {
    ...
    // In-memory boot: these sync tests use in-memory registries; the
    // OSS binary's build_app requires a data dir, so construct the no-backend
    // shape explicitly via the test seam.
...
async fn start_relay_with_auth() -> (String, tempfile::NamedTempFile) {
    ...
    // In-memory boot: see `start_relay`.
    let (app, _relay_handle, _flush_handle) = common::build_in_memory_app(config);
    tokio::spawn(async move {
        axum::serve(listener, app).await.unwrap();
    });
...
/// Enroll a fresh `test_keypair()` DID into the relay's `authorized_keys`
/// file, mint a real bearer token, and complete the handshake.
///
...
use common::{RECV_TIMEOUT as WS_RECV_TIMEOUT, TestClient};
...
/// connection in the space's signal recipient set. The backlog page the
/// listen is answered with stays on the connection's ack lane until a test
/// drains it.
...
/// broadcast to the space's listeners). The document id is derived from the
...
/// Is this frame the notice that a signal stream PAUSED (as opposed to a
/// refused subscribe or a records outage, which carry other causes)?
fn is_pause_notice(env: &SyncEnvelope) -> bool {
    matches!(
        &env.payload,
        Some(Payload::StaleSignalStream(s))
            if s.cause() == sync::StaleStreamReason::PausedLaneFull
    )
}
...
async fn test_oversized_blob_rejected() {
    // A blob just over the POLICY cap must be rejected by the
    // application-layer defense-in-depth check in `handle_blob_sync_ops`.
    //
    // Enforcement is based on `msg.ops.len()` (server-measured) — never on
    // the client-advertised `blob_size` field — so the frame carries ACTUAL
    // ops bytes just over the cap, not a lied `blob_size`. The WS codec
    // admits it (its cap is `WS_MESSAGE_MAX`, the blob cap plus envelope
    // headroom, so an at-cap blob's envelope-larger message can be
    // DELIVERED), which makes this the layer that genuinely holds the
    // policy boundary: the relay answers with a quota error on a connection
    // that stays open.
    let addr = start_relay().await;
    ...
    assert_eq!(
        err.code,
        i32::from(sync::ErrorCode::QuotaExceeded),
        "an over-policy blob draws the app-layer quota rejection, got {err:?}"
    );
    assert!(
        err.details.contains("blob_size"),
        "the rejection names the blob-size cap, got {:?}",
        err.details
    );
}
...
async fn test_ws_rejects_oversize_message() {
    ...
    // Just over the CODEC cap (the blob cap plus envelope headroom): the
    // codec drops it before application code ever runs.
    let oversize = vec![0u8; kutl_proto::protocol::WS_MESSAGE_MAX + 1];
    let _ = ws.send(tungstenite::Message::Binary(oversize.into())).await;

    // Server should close the connection (or the send itself errored after
    // close). Bounded read: a codec that silently ACCEPTS the frame yields
    // no reply at all, and an unbounded read would wedge the suite instead
    // of failing it.
    let result = tokio::time::timeout(WS_RECV_TIMEOUT, ws.next())
        .await
        .expect("no close within the recv timeout — the codec accepted an over-cap frame");
    assert!(
        matches!(
            result,
...
async fn test_unauthenticated_handshake_rejected() {
    ...
    let err = client.recv_handshake_refusal().await;
...
/// An authenticated client whose DID is not authorized for the space is
/// refused at subscribe: the refusal rides the `SubscribeStatus` every
/// subscribe gets, correlated to the document, with no load verdict — and
/// nothing else follows (no slot, no subscription, no catch-up).
#[tokio::test]
async fn test_subscribe_without_auth_rejected() {
    // The keys file is empty: the DID authenticates (any DID can mint a
    // bearer) but is authorized for no space.
    let (addr, _keys) = start_relay_with_auth().await;
    let (did, signing_key) = common::test_keypair();
    let mut client = TestClient::connect(&addr).await;
    client.handshake_as(&did, &signing_key).await;
    ...
    client
        .send_envelope(&subscribe_envelope("some-space", "some-doc"))
        .await;

    let reply = client.recv_envelope().await;
    match reply.payload {
        Some(Payload::SubscribeStatus(status)) => {
            assert_eq!(status.space_id, "some-space");
            assert_eq!(status.document_id, "some-doc");
            assert_eq!(
                status.load_status,
                i32::from(sync::LoadStatus::Unspecified),
                "a refused subscribe carries no load verdict"
            );
            let err = status.error.expect("a refused subscribe carries its error");
            assert_eq!(err.code, i32::from(sync::ErrorCode::AuthFailed));
        }
        other => panic!("expected a refused SubscribeStatus, got {other:?}"),
    }
    assert!(
        tokio::time::timeout(common::SUBSCRIBE_DRAIN_TIMEOUT, client.ws.next())
            .await
            .is_err(),
        "nothing follows a refused subscribe"
    );
}
...
async fn test_ctrl_channel_full_relay_continues() {
    ...
    subscribe_to_doc(&mut relay, writer.conn_id).await;
    subscribe_to_doc(&mut relay, slow.conn_id).await;
    listen(&mut relay, slow.conn_id).await;

    // Fill the slow client's 1-slot ctrl lane with a lifecycle broadcast (a
    // register in the space reaches every listener). Left undrained, the slot
...
/// A signal frame that finds a listener's data lane full pauses that
/// listener's signal stream and says so once. The listener stays present:
/// lifecycle broadcasts still reach it on the ctrl lane, later signals are
/// skipped without a second notice, and its next `SubscribeSignals` resumes
/// delivery. The backlog page takes no data slot: it is the reply to the
/// listener's own request and rides its own-ack lane.
#[tokio::test]
async fn test_full_data_lane_pauses_the_signal_stream_but_not_listening() {
    /// Three data slots: two signals fit above the document-headroom reserve
    /// (one slot of three), and the third must yield.
    const THREE_SLOT_DATA: usize = 3;
    let mut relay = Relay::new_standalone(test_relay_config());
    let author = connect_client(
        &mut relay,
        1,
        "did:key:zAlice",
        FAST_CHANNEL_CAPACITY,
    ...
    .await;
    let mut listener = connect_client(
        &mut relay,
        2,
        "did:key:zBob",
        THREE_SLOT_DATA,
    ...
    .await;
    listen(&mut relay, listener.conn_id).await;

    let flag = |message: &str| {
        make_flag_signal(
            TEST_SPACE,
            TEST_DOC,
            "did:key:zAlice",
    ...
    let stale_notices =
        |frames: &[SyncEnvelope]| frames.iter().filter(|e| is_pause_notice(e)).count();
    let signals = |frames: &[SyncEnvelope]| {
        frames
            .iter()
            .filter(|e| matches!(e.payload, Some(Payload::Signal(_))))
            .count()
    };

    assert!(
        drain_ack(&mut listener.ack_rx)
            .iter()
            .any(|e| matches!(e.payload, Some(Payload::SignalPage(_)))),
        "the backlog page answers on the own-ack lane"
    );
    for message in ["one", "two", "three"] {
        relay
            .process_command(RelayCommand::Signal {
                conn_id: author.conn_id,
                msg: flag(message),
            })
            .await;
    }
    assert_eq!(
        stale_notices(&drain_ack(&mut listener.ack_rx)),
        1,
        "the signal that found the lane at its reserve is reported once, on the ack lane"
    );

    // Still listening: a lifecycle broadcast reaches it on ctrl.
    register_doc(&mut relay, author.conn_id, "after-pause.md").await;
    assert!(
        drain_ctrl(&mut listener.ctrl_rx)
            .iter()
            .any(|e| matches!(e.payload, Some(Payload::RegisterDocument(_)))),
        "a paused stream does not cost the listener its lifecycle broadcasts"
    );

    // A further signal is skipped, not re-notified.
    relay
        .process_command(RelayCommand::Signal {
            conn_id: author.conn_id,
            msg: flag("four"),
    ...
    assert_eq!(
        stale_notices(&drain_ack(&mut listener.ack_rx)),
        0,
        "a paused stream is skipped, not re-notified per broadcast"
    );
    assert_eq!(
        signals(&drain_data(&mut listener.data_rx)),
        2,
        "exactly the signals that fit were delivered"
    );

    // Re-subscribing (with the lane drained) sets the stream flowing again.
    listen(&mut relay, listener.conn_id).await;
    relay
        .process_command(RelayCommand::Signal {
            conn_id: author.conn_id,
...
async fn test_signal_unspecified_audience_rejected() {
    ...
    match envelope.payload {
        Some(Payload::SignalAck(ack)) => {
            assert!(!ack.success, "the flag was refused");
            let error = ack.error.expect("a refusal carries its error");
            assert_eq!(error.code(), sync::ErrorCode::InvalidMessage);
            assert!(error.message.contains("audience"), "got: {}", error.message);
        }
        other => panic!("expected a refused SignalAck, got {other:?}"),
    }
}

/// A merge reject (`SyncOpsRejected`) names the document it rejects, so a client can
...
async fn test_merge_reject_error_names_the_document() {
    ...
    match envelope.payload {
        Some(Payload::SyncOpsRejected(r)) => {
            let e = r.error.expect("a refusal carries its error");
            assert_eq!(e.code, i32::from(sync::ErrorCode::InvalidMessage));
            assert_eq!(r.space_id, TEST_SPACE, "reject names the space");
            assert_eq!(r.document_id, TEST_DOC, "reject names the document");
        }
...
async fn test_signal_participant_needs_target() {
    ...
    match envelope.payload {
        Some(Payload::SignalAck(ack)) => {
            ...
            assert_eq!(error.code(), sync::ErrorCode::InvalidMessage);
            assert!(
                error.message.contains("target_did"),
                "got: {}",
                error.message
            );
        }
...
async fn test_live_peer_document_registration_broadcast() {
    ...
    // Client 1 listens to the space, as a daemon does at startup: lifecycle
    // broadcasts reach listeners, not document subscribers. The relay
    // answers with the backlog page (empty here).
    c1.send_envelope(&sync::SyncEnvelope {
        payload: Some(Payload::SubscribeSignals(sync::SubscribeSignals {
            space_id: space.into(),
            cursor: None,
        })),
    })
    ...
    assert!(
        matches!(
            c1.recv_envelope().await.payload,
            Some(Payload::SignalPage(_))
        ),
        "client 1's backlog page"
    );
    c1.subscribe(space, "00000000-0000-0000-0000-00000000000a")
        .await;
    ...
    // Client 2 first discovers doc-a via ListSpaceDocuments (a pure query).
    let list_req = sync::SyncEnvelope {
        payload: Some(Payload::ListSpaceDocuments(sync::ListSpaceDocuments {
            space_id: space.into(),
        })),
...
impl AfterMergeObserver for CapturingAfterMerge {
    async fn after_text_merge(&self, event: MergedEvent, _content: &str) {
...
async fn test_sync_ops_without_authorization_rejected() {
    ...
    // In-memory boot: in-memory registries; build_app requires a
    // data dir, so construct the no-backend shape via the test seam.
    let (app, _relay_handle, _flush_handle) = common::build_in_memory_app(config);
    tokio::spawn(async move {
        axum::serve(listener, app).await.unwrap();
    });
...
/// An authenticated-but-unauthorized client gets a failure
/// `RegisterDocumentAck` for a lifecycle op, and nothing else: the ack is
/// the refusal, it names the document, and it resolves the sender's pending
/// register — silence would be indistinguishable from a lost frame.
#[tokio::test]
async fn test_register_without_authorization_gets_failure_ack() {
    let (did_a, _key_a) = common::test_keypair();
    let (did_b, key_b) = common::test_keypair();

    // Only A's DID is authorized; B holds a valid token for an unlisted DID.
    let keys_dir = tempfile::tempdir().unwrap();
    let keys_path = keys_dir.path().join("authorized_keys");
    std::fs::write(&keys_path, &did_a).unwrap();
    ...
    client_b
        .send_envelope(&handshake_envelope_with_token(
            "b7c0a273-25d5-4be0-9f2c-27a06c05f78e",
            &token_b,
            "",
        ))
        .await;
    ...
    client_b
        .send_envelope(&SyncEnvelope {
            payload: Some(Payload::RegisterDocument(sync::RegisterDocument {
                space_id: "some-space".into(),
                document_id: "5b7f7a94-8a51-4f5b-9c39-0a4f24aa88aa".into(),
                path: "doc.md".into(),
                metadata: Some(sync::ChangeMetadata {
    ...
    // The failure ack is the first and only frame on the own-ack lane.
    let reply = client_b.recv_envelope().await;
    match reply.payload {
        Some(Payload::RegisterDocumentAck(reg_ack)) => {
            assert!(
                !reg_ack.success,
                "auth-failed register must ack success=false"
            );
            assert_eq!(
                (reg_ack.space_id.as_str(), reg_ack.document_id.as_str()),
                ("some-space", "5b7f7a94-8a51-4f5b-9c39-0a4f24aa88aa"),
                "the refusal must correlate to the register it answers"
            );
            let ack_err = reg_ack.error.expect("failure ack carries the error");
            assert_eq!(ack_err.code, i32::from(sync::ErrorCode::AuthFailed));
        }
        other => panic!("expected the failure RegisterDocumentAck, got {other:?}"),
    }
}
...
async fn test_a_signal_burst_does_not_evict_an_actively_syncing_document() {
    ...
    listen(&mut relay, reader.conn_id).await;
