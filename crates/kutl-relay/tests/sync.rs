...
/// An authenticated-but-unauthorized client must get BOTH the legacy `Error`
/// frame and a correlating failure `RegisterDocumentAck` for a lifecycle op.
/// The ack is what resolves the sender's pending register — silence would be
/// indistinguishable from a lost frame.
#[tokio::test]
async fn test_register_without_authorization_gets_error_and_failure_ack() {
    ...
    // The error frame arrives first, then the failure ack on the same lane.
    let err = client_b.recv_error().await;
    assert_eq!(err.code, i32::from(sync::ErrorCode::AuthFailed));

    let follow_up = client_b.recv_envelope().await;
    match follow_up.payload {
        ...
        other => {
            panic!("expected failure RegisterDocumentAck after the error frame, got {other:?}")
        }
    }
}
