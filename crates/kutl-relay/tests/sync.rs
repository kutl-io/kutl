...
async fn test_signal_flag_cross_document() {
    ...
    subscribe_to(&mut relay, 1, TEST_SPACE, "doc1").await;
    subscribe_to(&mut relay, 2, TEST_SPACE, "doc2").await;
    common::drain_subscribe_status(&mut a.ctrl_rx).await;
    common::drain_subscribe_status(&mut b.ctrl_rx).await;
    ...
    let signal = recv_signal(&mut b.ctrl_rx).await;
    assert_eq!(signal.document_id.as_deref(), Some("doc1"));
    match &signal.payload {
        Some(sync::signal::Payload::Flag(f)) => {
...
async fn test_signal_flag_space_sentinel() {
    ...
    // Send with _space sentinel document_id.
    let msg = make_flag_signal(
        TEST_SPACE,
        "_space",
        "did:key:zAlice",
    ...
    // B receives it with _space document_id preserved.
    let signal = recv_signal(&mut b.ctrl_rx).await;
    assert_eq!(signal.document_id.as_deref(), Some("_space"));
    match &signal.payload {
        Some(sync::signal::Payload::Flag(f)) => {
