...
/// — `edit_document` no longer auto-creates (RFD 0083 §1).
...
async fn test_mcp_read_log_attribution() {
    ...
    create_doc_via_mcp(&client, &addr, &token, &session_id, "space1", "doc1", "").await;
...
async fn test_mcp_sse_notifications() {
    ...
    // Seed the doc, then edit (edit_document no longer auto-creates).
    create_doc_via_mcp(&client2, &addr, &token2, &session_id2, "space1", "doc1", "").await;
...
/// persistent-mode relay (with `SQLite` `SpaceBackend`) now rejects MCP
/// `create_document` / `upload_blob` against unknown spaces per
/// RFD 0083 §4 (audit 2026-05-20). The tests using this fixture
...
async fn start_mcp_relay_with_data_dir() -> (String, tempfile::TempDir, tempfile::NamedTempFile) {
    ...
    let client = reqwest::Client::new();
    common::register_space(&client, &format!("http://{addr}"), "space1").await;
...
async fn test_mcp_initialize_with_did_no_auth() {
    ...
    // Boundary-normalization (parking-lot fix): the relay's MCP handlers
    // now canonicalize `space_id` against the space backend at entry, so
