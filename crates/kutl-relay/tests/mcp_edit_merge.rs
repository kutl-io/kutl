...
/// The space every test here writes to. Storeless boot materializes it on
...
/// rather than threading four strings through every call.
struct Writer {
    addr: String,
    session_id: String,
    token: String,
...
impl Writer {
    ...
    async fn call(&self, tool: &str, arguments: serde_json::Value) -> serde_json::Value {
        let resp = mcp_request(
            &self.client,
            &self.addr,
            &self.token,
    ...
    async fn create(&self, path: &str, content: &str) -> String {
        let body = self
            ...
            .await;
        ok_result(&body, "create_document")["document_id"]
            .as_str()
    ...
    async fn read(&self, document_id: &str) -> (String, String) {
        let body = self
            ...
            .await;
        let result = ok_result(&body, "read_document");
        (
            result["content"]
                .as_str()
    ...
    /// Write `content` against the base named by `base_version`, returning the
    /// raw JSON-RPC body so a test can assert on either outcome.
    async fn edit(
        &self,
        document_id: &str,
        base_version: &str,
    ...
    async fn read_then_edit(
        ...
        let body = self
            .edit(document_id, &version, &revise(&content), "peer edit")
            .await;
        ok_result(&body, "edit_document")
    }
...
/// The tool result payload, asserting the call was NOT an error.
fn ok_result(body: &serde_json::Value, tool: &str) -> serde_json::Value {
    assert!(
...
/// The error text of a tool call, asserting it WAS an error.
...
async fn test_stale_write_keeps_the_other_writer_section() {
    ...
    let body = ours
        .edit(&doc, &read_version, &ours_content, "propose main")
        .await;
    let result = ok_result(&body, "edit_document");
    assert_eq!(
...
async fn test_partial_merge_lands_what_it_can_and_reports_the_rest() {
    ...
    let body = ours
        .edit(&doc, &read_version, &ours_content, "two courses")
        .await;
    let result = ok_result(&body, "edit_document");
...
async fn test_edit_refuses_a_token_from_another_document() {
    ...
    let body = ours
        .edit(&doc_b, &version_a, "# B changed\n", "wrong token")
        .await;
    let text = err_text(&body);
    assert!(
...
async fn test_edit_refuses_a_token_whose_base_is_not_this_history() {
    ...
    let body = ours.edit(&doc, &forged, "# Changed\n", "wrong base").await;
    let text = err_text(&body);
    assert!(
