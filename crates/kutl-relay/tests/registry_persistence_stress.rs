...
const TEST_SPACE: &str = "5171e0a1-0000-4000-8000-000000000009";
...
fn test_config(data_dir: PathBuf) -> RelayConfig {
    // Authentication is mandatory. The keys file lives inside the
    // relay's data dir so it persists across the restart/crash-recovery cycles
    // these tests exercise (the `data_dir` `TempDir` owns its lifetime).
    let keys_path = data_dir.join("authorized_keys");
...
async fn connect_and_subscribe(relay: &mut Relay, conn_id: u64) {
    ...
    let (ctrl_tx, _ctrl_rx) = mpsc::channel(256);
...
/// Deterministic UUID document id for index `i`.
...
async fn test_relay_restart_preserves_registry() {
    ...
    // Phase 2: restart relay from same data dir.
    {
        ...
        for i in (DOC_COUNT - DELETE_COUNT)..DOC_COUNT {
            assert!(reg.get(&doc_id(i)).is_none(), "doc-{i} should be deleted");
        }
    }
}
...
async fn test_multiple_restart_cycles() {
    ...
    for cycle in 0..CYCLES {
        for i in 0..DOCS_PER_CYCLE {
            ...
            let entry = reg.get(&doc_id(doc_idx)).unwrap();
