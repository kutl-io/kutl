...
//!
//! No database required — device flow state lives in-memory (`AuthStore`).
...
/// Start an auth-on relay (auth is unconditional) on a random port.
...
async fn start_relay_with_auth() -> (String, tempfile::NamedTempFile) {
    ...
    // In-memory boot: the device-flow auth endpoints need no registry
    // persistence. The OSS binary's build_app requires a data dir, so
    // construct the no-backend shape via the test seam instead.
...
async fn test_device_flow_lifecycle() {
    ...
    let bearer_token = common::authenticate(&format!("http://{addr}"), &did, &signing_key).await;
...
async fn test_device_flow_token_works_for_ws() {
    ...
    let handshake = handshake_envelope_with_token(
        "36967b48-bd36-4f67-8e5e-a070df472974",
        device_token,
        "WS Test",
    );
