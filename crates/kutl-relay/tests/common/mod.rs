...
/// a subscribe. Returns the payload for tests that want to
...
/// Start an account-required relay (`require_auth = true`) with `external_url`
...
pub async fn start_account_required_relay() -> (String, tempfile::TempDir) {
    let dir = tempfile::tempdir().unwrap();
    // require_auth needs an authorizer; an OSS relay has no membership backend,
