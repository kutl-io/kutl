...
async fn test_duplicate_register_returns_error() {
    ...
    // Should receive an Error frame and a failure ack on ctrl, in that
    // order — the legacy `send_error` path runs first, then the new
    // RFD 0042 amendment 2026-05-24 ack is sent.
    let err_bytes = client_a.ctrl_rx.recv().await.expect("error frame expected");
    match decode_payload(&err_bytes) {
    ...
    let ack_bytes = client_a.ctrl_rx.recv().await.expect("failure ack expected");
    match decode_payload(&ack_bytes) {
        Some(Payload::RegisterDocumentAck(ack)) => {
            assert!(!ack.success);
            assert_eq!(ack.document_id, "doc-2");
            assert!(ack.error.is_some());
        }
        other => panic!("expected RegisterDocumentAck, got {other:?}"),
    }
}
