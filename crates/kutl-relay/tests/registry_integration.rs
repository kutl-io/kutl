...
async fn test_register_broadcasts_to_subscribers() {
    ...
    common::drain_subscribe_status(&mut client_b.ack_rx).await;
    ...
    // amendment 2026-05-24 they DO receive a `RegisterDocumentAck` — on the
    // own-ack lane (`ack_rx`), not the broadcast ctrl lane. Drain that ack and
...
async fn test_duplicate_path_conflict_copies() {
    ...
    // amendment 2026-05-24 — sender always receives an ack, on its own-ack lane).
    ...
    // RegisterDocumentAck rides the own-ack lane (the displacement broadcast for
    // the prior holder rides the separate ctrl lane), so the ack is the frame to
    // find on `ack_rx`.
    let mut ack = None;
