...
/// Drain all pending messages from a ctrl channel: the lifecycle broadcasts
/// these tests count (nothing else rides ctrl for a listener).
...
async fn test_create_convergence() {
    ...
    // Both daemons listen to the space, as real daemons do at startup.
