//! Integration tests: the subscribe cold-load path
...
const TEST_SPACE: &str = "5171e0a1-0000-4000-8000-000000000005";
...
impl RegistryBackend for StubRegistry {
    ...
    fn save_entry(
        ...
        _: &str,
        _: &RegistryEntry,
        _: &kutl_relay::registry_store::MirrorMetadata<'_>,
...
/// Connect a client. Returns `(data_rx, ack_rx)`; the `SubscribeStatus` this
/// suite asserts on is a direct response to the connection's own Subscribe, so
/// it rides the own-ack lane (`ack_rx`).
async fn connect_client(
    relay: &mut Relay,
    conn_id: u64,
) -> (mpsc::Receiver<Vec<u8>>, mpsc::UnboundedReceiver<Vec<u8>>) {
...
/// Baseline: genuinely first-time doc — backends
...
async fn test_genuinely_empty_classifies_as_empty() {
    ...
    let (_data_rx, mut ack_rx) = connect_client(&mut relay, 1).await;
    subscribe(&mut relay, 1).await;

    let status = recv_status(&mut ack_rx).await;
    assert_eq!(status.load_status, i32::from(sync::LoadStatus::Empty));
    assert_eq!(status.expected_version, 0);
    assert!(status.reason.is_empty());
}

/// Content was previously persisted (witness claims
...
/// `ContentBackend::load` returns `Err` — the prod
