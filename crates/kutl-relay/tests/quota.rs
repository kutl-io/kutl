...
/// Run the real challenge-response flow for [`test_identity`] and return a
/// bearer token the WS handshake can present.
async fn authenticate(addr: &str) -> String {
...
async fn start_relay_with_backends(
    ...
    let (app, _relay_handle, _flush_handle) = kutl_relay::testing::build_storeless_app(
        test_relay_config(),
...
impl TestClient {
    ...
    async fn handshake(&mut self) {
        ...
        let token = authenticate(&self.addr).await;
    ...
    /// catch-up `SyncOps` reply (if any). The status envelope precedes
    /// the catch-up on the WS.
