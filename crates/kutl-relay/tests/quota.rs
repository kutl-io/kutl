//! Integration tests for inbound-path storage quota enforcement.
//!
//! Covers the trust-boundary scenarios:
//!
//! 1. Blob put at cap rejected.
//! 2. Blob put uses server-measured bytes, not advertised `blob_size`.
//! 3. Delta applies inline, not spawned (bounds parallel overshoot).
//! 4. Text merge at cap rejected.
...
const TEST_SPACE: &str = "5171e0a1-0000-4000-8000-000000000004";
...
///
/// Exposes the recorded deltas via `deltas` for the inline-apply assertions.
...
/// the relay authorizes this DID and the WS client authenticates
...
/// Send a blob with a zero content hash and minimal metadata.
async fn send_blob(client: &mut TestClient, space: &str, doc: &str, bytes: Vec<u8>) {
    let meta = sync::ChangeMetadata {
        timestamp: 1,
        ..Default::default()
    };
    client
        .send_blob(space, doc, bytes, vec![0u8; 32], Some(meta))
        .await;
}

/// Send a blob with a lied `blob_size` field — the actual payload size
/// differs from the advertised value. Exercises the server-measured-size
/// check.
...
async fn test_blob_put_rejected_when_over_storage_cap() {
    ...
    send_blob(&mut client, TEST_SPACE, TEST_DOC, vec![0u8; 100]).await;
...
async fn test_blob_put_ignores_advertised_size_uses_actual_bytes() {
    // A client advertises blob_size = 0 but ships a large
...
async fn test_blob_put_applies_delta_inline_on_success() {
    // At the moment a subscriber observes the fan-out,
    ...
    let relayed = observer.recv_ops().await;
...
async fn test_text_merge_rejected_when_over_storage_cap() {
    // The text path also gates on the storage cap. Without this
    ...
    client.send_ops(TEST_SPACE, TEST_DOC, ops, meta).await;
...
async fn test_oss_relay_no_quota_backend_accepts_large_blob() {
    ...
    send_blob(
        ...
        TEST_SPACE,
        TEST_DOC,
        vec![0u8; OSS_SAFETY_NET_BLOB_LEN],
    )
    .await;
...
async fn test_patch_too_large_surfaces_as_quota_exceeded_to_client() {
    ...
    // Oversized ops buffer — exceeds MAX_PATCH_BYTES.
    let oversized_patch = vec![0u8; kutl_core::MAX_PATCH_BYTES + 1];
    client
        .send_ops(TEST_SPACE, TEST_DOC, oversized_patch, Vec::new())
        .await;
...
async fn test_op_count_cap_surfaces_as_quota_exceeded_to_client() {
    ...
    //
    // We fill via 50 000-char CHURN cycles (insert + delete of the same span
    // = 2 × FILL_BLOCK ops per merge) to keep the test fast AND the content
    // one block wide — an insert-only fill would grow a cap-sized string. A
    // final pure-insert top-up lands exactly on the cap for any cap value.
    ...
    while local.op_count() < kutl_core::MAX_OPS_PER_DOC {
        let missing = kutl_core::MAX_OPS_PER_DOC - local.op_count();
        let text = fill_text.clone();
        if local.op_count() == 0 {
            local
                .edit(agent, "filler", "seed", kutl_core::Boundary::Auto, |ctx| {
                    ctx.insert(0, &text)
                })
                .expect("edit");
        } else if missing < 2 * FILL_BLOCK {
            let top_up: String = "a".repeat(missing);
            local
                .edit(
                    agent,
                    "filler",
        ...
        client.send_ops(TEST_SPACE, TEST_DOC, ops, metadata).await;
    }
...
async fn test_text_flush_applies_storage_delta() {
    // After a successful text flush the relay must record the durable size
    // via `apply_storage_delta`. Only the blob inbound path applies a delta
    // at merge time; text merges rely on the flush-path delta to update
    // `storage_bytes_used`.
    let mock = Arc::new(MockQuotaBackend::new(0, 1_000_000_000, 5_000_000));
    ...
    // A positive-but-wrong value would satisfy the assertion above while
    // leaving the quota counter stale.
    let stored = content
