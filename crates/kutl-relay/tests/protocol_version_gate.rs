...
///
/// The token is invalid, so this also proves the version gate runs BEFORE
/// authentication: an auth-first relay would answer `AUTH_FAILED` and send the
/// user chasing credentials that were never the problem.
...
async fn test_relay_refuses_client_below_min_supported_major() {
    ...
    match reply.payload {
        Some(Payload::HandshakeAck(kutl_proto::sync::HandshakeAck {
            error: Some(e),
            min_supported_major,
            ..
        })) => {
            ...
            assert_eq!(
                ...
                "the refused ack still advertises the floor an old client reads its own verdict from"
            );
        }
        other => panic!("expected a refused HandshakeAck for the old client, got {other:?}"),
    }
}
...
async fn test_current_client_passes_the_version_gate() {
    ...
    match reply.payload {
        Some(Payload::HandshakeAck(kutl_proto::sync::HandshakeAck { error: Some(e), .. })) => {
            assert_eq!(
                e.code,
        ...
        other => panic!("expected an auth-refused HandshakeAck for the bogus token, got {other:?}"),
    }
}
