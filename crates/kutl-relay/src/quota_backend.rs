//! Storage quota backend trait.
//!
//! The relay does not enforce quotas on its own — it exposes this trait
//! for hosts to plug in a database-backed implementation. When the trait
//! object is `None`, `handle_blob_sync_ops` and the flush task skip all
//! quota checks.
