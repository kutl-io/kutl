...
use kutl_signals::did_key_verifying_key;
...
pub enum AuthError {
    ...
    InvalidUserCode,
    /// A token-validation backend (session or PAT store) was transiently
    /// unavailable. Distinct from [`TokenNotFound`](Self::TokenNotFound) so
    /// callers retry the request rather than discarding a possibly-valid
    /// token. Mapped to HTTP 503.
...
impl AuthStore {
    ...
    ///
    /// Returns `(identity, pat_context)` where `pat_context` is `Some`
    /// when the token was validated via the PAT backend. The context
    /// carries both `pat_hash` (for downstream `PatBackend::check_scope`
    /// calls) and `pat_id` (the `api_tokens.id` UUID, used as
    /// `signals.via_pat_id` for per-PAT attribution).
    pub async fn validate_token_with_backend(
    ...
    ) -> Result<(String, Option<PatAuthContext>), AuthError> {
        ...
        // Consult BOTH backends before concluding. A backend error is a
        // transient failure, not "token unknown" — but it must not short-circuit
        // the other, independent backend: a valid PAT must still authenticate
        // when the session store has a momentary blip (and vice versa). Record
        // degradation and keep going; only decide the failure mode once neither
        // backend has authenticated the token.
        ...
        if let Some(sessions) = sessions {
            match sessions.validate(&hash, now_ms).await {
                ...
                Ok(None) => {}
                Err(e) => {
                    error!(error = %e, "session backend lookup failed");
        ...
        if let Some(pats) = pats {
            match pats.validate(&hash).await {
                ...
                Ok(None) => {}
                Err(e) => {
                    error!(error = %e, "PAT backend lookup failed");
                    backend_degraded = true;
                }
            }
        }

        // No backend authenticated the token. If one was degraded we can't be
        // sure the token is truly unknown — return a retryable error so the
        // client keeps it; otherwise it is genuinely not found.
        if backend_degraded {
...
/// Auth-context fields tied to a validated PAT, propagated through
/// session storage and onward to signal-write attribution.
#[derive(Debug, Clone)]
pub struct PatAuthContext {
    /// `api_tokens.token_hash` of the validated PAT. Used for
    /// subsequent `PatBackend::check_scope` calls.
    pub pat_hash: String,
    /// `api_tokens.id` UUID of the validated PAT. Stored on
    /// `signals.via_pat_id` for per-PAT attribution.
    pub pat_id: String,
}
...
///
/// Delegates to [`kutl_signals::did_key_verifying_key`], which enforces a
/// 64-character base58 payload cap (bounding the O(n·m) bs58 decode against
/// megabyte-DID CPU exhaustion on the auth path).
pub fn parse_did_key(did: &str) -> Result<VerifyingKey, AuthError> {
    did_key_verifying_key(did).map_err(|e| AuthError::InvalidDid(e.to_string()))
}
...
mod tests {
    ...
    /// Session backend whose `validate` always fails with a transient error.
    struct FailingSessionBackend;

    #[async_trait::async_trait]
    impl crate::session_backend::SessionBackend for FailingSessionBackend {
        async fn create(&self, _hash: &str, _did: &str, _expires_at_ms: i64) -> anyhow::Result<()> {
            Ok(())
        }

        async fn validate(&self, _hash: &str, _now_ms: i64) -> anyhow::Result<Option<String>> {
            Err(anyhow::anyhow!("simulated database connection failure"))
        }

        async fn purge_expired(&self, _now_ms: i64) -> anyhow::Result<u64> {
            Ok(0)
        }
    }

    #[tokio::test]
    async fn test_validate_token_backend_failure_is_retryable() {
        // An unknown token combined with a *failing* session backend must
        // surface as the retryable BackendUnavailable variant — never as
        // TokenNotFound, which would make clients discard a valid token.
        let store = AuthStore::new();
        let backend = FailingSessionBackend;
        let result = store
            .validate_token_with_backend("kutl_unknown", 1000, Some(&backend), None)
            .await;
        assert!(matches!(result, Err(AuthError::BackendUnavailable)));
    }

    /// PAT backend that authenticates any token to a fixed identity.
    struct StubPatBackend;

    #[async_trait::async_trait]
    impl crate::pat_backend::PatBackend for StubPatBackend {
        async fn validate(
            &self,
            _token_hash: &str,
        ) -> anyhow::Result<Option<crate::pat_backend::PatIdentity>> {
            Ok(Some(crate::pat_backend::PatIdentity {
                did: "did:key:pat-user".to_owned(),
                pat_id: "pat-1".to_owned(),
            }))
        }
    }

    #[tokio::test]
    async fn test_valid_pat_authenticates_despite_failing_session_backend() {
        // A degraded session backend must NOT reject a valid PAT: the two
        // backends are independent, so a session-store blip should fall
        // through to the (healthy) PAT backend rather than returning 503.
        let store = AuthStore::new();
        let result = store
            .validate_token_with_backend(
                "kutl_valid_pat",
                1000,
                Some(&FailingSessionBackend),
                Some(&StubPatBackend),
            )
            .await;
        match result {
            Ok((did, pat)) => {
                assert_eq!(did, "did:key:pat-user");
                assert!(pat.is_some(), "expected PAT auth context");
            }
            other => panic!("expected PAT auth despite session failure, got {other:?}"),
        }
    }

    /// An oversized DID payload is rejected on the auth path — proving the
    /// 64-char base58 cap (from `kutl-signals`) applies here, bounding the
    /// O(n·m) bs58 decode against CPU exhaustion.
    #[test]
    fn test_parse_did_key_rejects_oversized_payload() {
        // A payload of 300 base58 characters is well above the 64-char cap.
        let oversized = format!("did:key:z{}", "1".repeat(300));
        let result = parse_did_key(&oversized);
        assert!(
            matches!(result, Err(AuthError::InvalidDid(_))),
            "oversized DID must be rejected on the auth path"
        );
    }
}
