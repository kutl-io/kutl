...
async fn dispatch_tool(
    ...
    match parsed {
        ...
        mcp_tools::ParsedToolCall::CreateChat {
            space_id,
            topic,
            target_did,
        } => dispatch_create_chat(state, session_id, space_id, topic, target_did).await,
    }
}
...
async fn dispatch_create_chat(
    ...
    let result = dispatch_relay_command(state, |tx| RelayCommand::McpCreateChat {
        session_id: session_id.to_owned(),
        space_id,
        topic,
        target_did,
    ...
    .await;
    match result {
        Ok(Ok(id)) => mcp::ToolCallResult::text(format!("created chat signal {id}")),
        Ok(Err(e)) => mcp::ToolCallResult::error(e.to_string()),
        Err(e) => mcp::ToolCallResult::error(e),
    }
}
