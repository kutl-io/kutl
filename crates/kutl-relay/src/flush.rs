...
/// witness write. Deployments without witness support pass `None`, which
/// keeps witness updates as a no-op on the trait's default impl.
///
...
pub async fn run_flush_task(
    ...
    loop {
        ...
        while let Some(entry) = entry_rx.recv().await {
            ...
            match result {
                Ok(()) => {
                    // Record the authoritative post-save size. The
                    ...
                    // Record the durable persistence witness
                    ...
                    // intentional: a send failure means the relay is shutting
                    // down. The document is already durably persisted (and the
                    // witness written), so the slot will be re-derived as clean
                    // on the next boot; nothing is lost by not confirming.
                    let _ = relay_tx
        ...
        if flushed > 0 || failed > 0 {
            debug!(flushed, failed, "flush cycle complete");
        }
    }
}
...
///
/// `quota_backend` is forwarded to the flush loop for post-write
/// reconciliation. Pass `None` when quota enforcement is disabled.
///
/// `registry_backend`, when `Some`, forwards the handle the flush task
/// uses for the durability-witness write. Deployments without witness
/// support pass `None`; DB-backed deployments pass the same registry
/// backend already threaded into the app.
