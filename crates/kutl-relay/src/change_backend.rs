...
pub struct SignalDetail {
    ...
    /// Original wrapped text for comment-kind flags (RFD 0077). Display-only
...
pub trait ChangeBackend: Send + Sync {
    ...
    /// Record a new signal (flag or reply).
    ///
    ...
    ///
    /// `anchor_text` carries the comment-kind posterity snapshot (RFD 0077).
    /// `Some` for comment flags with an inline marker; `None` otherwise.
    /// Backends without a dedicated column (the OSS sqlite path) persist it
    /// alongside the signal so `get_signal_detail` can surface it later.
    ///
    /// `via_pat_id` is the `api_tokens.id` UUID of the PAT used to author
    /// the signal, when PAT-authenticated. `None` for DID-auth (challenge
    /// response or session). Stored on `signals.via_pat_id` for per-PAT
    /// attribution per RFD 0044. The OSS sqlite backend ignores this field
    /// — it has no PATs.
    ///
    /// RFD 0084 Phase 1: the commercial PG backend resolves the
