...
/// to the remedy fitting their traffic kind; only `Sent` advances any
/// cursor or subscriber state.
...
pub struct Relay {
    ...
    /// Persistence backend for registries. `None` only on the storeless host
    /// relay (kutlhub) and standalone test/sim actors; the OSS binary always
    /// has one.
    ...
    /// Connections subscribed to each space's SIGNAL stream, with the cursor
    /// each one has been served up to. Populated by
    /// `SubscribeSignals`, mirroring `space_watchers`.
    ///
    /// **This is the recipient set for space-wide signals.** Document
    /// subscribers are the wrong one: a client subscribed to zero documents
    /// would receive nothing — a signals-only client (e.g. `kutl watch`) must
    /// not need a sentinel document subscription to appear in the set.
    ///
    /// Cursors are in-memory by design. A relay restart re-pages from the
    /// client's own mark, which is safe because paging is idempotent — the
    /// fold absorbs a re-delivered record.
    signal_subscribers: HashMap<String, HashMap<ConnId, Option<sync::Hlc>>>,
    ...
    /// `None` on the storeless host relay (kutlhub — no data dir) and standalone
    /// test actors, or when identity load fails at startup — records degrade to
    /// tier-3 asserted in that case. The OSS binary always loads one.
    ...
    /// than a concrete segment store. `None` on a storeless relay and on
    /// standalone test actors, in which case appends are no-ops.
...
impl Relay {
    ...
    pub async fn run(mut self) {
        ...
        if self.record_log.is_configured() {
            ...
            debug!("no record log; signal record appends are no-ops (storeless relay)");
        }
    ...
    async fn handle_disconnect(&mut self, conn_id: ConnId) {
        ...
        // Same reclaim for the signal subscriber set, and for the same reason:
        // one residual key per space ever subscribed would never be reclaimed.
    ...
    /// storeless relay (no `signal_store`) — seeding an empty baseline is
    /// exactly what a fresh in-memory known-set already is.
    ///
    ...
    async fn invoke_after_merge(&mut self, key: &DocKey, event: MergedEvent) {
        self.ensure_doc_seeded(&key.space_id, &key.document_id)
            .await;
        if let Some(DocContent::Text(doc)) = self.documents.get(key).map(|slot| &slot.content) {
            ...
            // stream-emitting enricher each log nothing here. Matches the field
            // set the former `TracingObserver::after_text_merge` logged.
            info!(
            ...
            self.after_merge.after_text_merge(event, doc).await;
        }
    }
    ...
    /// a `SignalPage` burst that fills the lane makes the next document op's
    /// `try_send` fail — which is why paging discipline is load-bearing
    /// rather than an afterthought.
    ///
    /// **The recovery.** A full lane must not tear
    /// down the WHOLE connection: a subscriber slow to drain one signal
    /// broadcast would lose its document sync as collateral. It is evicted from
    /// the signal stream alone and told so, which is the recoverable half of
    /// "evict and notify".
    ///
    ...
    /// overflow it and re-strand the very subscribers the notices exist
    /// to save. `send_stale_signal_stream` already sends on `ack`, which is why
    /// this can simply call it.
    ///
    /// The subscriber is REMOVED, not merely notified — see
    /// [`Self::evict_signal_subscriber`] for why retaining the entry
    ...
    ///
    /// This is the ONE door for signal frames onto the lane — live
    /// broadcasts and catch-up pages both come through it, so the
    /// document-headroom reserve binds every signal writer, not just the
    /// one it was first written into. Signals stop short of the last slots
    ...
    /// the remedy per traffic kind (evict the subscriber for a broadcast,
    /// a stale-stream notice for a page).
    ...
    /// Drop a connection from a space's signal subscriber set and tell it why.
    ///
    /// **Removing the entry is what stops a notice storm.** Retaining it (on
    /// the reasoning that the cursor should survive a `StaleSignalStream`
    /// eviction so a re-subscribe resumes) does not survive reading the code:
    /// the stored
    /// `Option<Hlc>` is written by `handle_subscribe_signals` and by the pager,
    /// and read by NOTHING. Recipients iterate `subs.keys()`, and a re-subscribe
    /// carries the client's own cursor on the frame. It is dead state, so
    /// nothing is lost by removing it — and a retained entry means every
    /// subsequent broadcast re-notifies a subscriber that is already evicted,
    /// forever, for anyone who never re-subscribes: a notice storm arriving by
    /// a different road.
    ///
    /// If relay-side resume is ever actually wanted (a client re-subscribing
    /// with no cursor of its own), the stored value has to start being READ
    /// first; retaining it while nothing consults it only looks like the
    /// feature.
    ///
    /// The notice rides the ack lane, never the lane that just overflowed.
    fn evict_signal_subscriber(&mut self, space_id: &str, conn_id: ConnId, reason: &str) {
        debug!(conn_id, space_id, reason, "evicting signal subscriber");
        if let Some(subs) = self.signal_subscribers.get_mut(space_id) {
            subs.remove(&conn_id);
        }
        self.send_stale_signal_stream(conn_id, space_id, reason);
    }
...
mod tests {
    ...
    async fn test_disconnect_drops_empty_space_watcher_set() {
        ...
        relay
            .process_command(RelayCommand::ListSpaceDocuments {
                conn_id,
                msg: sync::ListSpaceDocuments {
                    space_id: "5171e0a1-1111-4000-8000-000000000003".into(),
                },
            })
            .await;
        assert!(
            relay
                .space_watchers
                .contains_key("5171e0a1-1111-4000-8000-000000000003"),
    ...
    async fn test_disconnect_keeps_nonempty_space_watcher_set() {
        ...
        assert_eq!(
            relay
                .space_watchers
                .get("5171e0a1-1111-4000-8000-000000000003")
    ...
    async fn test_shared_clock_orders_cross_path_records_monotonically() {
        ...
        .expect("edit");
        materializer.after_text_merge(ev, &doc).await;
