...
use crate::protocol::{
    ...
    unregister_space_ack_envelope, wrap_error, wrap_handshake_ack, wrap_quota_exceeded,
};
...
#[expect(
    ...
    reason = "RegisterDocument carries 5 RFD 0081 provenance fields plus 5 RFD \
              0042 amendment mirror fields; boxing every actor command for one \
              outlier costs an allocation on every command — the actor channel \
              is tx/rx-bounded, not memory-bounded"
)]
...
enum LoadResult {
    ...
    /// case — the variant grew to ~240 bytes when
    /// `DocSlot::edited_at_pending` started tracking the editor's
    /// DID alongside the timestamp (RFD 0042 amendment 2026-05-24
    /// B-full follow-up).
    Found(Box<DocSlot>),
...
struct DocSlot {
    ...
    /// the write.
    ///
    /// RFD 0042 amendment 2026-05-24 (B-full follow-up): the DID is
    /// carried alongside the timestamp so the kutlhub-relay's
    /// `documents_table.updated_by` column can be patched in the same
    /// flush — replacing the UX server's `docEdited` coalescing
    /// writer that previously wrote `documents_table` directly. Last
    /// writer wins within a debounce window, matching the prior
    /// volatile-writer semantics.
    edited_at_pending: Option<EditedAtPending>,
}

/// Per-slot pending `edited_at` flush state. RFD 0042 amendment
/// 2026-05-24 (B-full follow-up): bundles the timestamp and author
/// DID so the mirror's `updated_by` column can be patched alongside
/// `updated_at` in a single `RegistryBackend::update_edited_at`
/// call.
#[derive(Clone, Debug)]
struct EditedAtPending {
    timestamp: i64,
    author_did: String,
}
...
impl Relay {
    ...
    async fn load_from_backends(&self, space_id: &str, doc_id: &str) -> LoadResult {
        ...
        // (ephemeral mode), the default impl returns the zero witness —
        // which classifies any `Ok(None)` as `Empty`, matching the
        // legacy behavior for self-hosted deployments.
    ...
    #[expect(
        ...
        reason = "linear blob-upload pipeline: pre-check, quota check, LWW guard, in-memory \
                  ...
                  storage delta + observer notification + flush. RFD 0042 amendment \
                  2026-05-24 B-full added the author-DID capture to `edited_at_pending` \
                  which pushed the function one line over the 100-line guideline"
    )]
    ...
    async fn handle_mcp_create_document(
        &mut self,
        session_id: &str,
        space_id: &str,
        path: &str,
    ...
    #[allow(clippy::too_many_arguments)]
    async fn handle_mcp_upload_blob(
        &mut self,
        session_id: &str,
        space_id: &str,
        path: &str,
    ...
    async fn handle_mcp_list_spaces(
        &mut self,
        session_id: &str,
    ...
    #[expect(
        ...
        reason = "source provenance is already bundled in `rfd0081` and UX-only \
    ...
    fn register_document_internal(
        ...
        rfd0081: registry::RegisterRfd0081,
        // RFD 0042 amendment 2026-05-24 (+ follow-up 2026-05-24):
        ...
        // RFD 0042 amendment 2026-05-24 (B-full follow-up): idempotent
        // register. If the document_id already exists at the same
        // path, treat the call as a mirror-field patch (the relay's
        // `save_entry` ON CONFLICT clause COALESCEs `title` /
        // `content_type` / convert provenance / `size_bytes`).
        // Re-registers from the UX-server-side migration paths
        // (content-type sniffer, convert orphan re-emit, ingestion
        // re-pickup) flow through this branch so the mirror gets
        // patched without a separate "patch metadata" envelope.
        //
        // The in-memory `RegistryEntry` is NOT mutated by a re-
        // register — immutable fields stay (created_by / created_at /
        // RFD 0081 provenance), and there's no path change. The
        // `save_entry` call below is the entire write — the mirror
        // INSERT-or-UPDATE absorbs the new fields. The broadcast
        // still fires so any subscribed UX-server-side observer
        // (none today after Phase-4 cleanup; future-proof) sees
        // the patch.
        let is_reregister = if let Some(existing) = reg.get_any(document_id) {
        ...
        // RFD 0042 amendment 2026-05-24: propagate persistence errors
        // and roll back the in-memory insert on failure so the registry
        // never diverges from the mirror. On a re-register (is_reregister
        // = true), there's no in-memory mutation to roll back — the
        // entry was already present and unchanged.
        if let Err(e) = persist_entry(
            self.registry_backend.as_deref(),
            &self.registries,
            space_id,
            ...
            if !is_reregister && let Some(reg) = self.registries.get_mut(space_id) {
                reg.remove_for_rollback(document_id);
            }
        ...
        let proto_msg = sync::RegisterDocument {
            ...
            ingestion_job_id,
            source_author_display,
            // RFD 0042 amendment 2026-05-24: forward UX-only metadata
            ...
            // RFD 0042 amendment 2026-05-24 follow-up: convert-path
            // metadata forwarded on the broadcast too, so the UX
    ...
    async fn handle_register_document(&mut self, conn_id: ConnId, msg: &sync::RegisterDocument) {
        let _authorized = match self.authorize_conn(conn_id, &msg.space_id).await {
            ...
            Err(e) => {
                ...
                // RFD 0042 amendment senders can correlate the result.
                self.send_error(conn_id, ErrorCode::AuthFailed, &e.to_string());
        ...
        let result = self.register_document_internal(
            &msg.space_id,
            &msg.document_id,
            &msg.path,
            &meta.author_did,
        ...
        // RFD 0042 amendment 2026-05-24: emit ack AFTER persistence
        ...
        let ack = match result {
            Ok(()) => {
                ...
                let entry = self
                    .registries
                    .get(&msg.space_id)
                    .and_then(|reg| reg.get(&msg.document_id))
                    .map(registry_entry_to_proto);
                register_document_ack_envelope(&msg.space_id, &msg.document_id, true, entry, None)
            }
            Err(e) => {
    ...
    #[expect(
        clippy::too_many_lines,
        reason = "linear three-phase handler (auth, in-memory rename + capture-prior, persist with \
                  on-failure rollback + failure ack, broadcast + observer + success ack); \
                  the auth-fail and persist-fail arms each need their own typed-ack envelope \
                  with the matching error message, which RFD 0042 amendment 2026-05-24 made \
                  load-bearing for the UX server's caller — extracting a generic helper would \
                  hide the ack contract the prose calls out"
    )]
    async fn handle_rename_document(&mut self, conn_id: ConnId, msg: &sync::RenameDocument) {
        ...
        // Capture prior state for potential rollback (RFD 0042 amendment
        // 2026-05-24: if backend persistence fails, the in-memory rename
        // must be undone so the registry doesn't diverge from the mirror).
        let prior_rename_state = reg
            .get_any(&msg.document_id)
            .map(|e| (e.path.clone(), e.renamed_by.clone(), e.renamed_at));
        ...
        // Rename doesn't carry UX-only mirror metadata — those land
        // via the original register. COALESCE-on-conflict in the
        // mirror keeps the previously-stored values.
        if let Err(e) = persist_entry(
            self.registry_backend.as_deref(),
            &self.registries,
            &msg.space_id,
            ...
            if let Some((prior_path, prior_renamed_by, prior_renamed_at)) = prior_rename_state
                && let Some(reg) = self.registries.get_mut(&msg.space_id)
            {
                reg.restore_after_rename_rollback(
                    &msg.document_id,
                    &msg.new_path,
                    &prior_path,
                    prior_renamed_by,
        ...
        // RFD 0042 amendment 2026-05-24: emit ack AFTER persistence
        // commits across both the registry and the `documents` mirror.
        let ack = rename_document_ack_envelope(&msg.space_id, &msg.document_id, true, None);
        self.send_ctrl(conn_id, &encode_envelope(&ack));
    }

    #[expect(
        clippy::too_many_lines,
        reason = "same three-phase shape as handle_rename_document — RFD 0042 amendment \
                  2026-05-24 adds the persist-fail rollback + failure ack; the linear flow \
                  reads better than a helper that hides the per-handler ack envelope shape"
    )]
    async fn handle_unregister_document(
        ...
        // mirror (RFD 0042 amendment 2026-05-24).
        ...
        if let Err(e) = persist_result {
            ...
            if let Some(reg) = self.registries.get_mut(&msg.space_id) {
                reg.restore_after_unregister_rollback(&msg.document_id);
            }
            self.send_error(
                conn_id,
                ErrorCode::InvalidMessage,
    ...
    /// Bulk unregister every active document in a space (RFD 0042
    /// amendment 2026-05-24 — B-full follow-up). Routes per-doc
    ...
    async fn handle_unregister_space(&mut self, conn_id: ConnId, msg: &sync::UnregisterSpace) {
        let _authorized = match self.authorize_conn(conn_id, &msg.space_id).await {
            Ok(a) => a,
            Err(e) => {
                self.send_error(conn_id, ErrorCode::AuthFailed, &e.to_string());
        ...
        let mut soft_deleted: Vec<String> = Vec::with_capacity(active_ids.len());
        let timestamp = kutl_core::env::now_ms();

        for document_id in &active_ids {
            // Unregister in memory.
            let reg = self.registries.entry(msg.space_id.clone()).or_default();
            if let Err(e) = reg.unregister(document_id, &meta) {
                warn!(
                    error = %e,
                    doc_id = %document_id,
                    "bulk unregister: in-memory unregister failed (skipping)"
                );
                continue;
            }

            // Persist soft-delete. Roll back on backend failure so
            // the registry never diverges from the mirror.
            let persist_result = match self.registry_backend.as_deref() {
                Some(backend) => {
                    let deleted_at = self
                        .registries
                        .get(&msg.space_id)
                        .and_then(|r| r.get_any(document_id))
                        .and_then(|e| e.deleted_at);
                    match deleted_at {
                        Some(ts) => backend.delete_entry(&msg.space_id, document_id, ts),
                        None => Ok(()),
                    }
                }
                None => Ok(()),
            ...
            if let Err(e) = persist_result {
                warn!(
                    error = %e,
                    doc_id = %document_id,
                    "bulk unregister: persist failed — rolling back and aborting"
                );
                if let Some(reg) = self.registries.get_mut(&msg.space_id) {
                    reg.restore_after_unregister_rollback(document_id);
                }
                let ack = unregister_space_ack_envelope(
                    &msg.space_id,
                    soft_deleted,
                    ...
                    Some(lifecycle_ack_error(
                        ErrorCode::InvalidMessage,
                        format!("bulk unregister failed at {document_id}: {e}"),
                    )),
                );
                self.send_ctrl(conn_id, &encode_envelope(&ack));
                return;
            ...
            // Broadcast per-doc lifecycle so feed observers see each
            // delete attributed correctly.
            ...
            soft_deleted.push(document_id.clone());
        }

        let ack = unregister_space_ack_envelope(&msg.space_id, soft_deleted, true, None);
        self.send_ctrl(conn_id, &encode_envelope(&ack));
    }

    /// Transfer ownership of every document in a space (RFD 0042
    /// amendment 2026-05-24 — B-full follow-up). Used by the
    ...
    async fn handle_transfer_space_ownership(
        &mut self,
        conn_id: ConnId,
        msg: &sync::TransferSpaceOwnership,
    ) {
...
fn compute_blob_hash(bytes: &[u8]) -> [u8; 32] {
...
///
/// `mirror` carries RFD 0042 amendment 2026-05-24 mirror-only metadata
...
///   `save_entry` failed. RFD 0042 amendment 2026-05-24: lifecycle
...
///
/// RFD 0042 amendment 2026-05-24: used by `RegisterDocumentAck` to carry
...
mod tests {
    ...
    async fn test_blob_lww_ignores_client_timestamp_poison() {
        ...
        let conn_id = 1;
        let (_data_rx, _ctrl_rx) = connect_client(&mut relay, conn_id).await;
        subscribe_doc(&mut relay, conn_id, "space", "blob").await;
        ...
        .await;
        assert_eq!(
            resident_blob(&relay, "space", "blob").as_deref(),
    ...
    async fn test_disconnect_keeps_nonempty_space_watcher_set() {
        ...
        let mut relay = Relay::new_standalone(test_config());
        let (_d1, _c1) = connect_client(&mut relay, 1).await;
        let (_d2, _c2) = connect_client(&mut relay, 2).await;
