...
impl Relay {
    ...
    #[expect(
        clippy::too_many_lines,
        reason = "convergence-sensitive subscribe path; keep the flow in one place rather than split it"
    )]
    ...
    async fn handle_text_sync_ops(&mut self, conn_id: ConnId, msg: &sync::SyncOps, key: &DocKey) {
        ...
        // Edit-revives: raise the document's lifecycle TOUCH
        // to the edit's ORIGIN time (the max across the ops' metadata), so an
        // edit concurrent with a delete keeps the document alive. Content ops
        // carry only a physical timestamp, so the touch is physical-ms dominant
        // (see `Hlc::physical_touch`): a stale offline edit carries its old time
        // and cannot revive past a causally-later delete, while a concurrent edit
        // beats it.
        if let Some(edit_hlc) = max_origin_hlc(&msg.metadata)
            && let Some(reg) = self.registries.get_mut(&msg.space_id)
        {
            reg.touch(&msg.document_id, edit_hlc);
        }
    ...
    fn register_document_internal(
        ...
        // RFD 0042 amendment 2026-05-24: propagate persistence errors and roll
        ...
        self.broadcast_displaced(space_id, &displaced);
    ...
    ///
    /// QUICK FIX (2026-06-07): only the WS register path is covered — see the
    /// auto-register design doc.
    ...
    async fn handle_rename_document(&mut self, conn_id: ConnId, msg: &sync::RenameDocument) {
        ...
        // returned token rolls back on a persist failure (RFD 0042 amendment
        // 2026-05-24; capture-restore since the merge is not invertible).
        let (token, displaced) = reg.rename(&msg.document_id, &msg.old_path, &msg.new_path, meta);
        ...
        // Drive every replica to the LWW winner, not the sender's requested path.
        forwarded.new_path.clone_from(&authoritative_path);
        if let Some(ref did) = authoritative {
        ...
        self.broadcast_displaced(&msg.space_id, &displaced);
        ...
        // RFD 0042 amendment 2026-05-24: emit ack AFTER persistence commits
        ...
        // it (replaces the former `correct_operated_loser` broadcast — the typed
    ...
    async fn handle_unregister_document(
        ...
        // mirror (RFD 0042 amendment 2026-05-24; capture-restore).
    ...
    async fn handle_unregister_space(&mut self, conn_id: ConnId, msg: &sync::UnregisterSpace) {
        ...
        // invisible to peers (stronger than the old per-doc loop, which could leak
    ...
    fn broadcast_displaced(&self, space_id: &str, displaced: &[String]) {
...
///
/// RFD 0042 amendment 2026-05-24 (B-full follow-up): idempotent register — if the
...
mod tests {
    ...
    impl RegistryBackend for CountingBackend {
        fn load_all(&self, _: &str) -> Result<Vec<registry::RegistryEntry>, RegistryStoreError> {
            Ok(vec![])
        }
    ...
    /// otherwise a mid-batch failure could leave a committed prefix (#12).
    #[test]
    ...
    // ---- path-arbitration conflict-copy broadcasts (catalog §2) ----
    ...
    async fn test_register_collision_displaces_prior_holder_and_broadcasts() {
        ...
        let (_a_data, mut a_ctrl) = connect_client(&mut relay, 1).await;
        let (_b_data, mut b_ctrl) = connect_client(&mut relay, 2).await;
    ...
    async fn test_bulk_unregister_uses_one_delete_batch() {
        ...
        let space = "space";
        let (_data, mut ctrl) = connect_client(&mut relay, 1).await;
    ...
    /// on `ctrl_rx` (drains the receiver), if one carries a persisted entry.
    ...
    async fn test_register_loser_ack_carries_conflict_path() {
        ...
        let (_w_data, mut w_ctrl) = connect_client(&mut relay, 1).await;
        let (_l_data, mut l_ctrl) = connect_client(&mut relay, 2).await;
        ...
        let loser_path = drained_register_ack_path(&mut l_ctrl, loser)
        ...
        assert_eq!(
            drained_register_ack_path(&mut w_ctrl, winner).as_deref(),
            Some(path),
    ...
    async fn test_reregister_displaced_doc_at_intended_path_succeeds() {
        ...
        let loser = "22222222-2222-2222-2222-222222222222";
        let path = "target.md";
        let (_w, mut w_ctrl) = connect_client(&mut relay, 1).await;
        let (_l, mut l_ctrl) = connect_client(&mut relay, 2).await;
        ...
        // Re-register the displaced loser at its intended path — idempotent.
        register_via_command(&mut relay, 2, space, loser, path, 1000).await;
        let mut success = None;
        while let Ok(bytes) = l_ctrl.try_recv() {
            if let Ok(env) = kutl_proto::protocol::decode_envelope(&bytes)
    ...
    async fn test_register_rejects_reserved_conflict_path() {
        ...
        let (_data, _ctrl) = connect_client(&mut relay, 1).await;
