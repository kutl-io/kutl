...
/// require the UX layer (chat, reactions, reopen).
...
fn create_flag_tool() -> ToolDefinition {
    ToolDefinition {
        ...
        description: "Notify other participants about something they should look at. \
                      This is the ONLY way others will see your activity via \
                      get_changes — document edits alone are not visible to polling \
                      participants. Use after editing a document, when you need \
                      input, or when you've completed a task others are waiting on. \
                      Kinds: question, review_requested, blocked, info, completed. \
...
/// Reply to a signal (flag, chat, decision, or another reply).
fn create_reply_tool() -> ToolDefinition {
    ToolDefinition {
        name: "create_reply".into(),
        description: "Add a reply to a flag, chat, decision, or another reply. Replies \
                      form threaded conversations on signals. Set parent_reply_id to \
                      reference a specific prior reply in the thread."
            .into(),
        input_schema: serde_json::json!({
            "type": "object",
            "properties": {
...
fn close_flag_tool() -> ToolDefinition {
    ToolDefinition {
        ...
        description: "Close a flag with a resolution. Reasons: 'resolved' (addressed), \
                      'declined' (won't do), 'withdrawn' (no longer relevant). Closed \
                      flags no longer appear in open signal lists."
            .into(),
        input_schema: serde_json::json!({
            "type": "object",
            "properties": {
...
pub enum ParsedToolCall {
    ...
    /// Create a chat signal (kutlhub-only).
    CreateChat {
        space_id: String,
        topic: Option<String>,
...
pub fn parse_tool_call(name: &str, arguments: &Value) -> Result<ParsedToolCall, ToolCallResult> {
    match name {
        ...
        "create_chat" => parse_create_chat(arguments),
...
fn parse_create_flag(args: &Value) -> Result<ParsedToolCall, ToolCallResult> {
    ...
    let document_id = require_string(args, "document_id")?;
...
fn parse_create_reply(args: &Value) -> Result<ParsedToolCall, ToolCallResult> {
    let space_id = require_string(args, "space_id")?;
    let parent_signal_id = require_string(args, "parent_signal_id")?;
    let parent_reply_id = optional_string(args, "parent_reply_id")?;
...
fn parse_close_flag(args: &Value) -> Result<ParsedToolCall, ToolCallResult> {
    let space_id = require_string(args, "space_id")?;
    let signal_id = require_string(args, "signal_id")?;
...
/// Parse `create_chat` arguments (kutlhub-only).
fn parse_create_chat(args: &Value) -> Result<ParsedToolCall, ToolCallResult> {
    let space_id = require_string(args, "space_id")?;
    let topic = optional_string(args, "topic")?;
    let target_did = optional_string(args, "target_did")?;
    Ok(ParsedToolCall::CreateChat {
        space_id,
        topic,
        target_did,
    })
...
mod tests {
    ...
    fn test_parse_tool_call_create_reply_basic() {
        ...
        match parsed {
            ParsedToolCall::CreateReply {
                ...
                assert_eq!(parent_signal_id, "sig-1");
    ...
    #[test]
    fn test_parse_tool_call_create_chat() {
        let args = serde_json::json!({
            "space_id": "s1",
            "topic": "design review"
        });
        let parsed = parse_tool_call("create_chat", &args).unwrap();
        match parsed {
            ParsedToolCall::CreateChat {
                space_id,
                topic,
                target_did,
            } => {
                assert_eq!(space_id, "s1");
                assert_eq!(topic.as_deref(), Some("design review"));
                assert!(target_did.is_none());
            }
            _ => panic!("expected CreateChat"),
        }
    }
