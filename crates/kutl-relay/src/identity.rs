...
impl RelayIdentity {
    ...
    ///   NOT degrade to an identity-less (tier-3) relay silently.
    ///
    ...
    fn save(&self, data_dir: &Path) -> Result<()> {
        ...
        // The durable spelling: a torn first-start write would be read as a
        // valid identity by the next boot and silently flip the DID.
        kutl_core::fs::write_atomic_secret_durable(&path, &json)
