...
use kutl_signals::segment::{QuarantinedSegment, SegmentStore, SegmentWriter};
use tracing::warn;
use uuid::Uuid;
...
impl SignalStore {
    ...
    /// which `scan_valid_len`-recovers the segment. Without the eviction one
    ...
    /// no writer needed; used by projection rebuild and catch-up.
    ///
    /// Prefer [`Self::load_and_warn`] at relay load sites: it additionally
    /// surfaces any quarantined (`.corrupt`) segments in a log line so a
    /// silently-dropped corrupt segment does not under-serve catch-up /
    ...
    /// [`Self::load`] plus a `warn!` for every quarantined segment.
    ///
    /// `SegmentStore::load` renames CRC-failed sealed segments (and corrupt
    /// active segments) to `<name>.corrupt` and drops them from the served
    /// records — silently under-serving catch-up / projection while reporting
    /// success. This wrapper serves the healthy remainder (never refuses) but
    /// makes the loss LOUD: one warn per quarantined segment, naming the space
    /// and the file. It is the single funnel every relay load site uses so the
    /// warn is not duplicated at each call site (DRY).
    pub fn load_and_warn(&self, space: Uuid) -> Result<SegmentStore> {
        let loaded = self.load(space)?;
        warn_if_quarantined(space, &loaded.quarantined);
        Ok(loaded)
    }
}

/// Emit one `warn!` per quarantined segment, naming the space and the file.
///
/// Shared by [`SignalStore::load_and_warn`] and the startup backfill path,
/// which loads via [`SignalStore::load`] deep inside a resumable per-row loop
/// and surfaces quarantine at the space boundary. A no-op when the list is
/// empty (the common case).
pub fn warn_if_quarantined(space: Uuid, quarantined: &[QuarantinedSegment]) {
    for q in quarantined {
        warn!(
            space_id = %space,
            segment = %q.path.display(),
            reason = %q.reason,
            "quarantined a corrupt signal segment; serving the healthy remainder \
             (catch-up/projection under-serves until the segment is re-seeded)"
        );
    }
}
...
mod tests {
    ...
    fn test_append_evicts_poisoned_writer_and_self_heals() {
        ...
        // writer (scan_valid_len recovers the segment) and succeeds.
    ...
    /// quarantined by the loader; `load_and_warn` serves the healthy remainder
    /// and the returned store's `quarantined` list is non-empty (the
    /// observable surfacing the warn path is driven from). We assert on the
    /// observable quarantine surfacing rather than capturing tracing output,
    /// which the repo has no harness for here.
    #[test]
    fn test_load_and_warn_serves_remainder_and_surfaces_quarantine() {
        use kutl_signals::segment::SegmentWriter;
        ...
        let loaded = store.load_and_warn(space).unwrap();
        assert!(
