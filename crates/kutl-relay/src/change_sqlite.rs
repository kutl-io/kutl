...
mod tests {
    ...
    async fn clear(
        ...
        let space_uuid = uuid::Uuid::parse_str(space).unwrap();
        let present = store.load(space_uuid).unwrap().records;
        backend
    ...
    /// Startup reconcile never clears projection rows behind a quarantined
    /// segment: with one sealed segment set aside the fold is incomplete, so
    /// the rows its records project are not phantoms. The rebuild still runs.
    /// That holds on every start, not only the one that quarantined: the
    /// `.corrupt` file the first start leaves behind keeps the fold
    /// incomplete, so a second start over the same directory refuses the
    /// clear again.
    #[tokio::test]
    async fn test_startup_reconcile_skips_the_clear_behind_a_quarantined_segment() {
        use kutl_signals::segment::SegmentWriter;
        use kutl_signals::testing::corrupt_sealed_segment;

        const RECORDS: usize = 40;
        /// Small enough that forty records seal several segments.
        const ROTATE_AT_BYTES: u64 = 256;

        let pool = test_pool().await;
        let backend = SqliteChangeBackend::new(pool.clone()).await.unwrap();
        let space = "55555555-5555-4555-8555-555555555555";
        let space_uuid = uuid::Uuid::parse_str(space).unwrap();
        let data_dir = TempDir::new().unwrap();
        let space_dir = SignalStore::root_for(data_dir.path()).join(space);
        {
            let mut writer = SegmentWriter::open(&space_dir, space_uuid, 0).unwrap();
            writer.rotate_threshold_for_test(ROTATE_AT_BYTES);
            for n in 0..RECORDS {
                let record = test_row(
                    &format!("sig-{n}"),
                    space,
                ...
                backend
                    .project_record_for_test(&record, None)
                    .await
                    .unwrap();
            }
        }
        let expected = i64::try_from(RECORDS).unwrap();
        assert_eq!(
            row_count(&pool, space).await,
            expected,
            "every record is projected"
        );

        corrupt_sealed_segment(&space_dir).unwrap();

        // The first start renames the segment to `.corrupt`; the second finds
        // that residue. Both must refuse the clear.
        for start in 1..=2 {
            crate::reconcile_projections_at_startup(&backend, data_dir.path())
                .await
                .unwrap();
            assert_eq!(
                row_count(&pool, space).await,
                expected,
                "start {start}: rows behind the quarantined segment survive the startup clear"
            );
        }
    }
