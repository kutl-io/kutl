...
pub enum RegistryStoreError {
    ...
    /// divergence this error exists to prevent.
...
pub struct MirrorMetadata<'a> {
    ...
    /// upload, used by tier-quota accounting.
...
pub trait RegistryBackend: Send + Sync {
    ...
    ///
    /// Used by registry rebuild on relay startup, so a deleted document's
    /// tombstone (`deleted_hlc`) survives a restart and a later lower-HLC
    /// re-register cannot resurrect it.
    ...
    /// as `Empty`, matching the in-memory (storeless) model.
    fn update_witness(
        &self,
        _space_id: &str,
        _document_id: &str,
...
/// SELECT all entries for a space INCLUDING tombstones (registry rebuild only).
...
/// SELECT distinct space IDs, counting tombstone-only spaces (rebuild only).
...
impl RegistryBackend for SqliteBackend {
    ...
    fn save_entry(
        ...
        crate::block_on_sync(async { sqlite_upsert_registry_row(&pool, &space_id, &entry).await })
    }
    ...
    fn update_edited_at(
        ...
        let document_id = document_id.to_string();
        crate::block_on_sync(async {
            sqlx::query("UPDATE registry SET edited_at = ? WHERE space_id = ? AND document_id = ?")
                .bind(edited_at)
                .bind(&space_id)
                .bind(&document_id)
                .execute(&pool)
...
mod tests {
    ...
    /// Active-set test oracle: the tombstone-inclusive rebuild reader
    /// filtered to entries without a tombstone (the trait deliberately
    /// has no active-only `load_all` — no product caller).
    fn load_active(backend: &SqliteBackend, space_id: &str) -> Vec<RegistryEntry> {
        backend
            .load_all_including_deleted(space_id)
            .unwrap()
            .into_iter()
    ...
    /// Space-list test oracle: spaces with at least one active entry
    /// (the trait deliberately has no active-only `load_spaces` — no
    /// product caller).
    fn active_spaces(backend: &SqliteBackend) -> Vec<String> {
        backend
            .load_spaces_including_deleted()
            .unwrap()
            .into_iter()
            .filter(|s| !load_active(backend, s).is_empty())
            .collect()
    }
    ...
    async fn test_sqlite_round_trip() {
        ...
        // Initially empty.
        let spaces = active_spaces(&backend);
        ...
        let entries = load_active(&backend, "space-a");
    ...
    async fn test_sqlite_rename_causal_floor_round_trips() {
        ...
        let entries = load_active(&reopened, "space-a");
        assert_eq!(entries.len(), 1);
        assert_eq!(
    ...
    /// store-level half of the tombstones-survive-a-restart guarantee: fast,
    /// container-free, and exercising the real durable reload path the storeless
    /// fixture is exempt from.
    ///
    ...
    /// below, plus `test_reloaded_tombstone_blocks_stale_reregister`). A rebuild
    /// reading an active-only `load_all` would filter the tombstones and lose
    /// exactly this — hence the rebuild-only inclusive reader.
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn test_sqlite_tombstone_not_resurrected_across_reopen() {
        ...
        // relay process going away).
        {
            ...
            backend.delete_entry("space-a", "doc-1", 4000).unwrap();
            assert!(
                load_active(&backend, "space-a").is_empty(),
        ...
        // restart, so `deleted_hlc` is not lost.
    ...
    async fn test_sqlite_load_all_including_deleted_returns_tombstones() {
        ...
        assert!(load_active(&backend, "space-a").is_empty());
        assert!(active_spaces(&backend).is_empty());
    ...
    /// A tombstone reloaded across a restart (via the
    ...
    async fn test_sqlite_save_batch_commits_all() {
        ...
        backend
            ...
            .unwrap();
        let mut paths: Vec<String> = load_active(&backend, "space-a")
            .into_iter()
            .map(|e| e.path)
            .collect();
        paths.sort();
    ...
    async fn test_sqlite_update_edited_at() {
        ...
        let entries = load_active(&backend, "space-1");
        assert_eq!(entries.len(), 1);
