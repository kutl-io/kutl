...
struct AppState {
    ...
    /// Maximum decoded blob size for `upload_blob` (per RFD 0083).
...
/// `GET /relay-policy` — advertise the relay's onboarding policy (RFD 0086),
...
async fn handle_relay_policy(
    ...
    // Pre-connect the relay does not yet know the caller: account-required
    // relays ask the client to authenticate; anonymous relays proceed.
    let next = if state.require_auth {
        kutl_proto::sync::NextStep::Authenticate
    } else {
        kutl_proto::sync::NextStep::Proceed
    };
...
///
/// Without a configured database, the relay is ephemeral. It uses an
/// optional `SQLite` registry backend for document-registration
/// persistence when `data_dir` is configured.
///
...
fn build_app_with_backends_and_web(
    ...
    // Backend presence for the advertised RelayPolicy (RFD 0086), captured
    ...
    // Space registration routes are only added when a space backend is
    // configured (e.g. when `data_dir` is set). DB-backed deployments
    // typically manage spaces via the UX server instead.
    ...
    // (e.g. when `data_dir` is set). DB-backed deployments typically
    // manage invitations via the UX server instead.
    //
...
async fn validate_bearer_token(
    ...
    let (identity, _pat_ctx) = reply_rx
        .await
        .map_err(|_| AuthErrorResponse::internal("relay did not respond"))?
