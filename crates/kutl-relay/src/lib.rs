...
/// In-memory-relay constructors for consumer test suites (behind the
...
pub async fn build_app(
    ...
    let sb: Arc<dyn space_backend::SpaceBackend> = Arc::new(
        space_backend::SqliteSpaceBackend::new(pool.clone())
            .await
            .map_err(|e| anyhow::anyhow!("{e}"))?,
    );
    // The admin routes share the actor's space backend handle.
    let web_sb = Arc::clone(&sb);
...
async fn reconcile_projections_at_startup(
    ...
    for space in &spaces {
        let outcome = reconcile_space_at_startup(change, &store, space).await;
        cleared_total += outcome.cleared;
        if outcome.rebuilt {
            reconciled += 1;
        }
    }
...
/// What the startup reconcile did for one space.
#[derive(Default)]
struct SpaceReconcileOutcome {
    /// Phantom projection rows cleared.
    cleared: u64,
    /// Whether the projection was rebuilt from the fold.
    rebuilt: bool,
}

/// One space's startup reconcile: load its fold once, clear phantom rows
/// (only behind a complete fold), rebuild the projection. Every failure is
/// warned and skipped; the relay always starts.
async fn reconcile_space_at_startup(
    change: &change_sqlite::SqliteChangeBackend,
    store: &signal_store::SignalStore,
    space: &str,
) -> SpaceReconcileOutcome {
    let mut outcome = SpaceReconcileOutcome::default();
    ...
    let loaded = match store.load(space_uuid) {
        Ok(loaded) => loaded,
        Err(e) => {
            tracing::warn!(
                space_id = %space,
    ...
    // A quarantined segment makes the fold incomplete: its records sit on
    // disk as `.corrupt`, not in `records`, so the rows they project are
    // not phantoms. The loader reports the file on every load while it
    // sits in the space directory, so the clear stays off across restarts
    // until an operator moves it out, not only on the start that renamed
    // it. The clear is a hard delete and waits for a complete fold; the
    // rebuild below is additive and still runs.
    let fold_complete = loaded.quarantined.is_empty();
    if !fold_complete {
        tracing::error!(
            space_id = %space,
            quarantined = ?loaded.quarantined,
            "skipping phantom-row clear for space: a quarantined segment makes the fold \
             incomplete; move the .corrupt files out of the space directory once the \
             space has re-seeded to re-enable the clear"
        );
    }
    let cleared_result = if fold_complete {
        change.clear_recordless_rows(space, &records).await
    } else {
        Ok(Vec::new())
    };
    match cleared_result {
        Ok(cleared) => {
            if !cleared.is_empty() {
                outcome.cleared = cleared.len() as u64;
                tracing::warn!(
                    space_id = %space,
                    signal_ids = ?cleared,
...
pub fn build_app_with_backends(
    ...
    space_backend: Option<Arc<dyn space_backend::SpaceBackend>>,
...
/// HTML admin routes (`GET /`, `GET /spaces/{id}`, `GET /join/{code}`). The
/// OSS relay passes a second handle to the actor's own backend; a host may
...
async fn ws_upgrade(
    ...
    // Codec-layer hard cap: reject any frame/message larger than the wire cap
    // before it reaches application code. `WS_MESSAGE_MAX`, not the blob cap:
    // an at-cap blob's message is envelope-larger than the blob, and a codec
    // capped at the blob cap alone refuses uploads the policy allows. Blob
    // POLICY is enforced on the raw bytes in the sync-op handler; per-tier
    // quotas separately via `QuotaBackend`.
    ws.max_message_size(WS_MESSAGE_MAX)
        .max_frame_size(WS_MESSAGE_MAX)
...
mod tests {
    ...
    /// An in-memory `SQLite` pool and the change backend over it.
    async fn in_memory_change_backend() -> (sqlx::SqlitePool, change_sqlite::SqliteChangeBackend) {
        let pool = sqlx::sqlite::SqlitePoolOptions::new()
            .max_connections(1)
    ...
    /// Number of projection rows `space` holds.
    async fn row_count(pool: &sqlx::SqlitePool, space: &str) -> i64 {
    ...
    /// A CREATED flag signal at `ts`, shaped to be both recorded and
    /// projected the way a live relay does both.
    fn flag_record(space: &str, id: &str, record_id: &str, ts: i64) -> kutl_proto::sync::Signal {
        kutl_proto::sync::Signal {
            ...
            hlc: Some(kutl_proto::sync::Hlc {
                physical_ms: u64::try_from(ts).expect("test stamp is non-negative"),
                logical: 0,
                actor: vec![0u8; 16],
            }),
    ...
    async fn test_startup_skips_bad_space_and_clears_good() {
        let (pool, change) = in_memory_change_backend().await;
        ...
        assert_eq!(
            row_count(&pool, good).await,
            0,
            "the valid space's phantom row must be cleared"
        );
        assert_eq!(
            row_count(&pool, bad).await,
            1,
            "the skipped space's row is left alone"
        );
    }
    ...
    async fn test_startup_clears_phantoms_and_keeps_recorded_rows() {
        ...
        // not touch.
        let kept = flag_record(space, "sig-kept", "rKept", 1_000);
        let dir = tempfile::tempdir().expect("tempdir");
        let mut store =
        ...
        assert_eq!(
            ...
            2,
            "one recorded row and one phantom before"
        );
