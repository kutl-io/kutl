//! Relay onboarding policy: derive the `RelayPolicy` advertised to clients
//! from the relay's configuration and wired backends.
...
pub fn build_relay_policy(inputs: &RelayPolicyInputs, next: NextStep) -> RelayPolicy {
    // Authentication is mandatory: every relay requires an authenticated
    // account.
    let auth_model = AuthModel::AccountRequired;
