...
/// rename frame per doc to every other watcher of the space, and ctrl
/// overflow evicts the WHOLE connection (~30 s reconnect-resync). Frames are
/// small (~0.1–0.5 KiB envelopes), so 256 slots cost ~128 KiB per stalled
/// connection worst case — negligible next to the data lane.
pub(crate) const CTRL_CAPACITY: usize = 256;
...
async fn dispatch_message(
    ...
    let cmd = command_from_envelope(conn_id, decode_envelope(bytes)?)?;
    relay_tx.send(cmd).await?;
    Ok(())
}

/// The actor command a client frame is: the one mapping from the wire's
/// client-sent payloads to [`RelayCommand`], so a harness that plays the
/// connection task in-process hands the actor exactly what a socket would.
pub(crate) fn command_from_envelope(
    conn_id: ConnId,
    ...
    let cmd = match envelope.payload {
        ...
        Some(Payload::Barrier(msg)) => RelayCommand::Barrier { conn_id, msg },
        Some(Payload::SubmitFlag(msg)) => RelayCommand::SubmitFlag { conn_id, msg },
        Some(Payload::SubmitComment(msg)) => RelayCommand::SubmitComment { conn_id, msg },
        Some(Payload::SubmitReply(msg)) => RelayCommand::SubmitReply { conn_id, msg },
        Some(Payload::SubmitTransition(msg)) => RelayCommand::SubmitTransition { conn_id, msg },
        Some(Payload::SubscribeSignals(msg)) => RelayCommand::SubscribeSignals { conn_id, msg },
        Some(Payload::SignalReseed(msg)) => RelayCommand::SignalReseedFrame { conn_id, msg },
        ...
        Some(_) => anyhow::bail!("unsupported or server-only message from client"),
