...
/// server's `/api/blob` endpoint per RFD 0083 §"Blob upload cap" — picking
...
/// streaming).
///
/// **Cross-language alignment:** the same literal lives at
/// `commercial/ux/server/src/api/blob.ts::DEFAULT_MAX_BLOB_BYTES`. The
...
pub struct RelayConfig {
    ...
    /// SQL database connection string. When `None`, the relay operates in
    /// no-database mode (in-memory auth, no ACL enforcement) for backward
    /// compatibility.
    ...
    /// Explicit acknowledgement that the operator wants to run the relay
    /// open to the network without authentication.
    ///
    /// Required (set to `true`) when `host` is non-loopback and `require_auth`
    /// is `false`. Without it the relay refuses to start, preventing accidental
    /// exposure of an unauthenticated server. Set via
    /// `KUTL_RELAY_ALLOW_OPEN_RELAY=true`.
...
impl RelayConfig {
    ...
    /// | `KUTL_DATABASE_URL` | *(none)* — shared with UX server |
    ...
    /// | `KUTL_RELAY_ALLOW_OPEN_RELAY` | `false` — required when binding non-loopback with auth disabled |
    ...
    pub fn from_env() -> Self {
        ...
        Self {
            ...
            authorized_keys_file: std::env::var("KUTL_RELAY_AUTHORIZED_KEYS_FILE")
                .ok()
                .map(Into::into),
            allow_open_relay: std::env::var("KUTL_RELAY_ALLOW_OPEN_RELAY")
...
impl Default for RelayConfig {
    /// Loopback-bound, in-memory, no-auth config — matches what
...
/// non-loopback. Used to decide the `require_auth` default and to gate the
/// open-relay safety check.
...
mod tests {
    ...
    fn test_default_matches_loopback_safe_config() {
        ...
        // Sanity: produces a safe-by-default config (loopback bind, no auth).
        assert!(is_loopback_host(&c.host));
    }

    /// Pin the literal numeric value of [`DEFAULT_MAX_BLOB_BYTES`]. The
    /// same number is duplicated in the UX server's blob.ts; this test
    ...
    fn test_default_max_blob_bytes_pinned() {
        ...
        assert_eq!(
            ...
            "DEFAULT_MAX_BLOB_BYTES must stay in sync with commercial/ux/server/src/api/blob.ts; update both or neither"
        );
    }
}
