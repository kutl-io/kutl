...
//!
//! The relay ships with [`SqliteSpaceBackend`] for name→UUID mapping.
//! Database-backed deployments provide their own implementation.
...
///
/// The base trait covers `register`, `resolve_by_name`, `resolve_by_id`,
/// and `list_spaces`. Database-backed deployments extend this with
/// membership and invitation support via their own implementation.
#[async_trait]
pub trait SpaceBackend: Send + Sync {
    ...
    /// Ensure a space row exists for `space_id`, creating it with `name` if
    /// absent. Idempotent and keyed by id: if the space already exists, this is
    /// a no-op and the existing name is preserved (never overwritten). Used to
    /// keep the in-memory registry from diverging from the backend when the WS
    /// path implicit-creates a space that was never explicitly registered.
...
impl SpaceBackend for SqliteSpaceBackend {
    ...
    async fn ensure(&self, space_id: &str, name: &str) -> Result<(), SpaceBackendError> {
        let now = kutl_core::env::now_ms();
        sqlx::query(
            "INSERT INTO spaces (space_id, name, created_at) VALUES (?, ?, ?) \
...
mod tests {
    ...
    async fn test_ensure_creates_when_absent() {
        ...
        backend.ensure(id, id).await.unwrap();

        let found = backend
            .resolve_by_id(id)
            .await
            .unwrap()
            .expect("ensure should have created the space");
        assert_eq!(found.space_id, id);
        assert_eq!(found.name, id, "ensure stores name = space_id");
    }

    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn test_ensure_idempotent_and_never_clobbers() {
        let dir = tempfile::tempdir().unwrap();
        let backend = SqliteSpaceBackend::open(dir.path()).await.unwrap();

        // A space registered with a real name.
        let registered = backend.register("real-name-1234").await.unwrap();

        // ensure() on the same id with a different name must NOT overwrite.
        backend
            .ensure(&registered.space_id, "would-be-clobber")
            .await
            .unwrap();
        // ensure() again is a harmless no-op.
        backend
            .ensure(&registered.space_id, &registered.space_id)
            .await
            .unwrap();

        let found = backend
            .resolve_by_id(&registered.space_id)
            .await
            .unwrap()
            .expect("space still present");
        assert_eq!(
            found.name, "real-name-1234",
            "ensure must not overwrite an existing name"
        );
    }
}
