...
mod tests {
    ...
    fn test_concurrent_rename_lww_is_order_independent() {
        ...
        for (first, first_ts, second, second_ts) in [
            ...
            reg.rename(&uuid(1), "foo.md", first, meta("did:x", first_ts));
            reg.rename(&uuid(1), "foo.md", second, meta("did:y", second_ts));
    ...
    fn test_rename_delete_same_doc_is_order_independent() {
        ...
        for delete_first in [false, true] {
            ...
            if delete_first {
                ...
                reg.rename(&uuid(1), "foo.md", "bar.md", meta("did:x", 2000));
            } else {
                reg.rename(&uuid(1), "foo.md", "bar.md", meta("did:x", 2000));
                reg.unregister(&uuid(1), &meta("did:y", 3000));
            }
        ...
        for delete_first in [false, true] {
            ...
            if delete_first {
                ...
                reg.rename(&uuid(1), "foo.md", "bar.md", meta("did:x", 3000));
            } else {
                reg.rename(&uuid(1), "foo.md", "bar.md", meta("did:x", 3000));
                reg.unregister(&uuid(1), &meta("did:y", 2000));
            }
    ...
    fn test_rename_rename_onto_one_path_conflict_copies() {
        ...
        for swap in [false, true] {
            ...
            if swap {
                reg.rename(&uuid(2), "b.md", "shared.md", meta("did:b", 3000));
                reg.rename(&uuid(1), "a.md", "shared.md", meta("did:a", 2000));
            } else {
                reg.rename(&uuid(1), "a.md", "shared.md", meta("did:a", 2000));
                reg.rename(&uuid(2), "b.md", "shared.md", meta("did:b", 3000));
            }
    ...
    fn test_rename_onto_concurrent_create_conflict_copies() {
        ...
        for swap in [false, true] {
            ...
            reg.register(&uuid(2), "b.md", meta("did:b", 1000));
            if swap {
                reg.rename(&uuid(2), "b.md", "shared.md", meta("did:b", 2000));
    ...
    fn test_rename_after_delete_revives() {
        ...
        reg.rename(&uuid(1), "doc.md", "doc-back.md", meta("did:a", 3000));
    ...
    fn test_rollback_restores_prior_state() {
        ...
        let (token, _displaced) = reg.rename(&uuid(1), "a.md", "b.md", meta("did:a", 2000));
