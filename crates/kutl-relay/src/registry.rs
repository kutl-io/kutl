...
pub struct DocumentRegistry {
    ...
    /// it must not be silent. A monotonic
...
impl DocumentRegistry {
    ...
    pub fn register(
        ...
        let hlc = hlc_of(&meta);
        self.lattice
            .observe(DocRecord::register(did, path, Some(hlc)));
    ...
    pub fn unregister(&mut self, document_id: &str, meta: &EntryMetadata) -> RollbackToken {
        ...
        self.lattice
            .observe(DocRecord::delete(did, self.keep_path(&did), Some(hlc)));
        self.reproject();
        token
    }

    /// The path this document currently holds, or empty if unknown — what a
    /// path-preserving op (delete) carries forward onto its fragment.
    ...
    /// arbitration bug, or a corrupt persisted reload), which
    /// must be observable, not silent. O(documents),
...
mod tests {
    ...
    fn test_divergence_detector_fires_on_injected_violation() {
        // If a confluence premise is ever violated — two ALIVE docs
