//! HTTP handlers for invite management.
//!
//! These routes are mounted when an `InviteBackend` is provided to
//! [`crate::build_app_with_backends`]. Deployments that manage invitations
//! elsewhere pass `None`, so these routes are not mounted there.
//!
...
///
/// **Auth:** intentionally unauthenticated. The invite `code` is
/// itself the capability — redeeming it is a pre-membership discovery step, so
/// gating it behind a bearer would defeat the join flow. See
/// [`handle_create_invite`] for the deployment-level access note.
...
impl InviteErrorResponse {
    fn from_backend_error(e: &InviteBackendError) -> Self {
        match e {
            InviteBackendError::SpaceNotFound(msg) => Self {
                status: StatusCode::NOT_FOUND,
                message: msg.clone(),
            },
            ...
            // Log the detailed storage error server-side; return a generic
            // message so SQL fragments, column names, and the db path are
            // never echoed to anonymous callers.
            InviteBackendError::Storage(msg) => {
                tracing::error!(error = %msg, "invite backend storage error");
                Self {
