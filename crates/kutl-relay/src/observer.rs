...
/// A new signal was created (flag, chat, or reply).
#[derive(Debug, Clone)]
pub struct SignalCreatedEvent {
    ...
    pub author_did: String,
    /// Signal type: `"flag"`, `"chat"`, or `"reply"`.
    ...
    // Chat fields (present when signal_type = "chat")
    /// Topic of the chat thread. Present for chats.
...
pub trait RelayObserver: Send + Sync {
    ...
    /// Called when a signal (flag, chat, or reply) is created.
