...
pub struct SignalCreatedEvent {
    ...
    /// `api_tokens.id` of the PAT the author used, when PAT-authenticated.
    /// `None` for DID challenge-response auth. Per RFD 0044.
...
pub struct ReactionEvent {
    ...
    /// `None` for DID challenge-response auth. Per RFD 0044, surfaces
    /// per-PAT attribution downstream of the observer.
    pub via_pat_id: Option<String>,
...
pub struct SignalClosedEvent {
    ...
    pub actor_did: String,
    /// `api_tokens.id` of the PAT the actor used, when PAT-authenticated.
    /// `None` for DID challenge-response auth.
    pub via_pat_id: Option<String>,
