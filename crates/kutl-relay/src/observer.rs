...
pub trait AfterMergeObserver: Send + Sync {
    ...
    async fn after_text_merge(&self, event: MergedEvent, doc: &Document);
...
impl AfterMergeObserver for NoopAfterMergeObserver {
    async fn after_text_merge(&self, _event: MergedEvent, _doc: &Document) {}
}
...
impl AfterMergeObserver for TracingObserver {
    async fn after_text_merge(&self, event: MergedEvent, _doc: &Document) {
        tracing::info!(
            space_id = %event.space_id,
            document_id = %event.document_id,
            author_did = %event.author_did,
