...
use kutl_proto::sync::{self as sync, ErrorCode, Signal, SignalEventType};
use tracing::error;
use uuid::Uuid;
...
impl SignalLogHandle {
    ...
    ///
    /// A relay with no log answers catch-up with empty pages and cannot
    /// accept pushed history, whatever substrate the others use; the re-seed
    /// advertisement is gated on this.
    #[must_use]
...
impl Relay {
    ...
    /// `Internal`.
    ///
    /// Validation runs BEFORE the no-record-log early-return, so it gates a
    /// relay without a record log exactly as it gates one with.
    ///
    ...
    /// and INDEPENDENTLY: a relay without a record log keeps no records, one
    /// without a projection maintains no rows, and neither implies the other. On any failure the
    ...
    /// for must succeed before the announcement fires (a deployment without a
    /// record log or a projection skips its absent stages, so the guarantee is
    ...
    ///
    /// **No filters, and none may be added.** Every listener whose stream is
    /// flowing gets every record. Audience decides who is TOLD and how a record renders, never
    ...
    ///
    /// Infallible by design. A listener whose lane is full has its stream
    /// paused and is told to re-subscribe, which is recovery rather than a
    /// failed emit; the record is already durable by the time this runs. A
    /// stream already paused is skipped, not re-notified.
    fn replicate_record(&mut self, space_id: &str, record: &Signal) {
        let Some(listeners) = self.listeners.get(space_id) else {
            return;
        };
        let recipients: Vec<ConnId> = listeners
            .iter()
            .filter(|(_, stream)| **stream == crate::relay::SignalStream::Flowing)
            .map(|(cid, _)| *cid)
...
impl Relay {
    ...
    ///
    /// A refusal is the typed [`sync::Error`] every other reply carries, built
    /// through [`refusal`]: the code is the verdict a client branches on, the
    /// prose is for people and may be reworded freely.
    pub(super) fn send_signal_ack(
        &self,
        conn_id: super::ConnId,
        client_ref: &str,
        signal_id: &str,
        record_id: &str,
        outcome: Result<(), sync::Error>,
    ) {
        ...
        if let Some(d) = msg.close_reason {
            let Ok(reason) = sync::CloseReason::try_from(d) else {
                self.send_signal_ack(
                    ...
                    Err(refusal(
                        ErrorCode::InvalidMessage,
                        format!("unknown close_reason discriminant {d}"),
                    )),
                );
                return;
            };
    ...
    pub(super) async fn handle_submit_reseed(
        ...
        // Every refusal here is the batch's, not the caller's standing: a
        // relay that takes no re-seed, a record the ingest gate turned away,
        // or a store fault. Authorization was settled above.
        let result = match outcome {
            Err(e) => Err(refusal(ErrorCode::InvalidMessage, e.to_string())),
            Ok(o) if o.rejected > 0 => Err(refusal(
                ErrorCode::InvalidMessage,
                format!(
    ...
    /// caller — the WS submit frames below and the MCP handlers, which map the
    /// lookup error into their own shape through one function
    /// (`signal_lookup_error`). One probe, because two would drift: a check
    ...
    ///
    /// Skipped when no projection is configured: a relay without one cannot
    /// answer, and refusing every such frame on one would be worse than the risk.
    pub(super) fn signal_is_in_space(
        &self,
        space_id: &str,
        signal_id: &str,
    ) -> impl Future<Output = Result<(), ChangeError>> + Send + use<> {
        // Nothing is copied unless there is a projection to ask.
        let job = self
            .record_log
            .reads_owned()
            .map(|backend| (backend, space_id.to_owned(), signal_id.to_owned()));
        async move {
            let Some((backend, space_id, signal_id)) = job else {
                return Ok(());
            };
            backend
    ...
    async fn submit_targets_signal_in_space(
        ...
        let error = match self.signal_is_in_space(space_id, signal_id).await {
            Ok(()) => return true,
            Err(ChangeError::NotFound(_)) => refusal(
                ErrorCode::NotFound,
                format!("signal {signal_id} is not in this space"),
            ),
            Err(e) => refusal(
                ErrorCode::InvalidMessage,
                format!("cannot verify signal {signal_id}: {e}"),
            ),
        };
        self.send_signal_ack(conn_id, client_ref, "", "", Err(error));
        false
    }
        ...
        match self.authorize_conn(conn_id, space_id).await {
            ...
            Err(e) => {
                self.send_signal_ack(
                    ...
                    "",
                    "",
                    Err(refusal(ErrorCode::AuthFailed, e.to_string())),
                );
                false
            }
        }
    ...
    /// Ack a create-shaped submit: the signal id on success, the refusal on
    /// failure.
    fn ack_submit(
        &self,
        conn_id: super::ConnId,
        ...
        match outcome {
            ...
            Err(e) => self.send_signal_ack(conn_id, client_ref, "", "", Err(change_refusal(&e))),
        }
    }
}

/// The refusal a signal door acks for a [`ChangeError`]: a missing target is
/// `NOT_FOUND`; everything else, the caller's bad argument and the store's own
/// fault alike, is `INVALID_MESSAGE`, the code every carrier gives a failure
/// that is neither authorization nor quota. The prose names the fault.
fn change_refusal(e: &ChangeError) -> sync::Error {
    let code = match e {
        ChangeError::NotFound(_) => ErrorCode::NotFound,
        ChangeError::Db(_) | ChangeError::Internal(_) | ChangeError::InvalidArgument { .. } => {
            ErrorCode::InvalidMessage
        }
    };
...
mod tests {
    ...
    /// The message of a refused ack, after checking the refusal carries the
    /// verdict a client branches on: the code is the contract, the prose is
    /// only inspected for what it names.
    fn refusal_message(ack: sync::SignalAck, code: sync::ErrorCode) -> String {
        assert!(!ack.success, "expected a refusal, got success");
    ...
    async fn test_submit_flag_from_a_non_member_is_rejected_and_acked() {
        ...
        refusal_message(got, sync::ErrorCode::AuthFailed);
    ...
    async fn test_submit_create_with_non_uuid_ids_is_rejected_and_names_the_field() {
        ...
        let err = refusal_message(got, sync::ErrorCode::InvalidMessage);
        assert!(
    ...
    /// close it. The same hole the MCP handlers close through the shared
    /// `signal_is_in_space` probe, quoted there as "a member of space-A could
    /// reply to a parent signal in space-B".
    #[tokio::test]
    async fn test_submit_transition_cannot_reach_a_signal_in_another_space() {
        ...
        let err = refusal_message(got, sync::ErrorCode::NotFound);
        assert!(
            err.contains("not in this space"),
            "the rejection should name the reason, got {err}"
        );
    }
    ...
    async fn test_subscribe_signals_pages_the_backlog() {
        ...
        // The page answers the connection's own request, so it rides the
        // own-ack lane, whatever the data lane holds.
        let mut page = None;
        while let Ok(bytes) = ack.try_recv() {
            if let Ok(env) = kutl_proto::protocol::decode_envelope(&bytes)
        ...
        let page = page.expect("a SignalPage must arrive on the own-ack lane");
        assert_eq!(
            page.records.len(),
            1,
    ...
    async fn test_reseed_frame_appends_and_a_refusing_relay_acks_the_refusal() {
        ...
        refusal_message(refusal, sync::ErrorCode::InvalidMessage);
        let none = relay
            .record_log
