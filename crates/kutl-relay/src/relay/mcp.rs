...
pub struct McpParticipant {
    ...
    /// Whether this actor is reachable right now, and over what: `"websocket"`
    /// (a live subscriber — usually a daemon or browser), `"mcp"` (a live MCP
    /// session), or `"offline"`.
    ///
...
impl Relay {
    ...
    /// - **Storeless mode** (`space_backend` is None): there is no
    ///   space-registry authority to resolve against, so return the raw
    ...
    ///   a space backend, so it never reaches this branch. The storeless
    ...
    ///
    /// In **storeless mode** (no `space_backend` — the kutlhub host relay
    /// or a standalone test actor) there is no canonical authority —
    /// return the raw caller-supplied string. Existence is gated by the
    /// downstream membership check (see the storeless bullet above), not
    /// created here. The kutlhub host and backendless test scenarios
    ...
    async fn mcp_check_space_registered(&self, space_id: &str) -> Result<String, McpError> {
        ...
        // Storeless mode: no space backend to resolve against. Return the
        // raw id; the caller's membership check (authorize_mcp_caller) is
    ...
    pub(super) async fn handle_mcp_list_participants(
        ...
        // Who is reachable right now. WS subscribers are space-scoped by the
        // loop below; MCP sessions are not scoped at all, so a session may only
        // MARK an actor of this space as connected — it must never add one, or
        // a session in another space would leak its DID into this list.
    ...
    pub(super) async fn handle_mcp_resolve_participant(
        ...
        // Neither authorizer is configured (storeless/test shape): there is
    ...
    /// this handler, and the storeless branch only lazily materializes the
    ...
    pub(super) async fn handle_mcp_list_spaces(
        ...
        if let Some(membership) = self.membership_backend.as_ref().map(Arc::clone) {
            ...
            // DID, so strip that prefix directly. Otherwise
    ...
    /// [`signal_is_in_space`](Relay::signal_is_in_space) in MCP's error shape.
    ///
    /// Prevents cross-space mutations where a caller authorized for space-A
    /// supplies a `signal_id` that actually lives in space-B.
    pub(super) async fn verify_signal_in_space(
        &self,
        space_id: &str,
        signal_id: &str,
    ) -> Result<(), McpError> {
        match self.signal_is_in_space(space_id, signal_id).await {
            Ok(()) => Ok(()),
...
pub(in crate::relay) mod tests {
    ...
    use super::super::{TEST_CONN_DID, resident_blob, test_config};
    use super::*;
    ...
    async fn feed_delivered_to(did: &str, records: Vec<sync::Signal>) -> Vec<String> {
        let relay = relay_with_changes(records);
    ...
    async fn test_close_reopen_persist_as_records() {
        ...
        // is what `verify_signal_in_space` then finds. Seeding the id first
