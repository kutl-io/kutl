...
impl Relay {
    ...
    pub(super) async fn handle_signal(&mut self, conn_id: ConnId, msg: &sync::Signal) {
        ...
        if let Err(reason) = crate::ids::check_uuid("document_id", document_id) {
            self.send_error(conn_id, ErrorCode::InvalidMessage, &reason);
            return;
        }
    ...
    /// cursor for more. The cursor advances only after a page is successfully
    /// enqueued, so a retry re-sends rather than skips.
    ///
    /// The page rides the **data** lane, lowest priority of the three, because
    /// bulk backfill should yield to control traffic rather than preempt it.
    /// Overflow is evict-and-notify, never presence's bare `try_send` with a
    /// silent drop — and the notice rides the **ack** lane, not the lane
    /// that just overflowed, because a notice sent down a congested lane
    /// re-strands the subscriber it exists to save.
    ///
    /// Live delivery to the subscriber set is NOT here: that needs the
    /// per-space subscriber registry, which is the next commit group. This
    /// serves the backlog.
    pub(super) async fn handle_subscribe_signals(
        ...
        // PAT context, which `authorize_space` on a bare DID does not.
        if self.authorize_conn(conn_id, &msg.space_id).await.is_err() {
            self.send_stale_signal_stream(conn_id, &msg.space_id, "not authorized for this space");
            return;
        }

        // REGISTER FIRST, then backfill. The two answer different questions:
        // registration is "deliver this space's signals to me from now on",
        // which needs nothing but an authorized connection; the page is "and
        // here is what you missed", which needs a record log and a space id the
        // log can key on. Gating registration on the page would mean a space
        // with no records — or a momentarily full data lane — silently left the
        // subscriber out of live delivery, which is the exact failure mode this
        // registry exists to remove.
        self.signal_subscribers
            .entry(msg.space_id.clone())
            .or_default()
            .insert(conn_id, msg.cursor.clone());

        // Backfill needs a UUID space id to key the log by. A non-UUID space
        // has no records (segments are UUID-named), so there is nothing to page
        // and nothing to report.
        let Ok(space_uuid) = uuid::Uuid::parse_str(&msg.space_id) else {
            return;
        };
        ...
        let records = match self
            .record_log
            .list(space_uuid, msg.cursor.as_ref(), SIGNAL_PAGE_SIZE + 1)
            .await
        {
            Ok(records) => records,
            Err(e) => {
        ...
        // Pages go through the same guarded door as live broadcasts, so the
        // document-headroom reserve binds this writer too — a catch-up burst
        // (a reconnecting multi-space daemon) must not spend the slots that
        // keep document ops from evicting the connection. The remedy differs
        // from a broadcast's: no eviction needed, a stale notice retries.
        match self.try_send_signal_frame(conn_id, bytes) {
            crate::relay::SignalSend::Sent => {
                // The cursor advances only after the page is enqueued, so a
                // retry re-sends rather than skips.
                if let Some(subs) = self.signal_subscribers.get_mut(&msg.space_id) {
                    subs.insert(conn_id, page.high_water);
                }
            }
            crate::relay::SignalSend::Yielded => {
                debug!(
                    conn_id,
                    "signal page yielded to document traffic; notifying stale"
                );
                self.send_stale_signal_stream(
                    conn_id,
                    &msg.space_id,
                    "signal page yielded to document traffic; re-subscribe",
                );
            }
            crate::relay::SignalSend::LaneFull => {
                debug!(
                    conn_id,
                    "signal page did not fit the data lane; notifying stale"
                );
                self.send_stale_signal_stream(
                    conn_id,
                    &msg.space_id,
                    "signal page did not fit the data lane; re-subscribe",
                );
            }
            crate::relay::SignalSend::Gone => {}
        }
    }

    /// Tell a subscriber its signal stream is not going to deliver, and that
    /// re-subscribing is the remedy.
    ///
    /// Every `SubscribeSignals` failure takes this shape rather than a
    /// `sync::Error`, and that is deliberate: a bare `Error` frame is treated as
    /// FATAL by the client and by daemon startup, so answering one subscribe
    /// with it would tear down a connection carrying unrelated document sync.
    /// A stale notice is scoped to the space it names and says
    /// what to do about it.
    ///
    ...
    /// leave the evicted subscribers stranded with nothing to retry.
    pub(super) fn send_stale_signal_stream(&self, conn_id: ConnId, space_id: &str, reason: &str) {
    ...
    async fn test_a_full_data_lane_evicts_the_signal_stream_not_the_connection() {
        ...
        while let Ok(bytes) = sub_ack.try_recv() {
            if let Ok(env) = kutl_proto::protocol::decode_envelope(&bytes)
                && matches!(
                    env.payload,
                    Some(sync::sync_envelope::Payload::StaleSignalStream(_))
                )
            {
                notices += 1;
            }
