...
use crate::protocol::{
    encode_envelope, refusal, register_document_ack_envelope, rename_document_ack_envelope,
...
impl Relay {
    ...
    /// broadcast the registration to the space's listeners, and notify the
    /// observer. Called by both WebSocket and MCP paths.
    ///
    ...
    pub(super) async fn register_document_internal(
        ...
        crate::ids::check_within_space_path(path)?;
    ...
    pub(super) async fn handle_register_document(
        ...
        if !self
            .authorize_or_reject(conn_id, &msg.space_id, |err| {
                register_document_ack_envelope(
                    &msg.space_id,
                    &msg.document_id,
                    false,
        ...
        meta.account_id = self.resolve_account_id(conn_id).await;
    ...
    /// Send a failure lifecycle ack (no persisted entry): build the
    /// `sync::Error` for `code` + `reason`, hand it to `build_ack` for the
    /// per-message ack shape, and send it on the ack lane. Rejection is
    /// ACKED, never silently dropped — a caller that gets silence cannot
    /// distinguish "refused" from "lost".
    fn send_lifecycle_failure(
        &self,
        conn_id: ConnId,
        code: ErrorCode,
        reason: String,
        build_ack: impl FnOnce(sync::Error) -> sync::SyncEnvelope,
    ) {
        let ack = build_ack(refusal(code, reason));
        self.send_ack(conn_id, &encode_envelope(&ack));
    }

    /// Authorize `conn_id` for `space_id`; on failure emit the failure ack
    /// (built by `build_ack`) and return `false`. The ack is the refusal:
    /// it names the document, so the sender correlates the verdict with its
    /// request. (The `AuthorizedSpace` token is deliberately dropped: no
    /// lifecycle handler consumes it.)
    ...
    fn send_rename_failure(
        ...
        self.send_lifecycle_failure(conn_id, code, reason, |err| {
            rename_document_ack_envelope(space_id, document_id, false, None, Some(err), None)
        });
    }
    ...
    pub(super) async fn handle_rename_document(
        ...
        if let Err(reason) = crate::ids::check_within_space_path(&msg.new_path)
            .and_then(|()| reject_reserved_conflict_path("rename", &msg.new_path))
        {
    ...
    pub(super) async fn handle_unregister_document(
        ...
        if !self
            .authorize_or_reject(conn_id, &msg.space_id, |err| {
                unregister_document_ack_envelope(&msg.space_id, &msg.document_id, false, Some(err))
            })
            .await
        {
            return;
        ...
        // Settle the document's parked edit while it is still resident and
        // its marker trackers still stand: the snippet debounce would
        // otherwise fire after the cascade below tombstones the markers and
        // the forget drops the trackers, re-seed from the tombstoned records
        // (an empty baseline) and re-materialize every marker as a fresh
        // CREATED on a deleted document. The document is soft-deleted
        // already, so the after-merge funnel fires nothing here; the flush
        // stops the timer, drops the parked edit and lands `edited_at`.
        let key = DocKey {
            space_id: msg.space_id.clone(),
            document_id: msg.document_id.clone(),
        };
        ...
        self.forget_document_markers(&msg.space_id, &msg.document_id);
    ...
    /// send it the revival register (see `revival_register_payload`), targeted
    /// at the sender ALONE — other clients still hold the live doc and must not
    /// be disturbed.
    fn send_revival_register(&self, conn_id: ConnId, space_id: &str, document_id: &str) {
        if let Some(payload) = self.revival_register_payload(space_id, document_id) {
            self.send_payload(conn_id, payload);
        }
    }

    /// Re-assert a document a content edit just REVIVED to the whole space,
    /// the editor included. The delete the edit beat was already broadcast
    /// and honored, so every daemon has dropped its file and identity — the
    /// editor's own file went with it when the edit reached the relay after
    /// the delete — and each must re-subscribe and re-materialize. No-op if
    /// the doc isn't active.
    pub(super) async fn broadcast_revival_register(
        &mut self,
        space_id: &str,
        document_id: &str,
        editor: ConnId,
    ) {
        let Some(payload) = self.revival_register_payload(space_id, document_id) else {
            return;
        };
        self.send_payload(editor, payload.clone());
        self.broadcast_lifecycle_to_space(space_id, editor, payload)
            .await;
    }

    /// The `RegisterDocument` that re-asserts a live document to a daemon whose
    /// delete lost: the doc's current active path, stamped with its liveness
    /// HLC. The stamp is NOT an admission ticket — registers are never
    /// freshness-gated (`handle_remote_register` treats a re-assert of a known
    /// id as a revival that MUST apply). The daemon FOLDS it into the per-doc
    /// lifecycle watermark, so a later stale rename/delete at or below that
    /// stamp is dropped, and re-subscribes off the seeded record. `None` if the
    /// doc isn't active (a genuine delete races this path).
    fn revival_register_payload(
        &self,
        space_id: &str,
        document_id: &str,
        ...
        let payload = sync::RegisterDocument {
            ...
            metadata: Some(relay_reassert_metadata(hlc)),
            ..Default::default()
        };
        Some(sync::sync_envelope::Payload::RegisterDocument(payload))
    }
    ...
    /// writer with no data loss. A no-op on a relay without a record log or
    /// a non-UUID space id (real spaces are UUIDs).
    ...
    pub(super) async fn handle_unregister_space(
        ...
        if !self
            .authorize_or_reject(conn_id, &msg.space_id, |err| {
                unregister_space_ack_envelope(&msg.space_id, Vec::new(), false, Some(err))
            })
            .await
        {
            return;
    ...
    async fn broadcast_bulk_unregister(
        ...
        for document_id in document_ids {
            ...
            self.forget_document_markers(space_id, document_id);
        }
    }
    ...
    pub(super) async fn handle_transfer_space_ownership(
        ...
        if !self
            .authorize_or_reject(conn_id, &msg.space_id, |err| {
                transfer_space_ownership_ack_envelope(&msg.space_id, 0, false, Some(err))
            })
            .await
        {
            return;
    ...
    /// Broadcast a lifecycle message to every connection listening to the
    /// space (except `sender`).
    ///
    /// `listeners` is the whole recipient set, whatever the state of each
    /// listener's signal stream: a paused stream is a full data lane, and
    /// lifecycle rides ctrl. A connection enters the set by `SubscribeSignals`
    /// (a daemon's first request in every session) and leaves it on
    /// disconnect. Document subscriptions do not make a connection a
    /// recipient: the clients that subscribe without listening (a browser
    /// behind a proxy, a one-shot CLI connection) discard lifecycle frames,
    /// and finding them would mean walking every document slot per message,
    /// an O(documents) cost on each file of a bulk rename.
    ///
    /// Recipients are collected first, then fanned out, so the per-recipient
    /// `send_broadcast` (which evicts on a full lane, mutating `listeners`)
    /// never runs while the set is borrowed.
    async fn broadcast_lifecycle_to_space(
        ...
        let recipients: Vec<ConnId> = self
            .listeners
            .get(space_id)
            .map(|conns| {
                conns
                    .keys()
                    .copied()
                    .filter(|&listener| listener != sender)
                    .collect()
            })
            .unwrap_or_default();
...
/// The `ChangeMetadata` the relay stamps on a lifecycle re-assert it authors
/// itself. No authoring DID: there is no single author for a relay-issued
/// correction (the surviving content's authors are on the CRDT ops the daemon
/// re-subscribes to). The caller supplies the liveness stamp, already
/// transformed for its own freshness-gate needs.
fn relay_reassert_metadata(hlc: Option<kutl_proto::sync::Hlc>) -> sync::ChangeMetadata {
    sync::ChangeMetadata {
        author_did: String::new(),
        timestamp: kutl_core::env::now_ms(),
        hlc,
...
mod tests {
    ...
    /// The recipient set for lifecycle broadcasts is the listener set and
    /// nothing else: a connection listening to the space receives the rename,
    /// a connection that only subscribed to the document does not.
    #[tokio::test]
    async fn test_lifecycle_broadcast_reaches_listeners_not_bare_subscribers() {
        let mut relay = Relay::new_standalone(test_config());
        let space = "5171e0a1-2222-4000-8000-000000000001";
        let doc = "0f0f0f0f-2222-4000-8000-000000000001";

        let (_s_data, _s_ack, _s_ctrl) = connect_client(&mut relay, 1).await;
        let (_w_data, _w_ack, mut w_ctrl) = connect_client(&mut relay, 2).await;
        let (_b_data, _b_ack, mut b_ctrl) = connect_client(&mut relay, 3).await;

        register_via_command(&mut relay, 1, space, doc, "a.md", 1_000).await;
        // The listener subscribes to the space's signals; the bare subscriber
        // only subscribes to the document.
        relay
            ...
            .await;
        super::super::subscribe_doc(&mut relay, 3, space, doc).await;
        ...
        assert!(
            drained_rename_to(&mut w_ctrl, doc, "b.md"),
            "the listener receives the rename broadcast"
        );
        assert!(
            !drained_rename_to(&mut b_ctrl, doc, "b.md"),
            "a bare document subscriber is not a lifecycle recipient"
        );
    }
    ...
    async fn test_register_collision_displaces_prior_holder_and_broadcasts() {
        ...
        // Both listen to the space, so they receive lifecycle broadcasts.
        for conn_id in [1, 2] {
            relay
                .process_command(RelayCommand::SubscribeSignals {
                    conn_id,
    ...
    #[tokio::test]
    async fn test_full_broadcast_lane_evicts_listener() {
        // Invariant: a lifecycle broadcast to a listener whose bounded ctrl
        // lane is FULL must EVICT that connection (not silently drop the
        // frame in place). The evicted listener re-subscribes on reconnect,
        // so eviction is the recoverable, OOM-bounded policy.
        ...
        // The victim: a listener with a 1-slot ctrl that we never drain.
        ...
        relay
            ...
            .await;
        // Authentication is mandatory; the listener must be authorized or
        // `SubscribeSignals` is refused before it registers.
        ...
        assert!(
            !relay.connections.contains_key(&watcher),
            "a broadcast to a full ctrl lane evicts the connection"
        );
        assert!(
            relay
                .listeners
                .get(space)
                .is_none_or(|l| !l.contains_key(&watcher)),
            "the evicted listener is dropped from the listener set"
        );
    }
}
