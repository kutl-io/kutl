...
impl Relay {
    ...
    fn send_lifecycle_failure(
        ...
        let ack = build_ack(lifecycle_ack_error(code, reason));
        self.send_ack(conn_id, &encode_envelope(&ack));
    }

    /// Authorize `conn_id` for `space_id`; on failure emit the legacy error
    /// frame AND a correlating failure ack (built by `build_ack`), returning
    /// `false`. Auth failures pre-date the persist step — the failure ack
    /// rides the same channel as the error frame so senders can correlate
    /// the result. (The `AuthorizedSpace` token is deliberately dropped: no
