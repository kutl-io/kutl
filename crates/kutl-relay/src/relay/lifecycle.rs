...
//! relay.rs still routes lifecycle envelopes to these handlers, and the MCP
//! family keeps reaching `register_document_internal` and `persist_entry`
//! through their pre-split `super::` paths.
...
impl Relay {
    ...
    pub(super) fn register_document_internal(
        &mut self,
        space_id: &str,
        document_id: &str,
        path: &str,
    ...
    pub(super) async fn handle_rename_document(
        ...
        let (forwarded, observed_author_did) =
            sanitize_forwarded_rename(msg, &authoritative_path, authoritative.as_deref());
    ...
    fn broadcast_lifecycle_to_space(
        ...
        for conn_id in recipients {
            self.send_broadcast(conn_id, &bytes);
        }
    }
    ...
    fn broadcast_displaced(&mut self, space_id: &str, displaced: &[String]) {
...
fn sanitize_forwarded_rename(
    ...
    if let Some(did) = authoritative {
        let mut new_meta = forwarded.metadata.unwrap_or_default();
        new_meta.author_did.clear();
        new_meta.author_did.push_str(did);
        forwarded.metadata = Some(new_meta);
    }
    let observed_author_did = forwarded
        .metadata
...
mod tests {
    ...
    use super::super::{RelayCommand, connect_client, test_config};
    use super::*;
    ...
    async fn test_bulk_register_acks_never_dropped() {
        // Mode 2 regression: a bulk register of more docs than the cap-16 ctrl
        // channel holds (`conn.rs:14`) must still deliver a success ack for
        // EVERY doc. The old `send_ctrl` `try_send`-and-warn-dropped on full, so
        // acks past slot 16 vanished and those docs stranded at
        // `confirmed:false` forever (the dogfooding bulk-seed symptom). Register
        // acks now ride the unbounded own-ack lane, so the burst is loss-free.
        ...
        // every ack — the cap-16 ctrl channel is no longer in the ack path.
    ...
    async fn test_full_broadcast_lane_evicts_subscriber() {
        // Commit B invariant: a lifecycle broadcast to a subscriber whose
