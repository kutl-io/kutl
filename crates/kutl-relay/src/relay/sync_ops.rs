...
use crate::protocol::{
    ...
    presence_update_envelope, sync_ops_envelope, wrap_error, wrap_quota_exceeded,
};
...
impl Relay {
    ...
    async fn handle_text_sync_ops(&mut self, conn_id: ConnId, msg: &sync::SyncOps, key: &DocKey) {
        ...
        if let Err(e) = slot.mutate_text(|doc| doc.merge(&msg.ops, &msg.metadata)) {
            if matches!(e, kutl_core::Error::OpCountExceeded { .. }) {
                ...
                // divergence — the sender keeps the edit, no peer ever sees
                // it. Carry the doc identity so operator logs are greppable
                // per document (the plain warn below has only conn_id).
                error!(
                    conn_id,
    ...
    pub(super) async fn handle_presence_update(&self, conn_id: ConnId, msg: &sync::PresenceUpdate) {
...
mod tests {
    ...
    use super::super::{RelayCommand, connect_client, resident_blob, subscribe_doc, test_config};
    use super::*;
