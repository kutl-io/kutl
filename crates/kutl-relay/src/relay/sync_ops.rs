...
impl Relay {
    pub(super) async fn handle_sync_ops(&mut self, conn_id: ConnId, msg: &sync::SyncOps) {
        ...
        if !self.documents.contains_key(&key) {
            let err = crate::protocol::wrap_error_for_document(
                ErrorCode::NotFound,
    ...
    async fn handle_text_sync_ops(&mut self, conn_id: ConnId, msg: &sync::SyncOps, key: &DocKey) {
        ...
        // one cascade helper so revive entry points can't drift.
        if edit_revived {
    ...
    async fn handle_blob_sync_ops(&mut self, conn_id: ConnId, msg: &sync::SyncOps, key: &DocKey) {
        ...
        // Notify observer about the blob edit. Use the authenticated DID so a
        // peer cannot forge a different `author_did` in observer fan-out.
...
/// Log a failed inbound text merge and build the error envelope the sender
/// gets back.
///
...
/// copy. Everything else is `InvalidMessage` — scoped, not bare: the client
/// acts on the structured space/document fields, and an unscoped reject is
/// unactionable — it degrades into a log line while the sender keeps
/// re-submitting the same unmergeable history.
fn classify_merge_reject(
    ...
    match e {
        ...
        _ => crate::protocol::wrap_error_for_document(
