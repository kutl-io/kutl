...
//! relay.rs still routes subscribe/unsubscribe to these handlers, and the
//! residue flush/snippet/sync-ops paths reach the slot types and
...
use crate::protocol::{
    blob_ops_envelope, encode_envelope, stale_subscriber_envelope, subscribe_status_envelope,
    sync_ops_envelope,
};
use crate::registry;
...
impl Relay {
    ...
    pub(super) async fn load_from_backends(&self, space_id: &str, doc_id: &str) -> LoadResult {
    ...
    pub(super) fn handle_unsubscribe(&mut self, conn_id: ConnId, msg: &sync::Unsubscribe) {
        let key = DocKey {
            space_id: msg.space_id.clone(),
            document_id: msg.document_id.clone(),
        };
...
/// droppable lane here is a permanent silent strand (the N=5000 bulk-add
/// flatline / the catch-up notice storm — both fixed by moving onto this
/// lane). ONE owner for that lane choice, shared by the live broadcast
...
/// subscriber from MANY docs at once, and that notice storm overflowed the
/// bounded cap-16 ctrl lane — the dropped notices left the subscriber
/// stranded registered-and-content-less forever, with nothing to retry (the
/// N=5000 bulk-add flatline: the observer froze at 2430/5000 with zero
/// daemon-side evidence). Same rationale as [`evict_for_failed_catch_up`]'s
...
/// sibling of the broadcast-path bug 3). Route it through the same recovery
...
/// full state. Surfaced by a 300-doc subscribe burst overflowing the cap-64
/// data lane.
///
...
/// subscribe-burst's notice storm overflowed cap 16 and the lost notices
/// re-stranded the subscriber the notice exists to save.
...
mod tests {
    ...
    use super::super::{
        RelayCommand, connect_client, flush_and_confirm, send_text_ops, subscribe_doc, test_config,
    };
    use super::*;
    ...
    /// nothing to retry (the N=5000 bulk-add flatline at 2430/5000).
    #[tokio::test]
    async fn test_broadcast_eviction_notice_rides_unbounded_ack_lane() {
        ...
        subscribe_doc(&mut relay, subscriber, "space", "doc").await;
        ...
        while sub_data.try_recv().is_ok() {}
        send_text_ops(&mut relay, publisher, "space", "doc", "after eviction\n").await;
        assert!(
    ...
    /// own-ack lane. Direct pin for `evict_for_failed_catch_up` — previously
    /// covered only en route by the bulk e2e probes.
    #[tokio::test]
    async fn test_subscribe_catch_up_overflow_evicts_with_ack_lane_notice() {
        ...
        let publisher: ConnId = 1;
        let (_pub_data, _pub_ack, _pub_ctrl) = connect_client(&mut relay, publisher).await;
        subscribe_doc(&mut relay, publisher, "space", "doc").await;
        send_text_ops(&mut relay, publisher, "space", "doc", "content\n").await;
        ...
        subscribe_doc(&mut relay, victim, "space", "doc").await;
        ...
        send_text_ops(&mut relay, publisher, "space", "doc", "after\n").await;
        assert!(
