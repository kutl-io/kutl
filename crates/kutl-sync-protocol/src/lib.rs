//! Pure classification rules for the kutl sync protocol.
//!
...
/// Classification of a subscribe cold-load.
///
...
pub enum LoadClass {
    ...
    /// was previously persisted. The dangerous failure mode — the
...
/// Server-side classification of a cold-load outcome.
///
...
/// Client-side symmetric fail-closed rule.
///
...
/// The hard edit guard: is the editor allowed
...
mod tests {
    ...
    // ------------------------------------------------------------------
    // classify_load: server-side classifier
    // ------------------------------------------------------------------

    /// Content `Present` → `FoundContent` regardless of witness.
    #[test]
    ...
    /// Both absent + witness == 0 → Empty (first-time doc).
    #[test]
    ...
    /// Backend Err → Unavailable regardless of witness. The
    ...
    // ------------------------------------------------------------------
    // resolve_client_classification: symmetric rule
    // ------------------------------------------------------------------

    /// Found passes through whether or not the client has
    ...
    /// The symmetric guard: server says Empty, client
    ...
    // ------------------------------------------------------------------
    // should_allow_editing: hard edit guard
    // ------------------------------------------------------------------
