//! End-to-end integration test for `kutl mcp serve` MCP server.
//!
//! Spawns `kutl mcp serve` as a subprocess, sends JSON-RPC requests via stdin,
//! and verifies responses on stdout. No relay or database required — tests
...
async fn test_watch_initialize_and_list_tools() {
    // Spawn kutl mcp serve in an isolated KUTL_HOME (no real spaces.toml).
    let home = tempfile::TempDir::new().unwrap();

    // `mcp serve` is the agent principal: it loads the tool-held
    // agent key on startup. Provision the default agent keyfile first, exactly
    // as an operator would (`kutl agent create --name default`).
    let create = Command::new(env!("CARGO_BIN_EXE_kutl"))
        .args(["agent", "create", "--name", "default"])
        .env("KUTL_HOME", home.path())
        .output()
        .await
        .expect("failed to run kutl agent create");
    assert!(
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_kutl"))
        ...
        .stderr(Stdio::piped())
        .spawn()
        .expect("failed to start kutl mcp serve");
