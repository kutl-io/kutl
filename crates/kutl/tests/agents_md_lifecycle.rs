...
fn test_watch_refuses_on_stale_incompatible_when_non_tty() {
    ...
    assert!(
        stderr.contains("incompatible") || stderr.contains("kutl update"),
...
fn test_watch_refuses_on_absent_agents_md_when_kutlspace_present_non_tty() {
    ...
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
        stderr.contains("no agent instructions") || stderr.contains("kutl update"),
