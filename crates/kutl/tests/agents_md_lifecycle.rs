...
//! `kutl init` and `kutl space apply`.
...
fn run_update(workdir: &Path) -> std::process::Output {
    let mut cmd = Command::new(kutl_bin());
    cmd.current_dir(workdir)
        .env("KUTL_HOME", workdir)
        .arg("space")
...
fn test_init_writes_agents_md_managed_block_at_repo_root() {
    ...
    let out = run_init(
        dir.path(),
        &["--relay", "ws://localhost:0", "--name", "demo"],
    );
    assert!(
        out.status.success(),
...
fn test_init_update_replaces_stale_block_in_place() {
    ...
    // First, init the space. The space does not yet exist, so init will
    // create it. The relay will be unreachable and we get a local-only
    ...
    // Now run `kutl space apply` against the already-initialized tree. The
    // command detects .kutlspace at the repo root and refreshes
    // AGENTS.md only, then exits.
    let out = run_update(dir.path());
    assert!(
        out.status.success(),
        "kutl space apply failed: stdout={} stderr={}",
        String::from_utf8_lossy(&out.stdout),
        String::from_utf8_lossy(&out.stderr)
    );
...
fn test_join_writes_agents_md_managed_block() {
    ...
    cmd.current_dir(dir.path())
        ...
        .args(["join", "demo", "--relay", "ws://localhost:0"]);
...
fn test_watch_refuses_on_stale_incompatible_when_non_tty() {
    ...
    assert!(
        stderr.contains("incompatible") || stderr.contains("kutl space apply"),
        "expected refusal message; got: {stderr}"
    );
}

#[test]
fn test_watch_warns_and_continues_on_absent_agents_md_when_kutlspace_present_non_tty() {
    // Initialize, then DELETE AGENTS.md. `kutl mcp serve` reads JSON-RPC
    // frames off stdin, never a terminal prompt, so it must not refuse
    // to start over a missing AGENTS.md — it warns and proceeds into
    // setup. Proof of "proceeds": stderr carries the identity-loaded
    // line that only prints once `setup()` is running, well past the
    // AGENTS.md check.
    let dir = tempdir().unwrap();
    make_git_repo(dir.path());
    ...
    let create = Command::new(kutl_bin())
        .current_dir(dir.path())
        .env("KUTL_HOME", dir.path())
        .args(["agent", "create", "--name", "default"])
        .output()
        .expect("spawn agent create");
    assert!(
        create.status.success(),
        "kutl agent create failed: {}",
    ...
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
        stderr.contains("no agent instructions") && stderr.contains("kutl space apply"),
        "expected an absent-AGENTS.md warning; got: {stderr}"
    );
    assert!(
        stderr.contains("kutl mcp: agent identity loaded"),
        "watch should proceed past the AGENTS.md check into setup, not refuse; got: {stderr}"
    );
}

#[test]
fn test_watch_silent_when_no_local_space() {
    // Plain non-git directory with no .kutlspace. kutl mcp serve should
