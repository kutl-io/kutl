...
#[derive(Parser)]
#[command(name = "kutl", about = "kutl — collaborative text sync tool")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
async fn cmd_daemon(action: DaemonAction) -> Result<()> {
    match action {
        ...
        DaemonAction::Status => {
            match daemon_mgmt::stale_pid_check()? {
                Some(pid) => {
                    println!("daemon is running (PID {pid})");
                    let registry = space::SpaceRegistry::load(&space::registry_path()?)?;
                    if registry.spaces.is_empty() {
                        println!("  no spaces registered");
                    } else {
                        for path in &registry.spaces {
                            let space_root = std::path::Path::new(path);
                            let label = match space::SpaceConfig::load(space_root) {
                                Ok(c) => {
                                    let name = kutl_client::KutlspaceConfig::display_name(
                                        space_root,
                                        &c.space_id,
                                    );
                                    format!("{} ({})", name, c.relay_url)
                                }
...
/// Maximum number of token characters shown in status output.
...
async fn cmd_auth_login(token: Option<String>, relay: Option<String>) -> Result<()> {
    if let Some(pat) = token {
        ...
        let relay_url = relay.unwrap_or_else(|| space::DEFAULT_RELAY_URL.to_owned());
        let creds = kutl_client::StoredCredentials {
            token: pat,
            relay_url,
...
/// Print the current authentication status and stored credentials.
fn cmd_auth_status() -> Result<()> {
    // Check env var first.
    if let Ok(token) = std::env::var(kutl_client::credentials::TOKEN_ENV_VAR)
        && !token.is_empty()
    {
        let prefix = &token[..token.len().min(TOKEN_PREFIX_LEN)];
        println!(
            "Authenticated via {} environment variable.",
            kutl_client::credentials::TOKEN_ENV_VAR
        );
    ...
    let path = kutl_client::credentials::default_credentials_path()?;
    if let Some(creds) = kutl_client::StoredCredentials::load(&path)? {
        let prefix = &creds.token[..creds.token.len().min(TOKEN_PREFIX_LEN)];
        if creds.account_id.is_empty() {
            println!(
                "Authenticated via {} (token only, no account info).",
                path.display()
            );
        } else {
            println!("Authenticated via {}.", path.display());
            println!("  account: {}", creds.account_id);
        }
        println!("  token: {prefix}...");
        println!("  relay: {}", creds.relay_url);
        if !creds.display_name.is_empty() {
            println!("  name:  {}", creds.display_name);
        }
    } else {
        println!("Not authenticated.");
        println!("  Run `kutl auth login` to authenticate.");
    }
    Ok(())
}
...
mod tests {
    ...
    mod save_space_config_tests {
        ...
        fn test_cmd_surface_copies_files() {
            ...
            // Check that the file landed in the parent.
            assert_eq!(
                std::fs::read_to_string(parent.path().join("specs/foo.md")).unwrap(),
                "hello"
            );
