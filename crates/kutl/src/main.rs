...
enum McpAction {
    ...
    Serve {
        ...
        /// `$KUTL_HOME/agents/<name>.json` (provision it with
...
async fn cmd_init(
    ...
    // scheme) would persist in `space.json` and only fail opaquely at connect.
...
async fn join_via_owner_slug(space_arg: &str, relay_url: &str, dir: Option<PathBuf>) -> Result<()> {
    ...
    let result = match resp.payload {
        ...
        Some(kutl_proto::sync::sync_envelope::Payload::Error(e)) => {
            // Map the relay's ErrorCode: an auth
...
async fn cmd_join(
    ...
    match join_target {
        JoinTarget::InviteUrl(url) => {
            ...
            // MEMBERSHIP_GRANT relays redeem invites over the
...
impl AsyncRelayConn {
    ...
    async fn connect(relay_url: &str, auth_token: &str) -> Result<Self> {
        ...
        let hs = kutl_proto::protocol::handshake_envelope_with_token(
        ...
        match ack.payload {
            Some(kutl_proto::sync::sync_envelope::Payload::HandshakeAck(ack)) => {
            ...
            Some(kutl_proto::sync::sync_envelope::Payload::Error(e)) => {
                ...
                anyhow::bail!("relay refused the connection: {}", refusal.message);
            }
...
fn resolve_document_target(arg: &std::path::Path) -> Result<DocumentTarget> {
    ...
    let state = kutl_daemon::state::DaemonState::load(&space_root.join(".kutl"));
...
fn cmd_log(path: &std::path::Path, format: OutputFormat) -> Result<()> {
    ...
    let doc = Document::load(&target.dt_path)
...
fn cmd_blame(path: &std::path::Path, format: OutputFormat) -> Result<()> {
    let target = resolve_document_target(path)?;
    let mut doc = Document::load(&target.dt_path)
        .with_context(|| format!("failed to load {}", target.dt_path.display()))?;
...
fn cmd_restore(args: &RestoreArgs) -> Result<()> {
    ...
    atomic_write(&args.path, restored.content.as_bytes())
...
fn reconstruct_restore(args: &RestoreArgs) -> Result<RestorePoint> {
    ...
    let doc = kutl_core::Document::load(&target.dt_path).with_context(|| {
        format!(
...
/// Write `bytes` to `path` atomically: a sibling temp file plus a rename, so a
/// crash mid-write never leaves the working file truncated (the daemon's
/// atomic-save pattern; no new dependency).
fn atomic_write(path: &std::path::Path, bytes: &[u8]) -> std::io::Result<()> {
    ...
    let file_name = path
        ...
        .unwrap_or_default();
    // A unique, hidden sibling so a concurrent restore of the same file never
    // clobbers another's temp; the rename onto `path` is the atomic commit.
    let tmp = dir.join(format!(
        ".{file_name}.kutl-restore.{pid}.tmp",
        pid = std::process::id()
    ));
    // Best-effort cleanup of the temp on any failure after creation.
    let cleanup = || {
...
mod tests {
    ...
    fn test_resolve_space_root_explicit_dir_ignores_kutl_home_ceiling() {
        ...
        let kutl_dir = workspace.join(".kutl");
        std::fs::create_dir_all(&kutl_dir).unwrap();
        std::fs::write(
            kutl_dir.join("space.json"),
            r#"{"space_id":"be18b85f-77fc-424d-8379-acf19e8a1ce6","relay_url":"ws://127.0.0.1:9/ws"}"#,
        )
        .unwrap();
    ...
    mod restore_tests {
        ...
        /// The atomic write replaces the working file's contents in place.
        #[test]
        fn test_atomic_write_replaces_content() {
            let dir = TempDir::new().unwrap();
            let path = dir.path().join("working.md");
            std::fs::write(&path, "current tip content").unwrap();

            atomic_write(&path, b"restored historical content").unwrap();

            assert_eq!(
                std::fs::read_to_string(&path).unwrap(),
                "restored historical content"
            );
            // No temp file left behind in the directory.
            let leftovers: Vec<_> = std::fs::read_dir(dir.path())
                .unwrap()
                .filter_map(std::result::Result::ok)
                .filter(|e| e.file_name().to_string_lossy().contains("kutl-restore"))
                .collect();
            assert!(
                leftovers.is_empty(),
                "temp file should be gone: {leftovers:?}"
            );
        }
