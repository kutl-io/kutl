...
///
/// RFD 0075 §staleness check: validates the new space's AGENTS.md anchor
/// before subscribing. This is per-subscription — a broken AGENTS.md in
/// one space rejects only that subscription, leaving the daemon and all
...
fn handle_subscribe_space(state: &mut WatchState, space: &str) -> ToolCallResult {
    ...
    // Validate the new space's AGENTS.md anchor before subscribing. Error
    // paths return before inserting into `state.subscriptions` — the
    // subscription is not added on failure.
    let anchor = crate::agents_md::anchor_for(&root);
    let stale_warning = match crate::agents_md::check_at_repo_root(&anchor) {
        ...
        Ok(crate::agents_md::CheckOutcome::StaleCompatible { sentinel }) => Some(format!(
            "warning: AGENTS.md kutl block at {} was generated by v{sentinel}; \
             current is v{}; run `kutl init --update` to refresh",
        ...
        Ok(crate::agents_md::CheckOutcome::StaleIncompatible { sentinel }) => {
            return ToolCallResult::error(format!(
                "AGENTS.md kutl block at {} was generated by v{sentinel} which is \
                 incompatible with the running v{}; run `kutl init --update` and \
                 re-attempt subscribe_space",
                anchor.join("AGENTS.md").display(),
                env!("CARGO_PKG_VERSION"),
            ));
        }
        Ok(crate::agents_md::CheckOutcome::Absent) => {
            return ToolCallResult::error(format!(
                "no agent instructions found at {}; run `kutl init --update` and \
                 re-attempt subscribe_space",
                anchor.join("AGENTS.md").display(),
            ));
        }
...
mod tests {
    ...
    async fn test_handle_subscribe_space_rejects_stale_incompatible_anchor() {
        ...
        let space_id = "stale-incompat-space-001";
        ...
        assert!(
            text.contains("kutl init --update"),
            "error message should suggest kutl init --update, got: {text}"
        );
    ...
    #[tokio::test]
    async fn test_handle_subscribe_space_rejects_absent_anchor() {
        // Space directory exists but has no AGENTS.md at all.
        let space_id = "absent-anchor-space-002";
        ...
        assert!(result.is_error, "absent anchor must return an error");
        let text = &result.content[0].text;
        assert!(
            text.contains("no agent instructions"),
            "error message should mention absent instructions, got: {text}"
        );
        assert!(
            text.contains("kutl init --update"),
            "error message should suggest kutl init --update, got: {text}"
    ...
    // StaleCompatible is currently unreachable because MIN_COMPATIBLE_KUTL_VERSION
    // == CARGO_PKG_VERSION (both are the same constant at build time), which means
    // any sentinel older than the running binary is also below the minimum
    // compatible version, collapsing StaleCompatible → StaleIncompatible. This
    // test will become meaningful only after the first incompatible-bump release.
}
