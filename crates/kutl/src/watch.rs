...
use kutl_proto::protocol::{decode_envelope, encode_envelope, subscribe_signals_envelope};
...
/// Initial retry delay for relay proxy reconnection, and for re-subscribing
/// the signal stream while the relay cannot load the space's records.
const PROXY_INITIAL_RETRY: std::time::Duration = std::time::Duration::from_secs(5);

/// Maximum retry delay for both (exponential backoff cap).
...
///
/// Distinct from the human `identity.toml`. The DID drives flag targeting,
...
/// `identity.toml` — and the relay authors signal records under the agent's
/// DID.
...
/// `agent_identity_path(name)` (`$KUTL_HOME/agents/<name>.toml`).
///
...
fn load_agent_identity(name: &str) -> Result<kutl_client::Identity> {
    ...
    kutl_client::Identity::load_if_present(&path)?.with_context(|| {
        format!(
...
/// (matching against either `space_id` from `.kutl/space.toml` or `space_name`
/// from `.kutlspace`).
fn resolve_space_config(space_name_or_id: &str) -> Option<(SpaceConfig, std::path::PathBuf)> {
    let path = kutl_client::space_registry::registry_path()
        .map_err(|e| {
            eprintln!("kutl mcp: failed to resolve the space registry path: {e}");
        })
        .ok()?;
...
/// List all spaces registered in `$KUTL_HOME/spaces.toml`.
...
const NOT_CONNECTED_REMEDY: &str = "not connected to the relay. The relay may be unreachable or \
     ...
     (3) the space is configured correctly in .kutl/space.toml";
...
async fn relay_session(
    ...
    let ws = match kutl_client::connect_and_handshake(
        relay_url,
        "2b6c76d3-124f-428b-82fc-896c484df5fd",
        auth_token,
        "",
    )
    .await
    {
        ...
        Err(kutl_client::HandshakeError::VersionGap(gap)) => {
            return Err(TerminalSessionError(gap).into());
        }
        // A handshake-phase refusal (rejected token, refused protocol) is
        // deterministic: identical on every retry. Read the relay's code,
        // never its prose — an auth refusal names the credential slot to
        // change, and every other refusal already states its own remedy.
        ...
        // No verdict arrived: retryable, like any other transport failure.
    ...
    send_subscribe_signals(&mut ws_sink, space_id)
        .await
...
type SessionSink = futures_util::stream::SplitSink<kutl_client::WsStream, tungstenite::Message>;
/// The WS stream half of a relay session's split socket.
type SessionStream = futures_util::stream::SplitStream<kutl_client::WsStream>;

/// What a stale-stream notice asks of the session, decided from its typed
/// cause alone so the decision is testable apart from the socket.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum StaleStreamAction {
    /// Re-subscribe at once: a pause, or a notice that names no cause, which
    /// is read as the pause the notice meant before it carried one.
    ResubscribeNow,
    /// Re-subscribe after a growing delay: the relay could not read its
    /// records, and the answer would not change at round-trip rate.
    Backoff,
    /// Stop: a refusal is the same answer to every retry for as long as the
    /// authorization stands, and the message is the remedy.
    Terminal,
}

/// The four-way decision behind the session's stale-stream arm.
fn stale_stream_action(cause: sync::StaleStreamReason) -> StaleStreamAction {
    match cause {
        sync::StaleStreamReason::PausedLaneFull | sync::StaleStreamReason::Unspecified => {
            StaleStreamAction::ResubscribeNow
        }
        sync::StaleStreamReason::RecordsUnavailable => StaleStreamAction::Backoff,
        sync::StaleStreamReason::NotAuthorized => StaleStreamAction::Terminal,
    }
}

/// The key a session deduplicates delivered records on: the record id, or
/// the signal id for a frame that carries none.
fn record_key(signal: &sync::Signal) -> String {
    if signal.record_id.is_empty() {
        signal.id.clone()
    } else {
...
/// A connected session's read loop: deliver matching flags, keep the signal
/// stream flowing (a pause is healed in place, a records outage is retried
/// with backoff, a refusal ends the session), and bail on session-fatal
/// frames so the reconnect ladder rebuilds the session.
async fn session_read_loop(
    ...
    // The records-unavailable retry: armed with a deadline by the notice and
    // fired by the select arm below, with the delay that doubles per notice.
    let retry_sleep = tokio::time::sleep(std::time::Duration::ZERO);
    tokio::pin!(retry_sleep);
    let mut retry_armed = false;
    let mut records_retry_delay = PROXY_INITIAL_RETRY;
    // Records this session has delivered or read from a page, so a page that
    // answers a re-subscribe delivers only what arrived while the stream was
    // down; the opening page seeds the set without delivering anything.
    ...
    loop {
        tokio::select! {
            ...
            () = &mut retry_sleep, if retry_armed => {
                retry_armed = false;
                send_subscribe_signals(ws_sink, space_id)
                    .await
                    .context("failed to re-subscribe after the relay's records outage")?;
            }
            msg = ws_stream.next() => {
                ...
                match envelope.payload {
                    ...
                    Some(Payload::Signal(ref signal)) => {
                        seen_records.insert(record_key(signal));
                        deliver_signal_frame(signal, agent_did, space_id, display_name, notify_tx);
                    }
                    // A page answers a subscribe the relay could serve: its
                    // records are readable again, so the next outage backs
                    // off from the floor. The session's opening page is the
                    // space's history, not news; a page earned by a
                    // re-subscribe carries what arrived while the stream was
                    // down, and only what this session has not seen.
                    Some(Payload::SignalPage(ref page)) => {
                        for signal in &page.records {
                            let fresh = seen_records.insert(record_key(signal));
                            if resubscribed && fresh {
                                deliver_signal_frame(signal, agent_did, space_id, display_name, notify_tx);
                            }
                        }
                        records_retry_delay = PROXY_INITIAL_RETRY;
                    }
                    // The relay could not serve this session's signal
                    // stream; the typed cause is the remedy. Ignoring the
                    // notice leaves the session connected but deaf to flags
                    // forever.
                    Some(Payload::StaleSignalStream(ref s)) => match stale_stream_action(s.cause()) {
                        // Heal in place by re-subscribing at once. The page
                        // rides the own-ack lane, so the re-subscribe cannot
                        // earn another pause; the page it earns is delivered.
                        StaleStreamAction::ResubscribeNow => {
                            eprintln!(
                                "kutl mcp: signal stream paused ({}); re-subscribing",
                                s.reason
                            );
                            resubscribed = true;
                            send_subscribe_signals(ws_sink, space_id)
                                .await
                                .context("failed to re-subscribe after signal-stream pause")?;
                        }
                        // The relay's records are unreadable; asked again at
                        // round-trip rate the answer would not change. Back
                        // off, doubling to the cap, until a page lands.
                        StaleStreamAction::Backoff => {
                            eprintln!(
                                "kutl mcp: signal records unavailable ({}); re-subscribing in {}s",
                                s.reason,
                                records_retry_delay.as_secs()
                            );
                            resubscribed = true;
                            retry_sleep
                                .as_mut()
                                .reset(tokio::time::Instant::now() + records_retry_delay);
                            retry_armed = true;
                            records_retry_delay = (records_retry_delay * 2).min(PROXY_MAX_RETRY);
                        }
                        // A refusal is the same answer to every retry for as
                        // long as the authorization stands: terminal, and
                        // the message is the remedy.
                        StaleStreamAction::Terminal => {
                            return Err(TerminalSessionError(format!(
                                "signal subscribe refused for space {space_id}: {}",
                                s.reason
                            ))
                            .into());
                        }
...
/// Send `SubscribeSignals` for `space_id` on the session's socket: the first
/// subscribe of a session and every re-subscribe are the same frame.
async fn send_subscribe_signals(ws_sink: &mut SessionSink, space_id: &str) -> Result<()> {
...
mod tests {
    ...
    /// Path to a non-existent `spaces.toml` inside the given tempdir,
    ...
    fn isolated_registry(dir: &tempfile::TempDir) -> std::path::PathBuf {
        dir.path().join("isolated-spaces.toml")
    }
    ...
    fn test_discover_space_no_walk_up() {
        ...
        // This guards Strategy 1 — Strategy 2 (`spaces.toml` lookup) is
    ...
    fn test_discover_space_not_found() {
        ...
        // — without it, a populated `$KUTL_HOME/spaces.toml` on the host
    ...
    fn register_space_in_tempdir(space_id: &str) -> (tempfile::TempDir, tempfile::TempDir) {
        ...
        // Write the registry.
        let mut registry = kutl_client::SpaceRegistry::default();
        registry.add(space_dir.path().to_str().unwrap());
        registry
            .save(&kutl_client::space_registry::registry_path_in(
                kutl_home.path(),
            ))
            .unwrap();
    ...
    /// `agent_identity_path(name)` (NOT `identity.toml`) and exposes its DID +
    ...
    fn test_load_agent_identity_loads_agent_keyfile_not_human() {
        ...
        // A DISTINCT human identity.toml must NOT be what the agent bootstrap
    ...
    /// The stale-stream decision from the typed cause alone: a pause and a
    /// cause-less notice re-subscribe at once, a records outage backs off,
    /// a refusal is terminal.
    #[test]
    fn test_stale_stream_action_by_cause() {
        use sync::StaleStreamReason as R;
        assert_eq!(
            stale_stream_action(R::PausedLaneFull),
            StaleStreamAction::ResubscribeNow
        );
        assert_eq!(
            stale_stream_action(R::Unspecified),
            StaleStreamAction::ResubscribeNow
        );
        assert_eq!(
            stale_stream_action(R::RecordsUnavailable),
            StaleStreamAction::Backoff
        );
        assert_eq!(
            stale_stream_action(R::NotAuthorized),
            StaleStreamAction::Terminal
        );
    }
}
