...
///
/// Covers both the local DID identity (from `$KUTL_HOME/identity.json`)
/// and the relay-bound bearer credentials (from `$KUTL_HOME/auth.json`
...
pub struct IdentityInfo {
    /// Local `did:key:...` identifier from `$KUTL_HOME/identity.json`.
    /// `None` when no identity file is present.
    pub did: Option<String>,
    /// Human-readable display name. Pulled from `identity.json` if set,
    /// otherwise from the relay-validated `auth.json`. `None` if neither
    /// has a value.
    pub display_name: Option<String>,
    /// Contact email from `identity.json`. `None` if not set.
...
pub struct SpaceInfo {
    ...
    /// the daemon's persisted `.kutl/state.json`; empty when none.
