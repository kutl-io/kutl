...
pub struct ClientStatus {
    ...
    /// Registered spaces (from `$KUTL_HOME/spaces.toml`), with per-space state.
...
/// Identity and authentication state for this client.
///
/// Covers both the local DID identity (from `$KUTL_HOME/identity.toml`)
/// and the relay-bound bearer credentials (from `$KUTL_HOME/auth.toml`
/// or `$KUTL_TOKEN`). Either side may be present without the other —
/// e.g., after `kutl init` / `kutl join` you have a DID but no token
/// until you authenticate.
#[derive(Debug, Serialize)]
#[serde(rename_all = "snake_case")]
pub struct IdentityInfo {
    /// Local `did:key:...` identifier from `$KUTL_HOME/identity.toml`.
    /// `None` when no identity file is present.
    pub did: Option<String>,
    /// Human-readable display name. Pulled from `identity.toml` if set,
    /// otherwise from the relay-validated `auth.toml`. `None` if neither
    /// has a value.
    pub display_name: Option<String>,
    /// Contact email from `identity.toml`. `None` if not set.
    ...
    /// `$KUTL_TOKEN` (no implied relay) or no token is present.
...
pub struct SpaceInfo {
    ...
    /// `true` if the registered path still exists and `.kutl/space.toml` is readable.
    pub healthy: bool,
    ...
    /// Seconds since the last modification of `.kutl/space.toml`. Best-effort
    /// proxy for "last activity"; not authoritative caught-up-ness.
    pub last_activity_seconds: Option<u64>,
    /// Rel paths of text documents frozen at the CRDT op cap
    /// (`MAX_OPS_PER_DOC`): their edits no longer sync anywhere. Read from
    /// the daemon's persisted `.kutl/state.ksnap`; empty when none.
    pub docs_at_op_cap: Vec<String>,
    /// Rel paths of text documents approaching the op cap
    /// (`OP_CAP_WARN_THRESHOLD`, still under it): edits still sync — this is
    /// the window to split or compact before the freeze. Same source and
    /// resolution as `docs_at_op_cap`; the two lists are disjoint.
    pub docs_approaching_op_cap: Vec<String>,
    /// Number of currently-open signals in this space, folded from its local
    /// segments (`.kutl/signals/<space_id>/`). Closed and tombstoned signals
    /// are excluded; `0` when the space has no segments.
    pub open_signals: usize,
    /// The per-space surface target from `.kutlspace` (`surface.target`), or
    /// `None` when the space has no surface configured. Read raw — never
    /// resolved (resolution has a `create_dir_all` side effect).
    pub surface_target: Option<String>,
}
...
impl SpaceInfo {
    ...
    pub(super) fn unhealthy(path_str: &str, reason: &'static str) -> Self {
        Self {
            ...
            docs_at_op_cap: Vec::new(),
            docs_approaching_op_cap: Vec::new(),
            open_signals: 0,
            surface_target: None,
        }
    }
