...
use super::schema::{ClientStatus, IdentityInfo};
...
/// Render the identity section. Header line summarises auth state;
/// indented sub-lines carry the static identity fields (DID, name,
/// email). Subsumes the former `kutl config show`.
fn write_identity(out: &mut String, identity: Option<&IdentityInfo>) {
    let Some(id) = identity else {
        let _ = writeln!(out, "identity: none provisioned");
        return;
    };

    // Header summarises auth state on one line.
    if id.from_env {
    ...
    // Static identity fields below the header, indented.
