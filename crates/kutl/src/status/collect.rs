...
pub fn collect_static(kutl_home_dir: &Path) -> Result<ClientStatus> {
    ...
    // The daemon's liveness is the same answer `daemon stop` acts on: alive
    // AND the recorded process instance, so a recycled PID reads as not
    // running here too. The desktop app records no identity sidecar.
    let daemon = daemon_info(crate::daemon_mgmt::live_daemon_pid_in(kutl_home_dir));
...
fn load_registry(kutl_home_dir: &Path) -> Result<kutl_client::SpaceRegistry> {
    let path = kutl_client::space_registry::registry_path_in(kutl_home_dir);
...
fn build_space_info(path_str: &str) -> SpaceInfo {
    ...
    let last_activity_seconds =
        kutl_client::SpaceConfig::marker(path).and_then(|marker| filesystem_age_seconds(&marker));
    ...
    // snapshot therefore reads as "no frozen docs"; the daemon quarantines and
    // rebuilds it), so this stays a pure-FS read.
    let daemon_state = kutl_daemon::state::DaemonState::load_readonly(&path.join(".kutl"));
...
fn load_identity(kutl_home_dir: &Path) -> Result<Option<IdentityInfo>> {
    // Local DID side (identity.toml). Present after `kutl init` /
    // `kutl join` even before any relay round-trip.
    let local = kutl_client::identity::Identity::load_if_present(
        &kutl_client::identity::default_identity_path_in(kutl_home_dir),
    )?;
        ...
        && !token.is_empty()
    {
        ...
        let path = kutl_client::credentials::default_credentials_path_in(kutl_home_dir);
    ...
    // Display name preference: identity.toml wins (it's what the user
...
fn check_pid_file(path: &Path) -> DaemonInfo {
    daemon_info(
        kutl_client::read_pid_file(path)
            .ok()
            .flatten()
            .filter(|&pid| kutl_client::is_process_alive(pid)),
    )
}

/// The status row for a process that is running at `pid`, or not running.
fn daemon_info(pid: Option<u32>) -> DaemonInfo {
    DaemonInfo {
        running: pid.is_some(),
        pid,
    }
...
mod tests {
    ...
    fn fake_home_with_space() -> (TempDir, std::path::PathBuf) {
        ...
        // Create a space root with a valid space config.
        ...
        // Register that path in the space registry.
        let mut reg = SpaceRegistry::default();
        reg.add(space_root.to_str().unwrap());
        reg.save(&kutl_client::space_registry::registry_path_in(&kutl_home))
            .unwrap();
    ...
    fn test_collect_static_returns_registered_space() {
        ...
        assert!(
            ...
            "freshly written space config should produce an activity stamp"
        );
        let age = space.last_activity_seconds.unwrap();
        assert!(age < 60, "stamp should be recent (got {age}s)");

        // Identity is None when no auth file exists.
