...
/// guidance and gets refreshed by `kutl update`.
...
/// invokes; `ForceUpdate` is what `kutl update` invokes.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ApplyMode {
...
pub fn handle_incompatible(repo_root: &Path, sentinel: &str) -> anyhow::Result<()> {
    ...
    if !std::io::stdin().is_terminal() || !std::io::stderr().is_terminal() {
        anyhow::bail!(
            "{} kutl block was generated by v{sentinel} which is incompatible with the running v{current}; run `kutl update` and re-run this command",
            path.display()
        );
    }
    ...
    if !(answer.is_empty() || answer == "y" || answer == "yes") {
        anyhow::bail!("declined; run `kutl update` when ready and re-run this command");
    }
...
pub fn handle_absent(repo_root: &Path) -> anyhow::Result<()> {
    ...
    if !std::io::stdin().is_terminal() || !std::io::stderr().is_terminal() {
        anyhow::bail!(
            "no agent instructions found at {}; run `kutl update` and re-run this command",
            path.display()
        );
    }
...
mod tests {
    ...
    fn test_render_produces_expected_block_shape() {
        ...
        assert!(out.contains("`kutl update`"));
    }
    ...
    fn test_kfm_spec_matches_relay_embedding() {
        // Smoke test for RFD 0083 §"Markdown dialect — Distribution":
    ...
    fn test_anchor_for_returns_git_root_one_hop_up() {
        // RFD 0060 best-practice layout: kutl folder is a direct child
    ...
    fn test_anchor_for_falls_back_to_start_when_git_is_deep_ancestor() {
        // Workspace nested deep inside another git repo (e.g., the
        // Carol demo case: agent workspace lives inside the kutl repo,
        // many levels below `.git`). With strict-1 walk-up, the
    ...
    fn test_anchor_for_bounded_ceiling_blocks_walk_to_parent() {
        ...
        // ceiling forbids crossing it. Mirrors the Carol demo case
        ...
        // Resolve symlinks before passing as ceiling — `find_git_repo_root_bounded`
        // does a plain `starts_with` so paths must match canonicalization.
