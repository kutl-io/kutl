...
///
/// Composition: the file-sync scope-1 template followed by a separator
/// and the KFM dialect spec. Both sections live inside the
/// `kutl:start`/`kutl:end` marker pair when written into a repo's
/// `AGENTS.md`, so the KFM reference travels alongside the file-sync
/// guidance and gets refreshed by `kutl space apply`.
pub const TEMPLATE: &str = concat!(
    include_str!("agents_md_template.md"),
    "\n\n---\n\n",
...
/// bump this.
///
/// At 0.5.0 for a textbook instance of that condition: every block written
/// before it tells the reader to refresh itself by running `kutl update`,
/// and `kutl update` now only prints a Homebrew upgrade hint. The
/// instruction does not fail — it succeeds and refreshes nothing, so a stale
/// block would otherwise persist indefinitely while appearing maintained.
/// Blocks below this are classified incompatible so the surfaces prompt for
/// `kutl space apply`, which is the command that actually re-renders.
pub const MIN_COMPATIBLE_KUTL_VERSION: &str = "0.5.0";
...
///
/// Priority:
/// 1. The git repository root, when `start_dir` (or its immediate parent)
///    is inside a git working tree. The walk is bounded by
///    [`kutl_client::find_git_repo_root`] semantics: at most one hop up,
///    never crossing above `$KUTL_HOME`.
/// 2. Otherwise, `start_dir` itself — the directory the user chose as the
///    project root when initializing without git, or when the git repo is
///    too deep an ancestor to be considered structurally related. AGENTS.md
///    is written next to the `.kutlspace` file rather than at a git anchor.
///
...
/// Variant of [`anchor_for`] that takes an explicit `$KUTL_HOME` ceiling
/// instead of resolving it from the ambient env. Used by in-process
/// unit tests for deterministic isolation; production callers use
/// [`anchor_for`].
#[cfg(test)]
#[must_use]
fn anchor_for_bounded(start_dir: &Path, ceiling: Option<&Path>) -> PathBuf {
    kutl_client::find_git_repo_root_bounded(start_dir, ceiling)
        .unwrap_or_else(|| start_dir.to_path_buf())
}

/// Behavior selector for `apply_block`. `Default` is what `kutl init`
/// invokes; `ForceUpdate` is what `kutl space apply` invokes.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ApplyMode {
...
pub fn handle_incompatible(repo_root: &Path, sentinel: &str) -> anyhow::Result<()> {
    ...
    if !std::io::stdin().is_terminal() || !std::io::stderr().is_terminal() {
        anyhow::bail!(
            "{} kutl block was generated by v{sentinel} which is incompatible with the running v{current}; run `kutl space apply` and re-run this command",
            path.display()
        );
    }
    ...
    if !(answer.is_empty() || answer == "y" || answer == "yes") {
        anyhow::bail!("declined; run `kutl space apply` when ready and re-run this command");
    }
...
/// Resolve an absent AGENTS.md for `kutl mcp serve`: warn to stderr and
/// continue.
///
/// This command's stdin carries JSON-RPC frames, never a terminal
/// prompt, so missing agent instructions cannot be resolved
/// interactively the way [`handle_incompatible`] resolves a stale
/// block. Absence is a degraded-but-usable state for this caller, not a
/// blocking one: it warns unconditionally and never fails.
pub fn warn_absent(repo_root: &Path) {
    let path = repo_root.join("AGENTS.md");
    eprintln!(
        "warning: no agent instructions at {}; run `kutl space apply` to write them",
        path.display()
    );
}
...
mod tests {
    ...
    fn test_render_produces_expected_block_shape() {
        ...
        assert!(out.contains("`kutl space apply`"));
    }

    #[test]
    fn test_template_contains_the_surface_sentinel() {
        // The markdown template can't invoke the sentinel macro, so it
        // carries a literal copy of the authority string. Pin it here so a
        // change to `kutl_client::surface::SURFACE_SENTINEL_HEADER` fails this test
        // instead of silently leaving the template's copy stale.
        assert!(TEMPLATE.contains(kutl_client::surface::SURFACE_SENTINEL_HEADER));
    }

    #[test]
    fn test_template_includes_kfm_spec() {
        // Composition order: scope-1 file-sync template, separator, KFM
        // dialect spec. Both sections must end up inside the managed
        // block when AGENTS.md is rendered.
        assert!(TEMPLATE.contains("# Working in a kutl-aware repo"));
        assert!(TEMPLATE.contains("# kutl-flavored markdown (KFM)"));
        assert!(TEMPLATE.contains("FLAG_KIND_COMMENT"));
        let file_sync_idx = TEMPLATE
            .find("# Working in a kutl-aware repo")
            .expect("file-sync header present");
        let kfm_idx = TEMPLATE
            .find("# kutl-flavored markdown (KFM)")
            .expect("KFM header present");
        assert!(
            file_sync_idx < kfm_idx,
            "file-sync template must precede KFM section",
        );
    }

    /// The decision grammar the engine tracks (`## ?` / `## =`) must be
    /// taught from the same facts on BOTH agent-facing surfaces: the
    /// managed AGENTS.md block (via its KFM appendix) and the MCP server
    /// instructions. This divergence is not hypothetical — the appendix
    /// once advertised a web-editor decision form the sync engine never
    /// minted, and agents followed the documentation they had.
    #[test]
    fn test_agent_surfaces_share_one_dialect_source() {
        // Every agent-facing surface embeds the SAME KFM file rather than
        // restating the dialect in its own prose. Asserting the shared bytes
        // is what makes divergence impossible; a per-phrase comparison of
        // separate copies would only catch the phrases someone thought to
        // list, which is how the appendix once came to advertise a decision
        // form the engine never minted.
        let kfm: &str = include_str!("../../../docs/kutl-markdown.md");
        for (name, surface) in [
            ("AGENTS.md template", TEMPLATE),
            ("MCP server instructions", crate::watch::SERVER_INSTRUCTIONS),
        ] {
            assert!(
                surface.contains(kfm),
                "{name}: must embed the KFM spec verbatim, not restate it"
            );
        }
        // The web editor's compatibility forms are render sugar, not
        // tracked grammar — the block must never again advertise them as
        // signal-producing.
        assert!(!TEMPLATE.contains("Both forms produce the same `decision` signal"));
        assert!(
            !TEMPLATE.contains("Decision-row pill + coloured callout block"),
            "the rendering matrix must not claim a callout gets decision affordances"
        );
    }

    #[test]
    fn test_kfm_spec_matches_relay_embedding() {
        // Both binaries embed `oss/docs/kutl-markdown.md` via
        // `include_str!`. The CLI's `include_str!` resolves relative
        // to `oss/crates/kutl/src/`; the relay's resolves relative to
        // `oss/crates/kutl-relay/src/`. Both must point at the same
        // file. This assertion catches accidental path divergence
        // (e.g., a future refactor copying the file).
        let cli_kfm: &str = include_str!("../../../docs/kutl-markdown.md");
        assert_eq!(
            cli_kfm,
            kutl_relay::mcp_tools::KFM_SPEC,
            "kutl CLI and kutl-relay must embed identical KFM spec",
        );
        // Sanity: the embedded TEMPLATE used by `render` contains the
        // same KFM bytes as the standalone include.
        assert!(TEMPLATE.contains(cli_kfm));
    }
    ...
    // The `anchor_for_bounded` variant is used below so these unit
    // tests don't pick up ambient `$KUTL_HOME` from the developer's
    // shell, which would otherwise turn the walk into a no-op when the
    // tempdir is outside the inherited ceiling. The env-aware
    // `anchor_for` is exercised end-to-end in `tests/agents_md_init.rs`
    // via subprocess invocation where the env is controlled.

    #[test]
    fn test_anchor_for_returns_git_root_one_hop_up() {
        // Best-practice layout: the kutl folder is a direct child
        // of the project root. anchor_for resolves to the repo root.
        ...
        let actual = std::fs::canonicalize(anchor_for_bounded(&kutl_dir, None)).unwrap();
        assert_eq!(actual, expected);
    }

    #[test]
    fn test_anchor_for_falls_back_to_start_when_git_is_deep_ancestor() {
        // Workspace nested deep inside another git repo (e.g., an
        // agent workspace many levels below an unrelated repo's
        // `.git`). With strict-1 walk-up, the
        // ancestor `.git` is NOT discovered — anchor stays at the
        // workspace. This is the load-bearing invariant: agents
        // never write AGENTS.md into an unrelated parent repo.
        let dir = tempdir().unwrap();
        let out = std::process::Command::new("git")
            .args(["init", "--quiet"])
            .current_dir(dir.path())
            ...
            .expect("git init");
        assert!(out.status.success());
        let workspace = dir.path().join("a/b/c/workspace");
        std::fs::create_dir_all(&workspace).unwrap();
        let actual = std::fs::canonicalize(anchor_for_bounded(&workspace, None)).unwrap();
    ...
    fn test_anchor_for_returns_start_dir_outside_git_repo() {
        ...
        let actual = std::fs::canonicalize(anchor_for_bounded(dir.path(), None)).unwrap();
        let expected = std::fs::canonicalize(dir.path()).unwrap();
        assert_eq!(actual, expected);
    }

    #[test]
    fn test_anchor_for_bounded_ceiling_blocks_walk_to_parent() {
        // KUTL_HOME-style ceiling: even when `.git` is one hop up, the
        // ceiling forbids crossing it. Mirrors an agent workspace
        // where KUTL_HOME is set to the workspace itself.
        let dir = tempdir().unwrap();
        let out = std::process::Command::new("git")
            .args(["init", "--quiet"])
            .current_dir(dir.path())
        ...
        // Canonicalized for the equality assert below (the walk returns
        // canonical paths); the ceiling comparison itself canonicalizes
        // both sides, so a raw path would bound identically.
        let workspace_canon = std::fs::canonicalize(&workspace).unwrap();
        let actual =
            std::fs::canonicalize(anchor_for_bounded(&workspace_canon, Some(&workspace_canon)))
                .unwrap();
        assert_eq!(actual, workspace_canon);
    }
    ...
    #[test]
    fn test_warn_absent_does_not_panic_when_block_is_absent() {
        // `warn_absent` has no `Result` return — its signature already
        // guarantees the `mcp serve` Absent path can't fail. This test
        // is the remaining thing worth proving: it doesn't panic, and
        // that holds regardless of whether the calling process has a
        // terminal attached (`warn_absent` does no stdin/stderr TTY
        // reading), which is the case that matters for `mcp serve`,
        // whose stdin is always a JSON-RPC pipe.
        let dir = tempdir().unwrap();
        warn_absent(dir.path());
    }
