...
/// Monotonic worker-instance identity, scoped per supervisor run.
///
/// A path can be removed and re-added under the same key across reloads, each
/// time getting a fresh worker. The epoch disambiguates which worker instance
/// an exit notification refers to so a stale exit from a torn-down worker
/// cannot evict (or block on) the live worker that replaced it.
type WorkerEpoch = u64;

/// Worker exit notification carrying the worker's path and its epoch.
///
/// The epoch lets the supervisor ignore exits from workers that have already
/// been replaced (see [`WorkerEpoch`]).
struct WorkerExit {
    /// Canonical registry path key of the worker that exited.
    path: String,
    /// Epoch of the specific worker instance that exited.
    epoch: WorkerEpoch,
}
...
struct WorkerHandle {
    ...
    /// Epoch identifying this specific worker instance.
    epoch: WorkerEpoch,
}
...
/// Decide whether an exit notification refers to the worker currently mapped
/// for its path.
///
/// Returns `true` only when a worker is present for the path AND its epoch
/// matches the exiting worker's. A `None` handle (path no longer mapped) or an
/// epoch mismatch (a newer worker replaced the one that exited) means the exit
/// is stale and must be ignored — acting on it would evict or block on the
/// live worker.
fn is_current_worker_exit(handle: Option<&WorkerHandle>, exit_epoch: WorkerEpoch) -> bool {
    handle.is_some_and(|h| h.epoch == exit_epoch)
}
...
fn spawn_worker(
    ...
    let task = tokio::spawn(async move {
        ...
        let result = if let Some(dispatch) = dispatch {
            ...
            kutl_daemon::run(config).with_subscriber(dispatch).await
        } else {
...
async fn reload(
    ...
    for space_path in &registry.spaces {
        ...
        if let Some(config) = build_worker_config(space_path, did, signing_key, display_name) {
            ...
            *next_epoch += 1;
            let handle = spawn_worker(config, exit_tx.clone(), epoch);
...
pub async fn run(
    ...
    // Monotonic epoch handed to each spawned worker; never reset within a run.
    let mut next_epoch: WorkerEpoch = 0;
    ...
    loop {
        tokio::select! {
            _ = sighup.recv() => {
                ...
                reload(&mut workers, &did, signing_key.as_ref(), &display_name, &exit_tx, &mut crash_counts, &mut next_epoch).await;
            }
            _ = sigterm.recv() => {
            ...
            Some(exit) = exit_rx.recv() => {
                let WorkerExit { path, epoch } = exit;

                // Ignore exits from a worker instance that has already been
                // replaced (e.g. removed then re-added under the same path key
                // in a later reload). Acting on a stale exit would evict the
                // live worker and block the select loop awaiting its handle.
                if !is_current_worker_exit(workers.get(&path), epoch) {
                    info!(path = %path, epoch, "ignoring stale worker exit (worker already replaced)");
                    continue;
                }
                ...
                if let Some(handle) = workers.remove(&path) {
                    ...
                    *count += 1;
                    ...
                    if let Some(config) = build_worker_config(&path, &did, signing_key.as_ref(), &display_name) {
                        let epoch = next_epoch;
                        next_epoch += 1;
                        let new_handle = spawn_worker(config, exit_tx.clone(), epoch);
...
mod tests {
    ...
    /// Build a [`WorkerHandle`] wrapping a trivial completed task, for testing
    /// the epoch-matching decision logic without a real space worker.
    fn dummy_handle(epoch: WorkerEpoch) -> WorkerHandle {
        WorkerHandle {
            cancel: CancellationToken::new(),
    ...
    #[tokio::test]
    async fn test_is_current_worker_exit_matching_epoch() {
        // An exit whose epoch matches the mapped worker is current.
        let handle = dummy_handle(7);
        assert!(is_current_worker_exit(Some(&handle), 7));
    }

    #[tokio::test]
    async fn test_is_current_worker_exit_stale_epoch() {
        // An exit from an older worker instance (lower epoch) is stale: a newer
        // worker replaced it under the same path key.
        let handle = dummy_handle(8);
        assert!(
            !is_current_worker_exit(Some(&handle), 3),
            "stale exit from a replaced worker must be ignored"
        );
    }

    #[test]
    fn test_is_current_worker_exit_no_mapped_worker() {
        // No worker mapped for the path: the exit is stale.
        assert!(!is_current_worker_exit(None, 0));
    }
    ...
    async fn test_cancel_stops_worker() {
        ...
        let handle = spawn_worker(worker_config, exit_tx, 0);
