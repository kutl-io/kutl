...
//! keyfile is an ordinary [`Identity`] stored at `$KUTL_HOME/agents/<name>.json`;
...
fn list(format: crate::OutputFormat) -> Result<()> {
    ...
    let mut agents = read_agents(&dir)?;
...
/// Enumerate `<dir>/*.json` agent keyfiles, returning `(name, did)` pairs.
///
...
fn read_agents(dir: &Path) -> Result<Vec<(String, String)>> {
    ...
    for entry in entries {
        ...
        agents.push((name.to_owned(), identity.did));
    }
    Ok(agents)
}
