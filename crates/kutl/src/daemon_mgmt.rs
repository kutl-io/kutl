...
fn id_path() -> Result<PathBuf> {
    Ok(id_path_in(&crate::dirs::kutl_home()?))
}

/// The process-identity sidecar under an explicit kutl home.
fn id_path_in(kutl_home: &Path) -> PathBuf {
    kutl_home.join(PID_ID_FILENAME)
}
...
/// Read the stored process-identity token under `kutl_home`, if any.
fn read_stored_identity_in(kutl_home: &Path) -> Option<String> {
    let content = std::fs::read_to_string(id_path_in(kutl_home)).ok()?;
...
fn verify_daemon_identity_in(kutl_home: &Path, pid: u32) -> bool {
    let stored = read_stored_identity_in(kutl_home);
...
/// Whether the daemon recorded under a kutl home is running, and when it is
/// not, why: `stop` and `reload` tell a dead PID from a recycled one in
/// what they report.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum Liveness {
    /// No PID file, or one whose content is not a PID.
    NoPidFile,
    /// The recorded PID has no live process.
    Dead(u32),
    /// The recorded PID is alive but its identity does not match the
    /// sidecar: the kernel gave the dead daemon's PID to another process.
    Recycled(u32),
    /// The recorded PID is the daemon.
    Live(u32),
}

/// The one liveness answer `status`, `daemon start`, `stop` and `reload`
/// share: the daemon is running only when the PID file's process is alive
/// AND its identity matches the sidecar, so a recycled PID belonging to an
/// unrelated process is never reported as the daemon. Errors only when the
/// PID file exists and cannot be read.
fn daemon_liveness(kutl_home: &Path) -> Result<Liveness> {
    let Some(pid) = read_pid_file(&kutl_home.join(PID_FILENAME))? else {
        return Ok(Liveness::NoPidFile);
    };
    Ok(if !is_process_alive(pid) {
        Liveness::Dead(pid)
    } else if !verify_daemon_identity_in(kutl_home, pid) {
        Liveness::Recycled(pid)
    } else {
        Liveness::Live(pid)
    })
}

/// The daemon's PID when [`daemon_liveness`] says it is running; `None`
/// for every other answer, an unreadable PID file included.
pub fn live_daemon_pid_in(kutl_home: &Path) -> Option<u32> {
    match daemon_liveness(kutl_home) {
        Ok(Liveness::Live(pid)) => Some(pid),
        _ => None,
    }
}
...
pub fn stale_pid_check() -> Result<Option<u32>> {
    match daemon_liveness(&crate::dirs::kutl_home()?)? {
        Liveness::Live(pid) => Ok(Some(pid)),
        Liveness::NoPidFile => Ok(None),
        Liveness::Dead(_) | Liveness::Recycled(_) => {
            remove_pid();
            Ok(None)
        }
    }
...
pub fn stop_daemon() -> Result<String> {
    let pid = match daemon_liveness(&crate::dirs::kutl_home()?)? {
        Liveness::NoPidFile => return Ok("daemon is not running".into()),
        Liveness::Dead(_) => {
            remove_pid();
            return Ok("daemon is not running (stale PID file removed)".into());
        }
        // A recycled PID is an unrelated process: never signal it.
        Liveness::Recycled(pid) => {
            remove_pid();
            return Ok(format!(
...
pub fn signal_reload() -> Result<()> {
    let pid = match daemon_liveness(&crate::dirs::kutl_home()?)? {
        Liveness::NoPidFile | Liveness::Dead(_) => return Ok(()),
        // `signal_reload` fires automatically on every `kutl init`/`join`,
        // so an unverified SIGHUP could hit an unrelated process that
        // inherited the dead daemon's PID.
        Liveness::Recycled(pid) => {
            tracing::warn!(
                pid,
...
mod tests {
    ...
    /// The PID recorded under the fake home, as the daemon's own writes
    /// left it.
    fn read_pid() -> Result<Option<u32>> {
        read_pid_file(&pid_path()?)
    }

    /// Each answer `stop` and `reload` tell apart: no PID file, a dead PID,
    /// a live PID whose sidecar names another process instance, and the
    /// daemon itself.
    #[test]
    #[serial]
    fn test_daemon_liveness_tells_dead_from_recycled() {
        let (_guard, kutl_dir) = fake_home();
        assert_eq!(daemon_liveness(&kutl_dir).unwrap(), Liveness::NoPidFile);

        // A reaped child's PID is free and not instantly reused.
        let mut child = std::process::Command::new("true")
            .spawn()
            .expect("spawn a trivial child");
        let dead = child.id();
        child.wait().expect("reap the child");
        write_pid(dead).unwrap();
        assert_eq!(daemon_liveness(&kutl_dir).unwrap(), Liveness::Dead(dead));

        let me = std::process::id();
        write_pid(me).unwrap();
        assert_eq!(daemon_liveness(&kutl_dir).unwrap(), Liveness::Live(me));
        std::fs::write(id_path_in(&kutl_dir), "starttime=0").unwrap();
        assert_eq!(
            daemon_liveness(&kutl_dir).unwrap(),
            Liveness::Recycled(me),
            "the same live PID with a mismatched identity is a recycled PID"
        );
    }

    /// The liveness answer status reads is the one stop uses: a recorded
    /// PID whose sidecar names a different process instance is not the
    /// daemon, however alive the PID is.
    #[test]
    #[serial]
    fn test_live_daemon_pid_in_rejects_a_recycled_pid() {
        let (_guard, kutl_dir) = fake_home();
        write_pid(std::process::id()).unwrap();
        assert_eq!(
            live_daemon_pid_in(&kutl_dir),
            Some(std::process::id()),
            "this process, recorded with its own identity, is live"
        );
        std::fs::write(id_path_in(&kutl_dir), "starttime=0").unwrap();
        assert_eq!(
            live_daemon_pid_in(&kutl_dir),
    ...
    /// Register `workspace` in the fake home's space registry.
    fn register_space(kutl_dir: &Path, workspace: &Path) {
        let mut reg = kutl_client::SpaceRegistry::default();
        reg.add(workspace.to_str().unwrap());
        reg.save(&kutl_client::space_registry::registry_path_in(kutl_dir))
            .unwrap();
    }
    ...
    fn test_write_pid_records_identity_for_live_process() {
        ...
        let stored = read_stored_identity_in(&kutl_dir);
        let current = process_identity(pid);
        assert_eq!(stored, current);
        // Identity must permit signalling our own live process.
        assert!(verify_daemon_identity_in(&kutl_dir, pid));
    }
    ...
    fn test_verify_identity_blocks_mismatched_sidecar() {
        ...
        if process_identity(pid).is_some() {
            ...
            assert!(
                !verify_daemon_identity_in(&kutl_dir, pid),
