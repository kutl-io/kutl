...
///
/// The token is the process start time, which is stable for the lifetime of a
/// process and differs across PID reuse. Returns `None` when identity cannot be
/// determined (unsupported platform, process gone, or read failure) — callers
/// must treat `None` as "cannot verify" and fall back to liveness-only checks.
#[cfg(target_os = "linux")]
fn process_identity(pid: u32) -> Option<String> {
    // Field 22 (1-indexed) of /proc/<pid>/stat is `starttime` in clock ticks
    // since boot. The comm field (field 2) may contain spaces inside
    // parentheses, so split on the last ')' before tokenizing the rest.
    let stat = std::fs::read_to_string(format!("/proc/{pid}/stat")).ok()?;
    let after_comm = stat.rsplit_once(')')?.1;
    // After ')' the fields are space-separated starting at field 3 (state), so
    // starttime (field 22) is index 19 in this remainder.
    const STARTTIME_INDEX_AFTER_COMM: usize = 19;
    let starttime = after_comm
        .split_whitespace()
        .nth(STARTTIME_INDEX_AFTER_COMM)?;
    Some(format!("starttime={starttime}"))
}

/// Read a stable process-identity token for `pid` (macOS).
///
/// Uses `ps -o lstart= -p <pid>` to read the process start time. Returns `None`
/// when the process is gone or `ps` is unavailable.
#[cfg(target_os = "macos")]
fn process_identity(pid: u32) -> Option<String> {
    ...
    let lstart = String::from_utf8(output.stdout).ok()?;
    let lstart = lstart.trim();
    if lstart.is_empty() {
        return None;
    }
    Some(format!("lstart={lstart}"))
}

/// Read a stable process-identity token for `pid`.
///
/// Unsupported-platform stub — always returns `None`, so identity verification
/// falls back to the liveness-only check (preserving prior behavior).
...
mod tests {
    ...
    #[test]
    fn test_process_identity_is_stable_for_self() {
        // On supported platforms, our own identity is determinable and stable
        // across repeated reads. On unsupported platforms both are None (equal).
