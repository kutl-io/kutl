//! Auth-adaptive join decision (RFD 0086): given the relay's advertised auth
//! model and the local auth state, decide how `kutl join` proceeds — lazily,
//! instead of silently minting a DID before knowing the relay's policy.
...
pub enum JoinAuthAction {
    /// Anonymous-DID relay: ensure a local DID exists (create + announce if
    /// absent, reuse otherwise) and proceed. No account needed.
    UseAnonymousDid,
...
pub fn decide_join_auth(
    ...
    match auth_model {
        ...
        // Anonymous-DID — and the unspecified default, treated permissively for
        // backward compatibility with relays that advertise no policy — use a
        // self-minted DID as the principal.
        AuthModel::AnonymousDid | AuthModel::Unspecified => JoinAuthAction::UseAnonymousDid,
    }
}

/// Map a relay `ResolveSpace`/join error to a user-facing message, applying the
/// RFD 0050 anti-enumeration rule on the client side: an authentication failure
...
/// policy and the HTTP discovery base it was fetched from (RFD 0086). A proxy
/// front door advertises an explicit `relay_endpoint` and the client follows it;
/// a direct relay leaves it empty, so the client falls back to
/// `{discovery_base}/ws` — the current behavior, and the OSS path.
...
/// of the HTTP discovery base that advertised it. Anti-redirect (RFD 0086): the
...
mod tests {
    ...
    #[test]
    fn test_decide_join_auth_anonymous_relay_uses_did() {
        // An anonymous-DID relay needs only a local DID, regardless of token/TTY.
        assert_eq!(
            decide_join_auth(AuthModel::AnonymousDid, false, false),
            JoinAuthAction::UseAnonymousDid
        );
        assert_eq!(
            decide_join_auth(AuthModel::AnonymousDid, true, true),
            JoinAuthAction::UseAnonymousDid
        );
    }
    ...
    fn test_map_join_error_non_auth_message_is_unified() {
        ...
        // RFD 0050 anti-enumeration rule. What makes that safe is the RELAY
