...
fn folded_spaces() -> Result<Vec<FoldedSpace>> {
    ...
    for path_str in &registry.spaces {
        ...
        if kutl_client::SpaceConfig::load(&space_root).is_err() {
            continue; // A stale / unreadable registry entry is skipped, not fatal.
        }
        out.push(folded_space_at(&space_root)?);
    }
    Ok(out)
}
...
async fn fetch_one_space(
    ...
    // returned cursor; the cursor round-trips through the relay's subscriber
    // entry.
    let mut client =
        kutl_client::SyncClient::connect_with_auth(relay_url, CLI_CLIENT_NAME, token, "")
            .await
...
/// the daemon's local state map (`<space_root>/.kutl/state.json`). The map is
...
mod tests {
    ...
    /// state.json and returns the document uuid. An untracked path errors.
    #[test]
    fn test_resolve_document_id_finds_tracked_and_rejects_untracked() {
        ...
        // Write a state.json with one tracked document.
        ...
        std::fs::write(kutl_dir.join("state.json"), json).unwrap();
