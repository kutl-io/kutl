...
//! challenge-response using the identity at `$KUTL_HOME/identity.toml`
...
//!
//! `kutl-stress mint-identity` prints the identity file (TOML) to stdout (creating
...
enum Mode {
    ...
    Run,
    /// Print the local identity file (TOML) to stdout, minting it if missing.
    MintIdentity,
}
    ...
    match args {
        ...
        other => bail!(
            "unknown arguments {other:?}\n\
             ...
             \x20              connects authenticated as $KUTL_HOME/identity.toml\n\
             \x20              (enroll that DID as a line in the relay's\n\
             \x20              authorized_keys file — it live-reloads)\n\
             mint-identity  print the identity file (TOML) to stdout, minting it first\n\
...
async fn main() -> Result<()> {
    ...
    if parse_mode(&args)? == Mode::MintIdentity {
        ...
        print!("{}", kutl_client::text_file::to_toml(&identity)?);
        return Ok(());
    }
