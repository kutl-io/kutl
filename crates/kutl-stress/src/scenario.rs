...
///
/// `identity` is the shared did:key identity every stress client
/// authenticates with (per-connect challenge-response).
pub async fn run(config: &StressConfig, identity: &Identity) -> Result<Report> {
    match config.scenario.as_str() {
        "sustained" => sustained(config, identity).await,
        "burst" => burst(config, identity).await,
        "large_doc" => large_doc(config, identity).await,
        "many_docs" => many_docs(config, identity).await,
        "slow_client" => slow_client(config, identity).await,
        "pause_read" => pause_read(config, identity).await,
        "throttle_read" => throttle_read(config, identity).await,
        "churner" => churner(config, identity).await,
        "chaos" => chaos(config, identity).await,
...
async fn connect_all(config: &StressConfig, identity: &Identity) -> Result<Vec<StressClient>> {
...
async fn sustained(config: &StressConfig, identity: &Identity) -> Result<Report> {
    info!(
    ...
    let mut clients = connect_all(config, identity).await?;
...
async fn burst(config: &StressConfig, identity: &Identity) -> Result<Report> {
    info!(
        clients = config.client_count,
        ops = config.ops_per_client,
...
async fn large_doc(config: &StressConfig, identity: &Identity) -> Result<Report> {
    info!(
        clients = config.client_count,
        ops = config.ops_per_client,
...
async fn many_docs(config: &StressConfig, identity: &Identity) -> Result<Report> {
    info!(
        clients = config.client_count,
        ops = config.ops_per_client,
...
async fn slow_client(config: &StressConfig, identity: &Identity) -> Result<Report> {
    info!(
        clients = config.client_count,
        ops = config.ops_per_client,
...
async fn pause_read(config: &StressConfig, identity: &Identity) -> Result<Report> {
    let degraded_count = config.degraded_count.min(config.client_count);
...
async fn throttle_read(config: &StressConfig, identity: &Identity) -> Result<Report> {
    let degraded_count = config.degraded_count.min(config.client_count);
...
#[allow(clippy::too_many_lines)]
async fn churner(config: &StressConfig, identity: &Identity) -> Result<Report> {
...
#[allow(clippy::too_many_lines)]
async fn chaos(config: &StressConfig, identity: &Identity) -> Result<Report> {
    ...
    for op_idx in 0..config.ops_per_client {
        ...
        for i in 0..clients.len() {
            ...
            if result.is_err() {
                ...
                // Reconnect with a fresh client. This re-runs the full
                // challenge-response: the restarted relay's in-memory token
                // store is empty, so the old bearer would be rejected.
