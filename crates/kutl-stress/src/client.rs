...
//! Every connect authenticates via the did:key challenge-response
//! (the relay rejects tokenless handshakes).

use std::time::Duration;

use anyhow::Result;
use kutl_client::{Identity, SyncClient, SyncEvent};
...
impl StressClient {
    /// Connect, authenticate, handshake, and subscribe. Returns a
    /// ready-to-use client.
    ///
    /// Authentication is per-connect (not once at startup): the chaos
    /// scenario kills the relay pod and the in-memory token store dies with
    /// it, so every reconnect must re-mint its bearer via the
    /// challenge-response flow. Two extra HTTP round-trips are noise next
    /// to a WS stress session.
    pub async fn connect(
        ...
        let token =
            kutl_client::authenticate(relay_url, &identity.did, &identity.decode_signing_key()?)
                .await?;
        let mut inner =
            SyncClient::connect_with_auth(relay_url, client_name, &token, client_name).await?;
