...
/// Convert a `u64` millisecond value to `i64`, saturating to [`i64::MAX`].
///
/// Lattice/HLC stamps carry `physical_ms` as `u64`; the proto/display
/// `timestamp` fields are `i64`. This is the one place that conversion is
/// spelled. Saturation is lossless for any real wall-clock millis
/// (`i64::MAX` ms is ~292 million years past the epoch).
pub fn ms_u64_to_i64_saturating(ms: u64) -> i64 {
...
mod tests {
    ...
    #[test]
    fn test_ms_u64_to_i64_saturating() {
        assert_eq!(ms_u64_to_i64_saturating(0), 0);
        assert_eq!(
            ms_u64_to_i64_saturating(1_700_000_000_000),
            1_700_000_000_000
        );
        // i64::MAX round-trips; anything above saturates rather than wrapping.
        let i64_max_as_u64 = u64::try_from(i64::MAX).unwrap();
        assert_eq!(ms_u64_to_i64_saturating(i64_max_as_u64), i64::MAX);
        assert_eq!(ms_u64_to_i64_saturating(u64::MAX), i64::MAX);
    }
}
