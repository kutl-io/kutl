...
pub use kutl_proto::sync::{
    Boundary, ChangeList, ChangeMetadata as Change, RemoteTip, VersionSpan,
};
...
/// Extract the version span END as `Vec<usize>` (the frontier immediately
/// AFTER this change was applied — the correct cut for restoring to "the
/// document as the user saw it at this entry"). Empty vec if the span is
/// missing.
#[allow(clippy::cast_possible_truncation)] // diamond-types versions fit in usize
#[must_use]
pub fn span_end(change: &Change) -> Vec<usize> {
    change
        .version_span
        .as_ref()
        .map(|s| s.end.iter().map(|&v| v as usize).collect())
        .unwrap_or_default()
}
...
mod tests {
    ...
    fn test_change_list_roundtrip() {
        ...
        let list = ChangeList {
            ...
            author_by_agent: std::collections::HashMap::new(),
        };
