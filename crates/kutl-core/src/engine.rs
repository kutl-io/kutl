...
use diamond_types::list::operation::ListOpKind as OpKind;
use diamond_types::list::{ListBranch as Branch, ListOpLog as OpLog};
...
///
/// `Engine` is `Send` but not `Sync`: the branch buffers local edits behind
/// a `RefCell`. A host that drives it from a spawned task must not hold a
/// shared borrow of it across an await; the relay and daemon actors take
/// `&mut self` in every awaiting handler for exactly this reason.
#[derive(Debug, Clone)]
pub struct Engine {
    oplog: OpLog,
    branch: Branch,
...
/// Materialise `branch`'s content with one right-sized allocation.
///
/// The branch hands out its buffered rope, whose `Display`-based `to_string`
/// grows the `String` by doubling. The plain rope behind the buffer's guard
/// reserves `len_bytes` up front, so read through the guard and call the
/// plain rope's own method.
fn branch_text(branch: &Branch) -> String {
    (*branch.content().borrow()).to_string()
}
...
impl Engine {
    ...
    pub fn content(&self) -> String {
        branch_text(&self.branch)
    }

    /// Whether the current content equals `other`. The rope compares itself
    /// (a byte-length gate, then chunk by chunk), so no O(doc-size) `String`
    /// is materialised: the hot remote-text path asks this on every inbound
    /// op, and building the full content for a boolean was pure allocation.
    #[must_use]
    pub fn content_eq(&self, other: &str) -> bool {
        self.branch.content() == other
    }
    ...
    pub fn load_from(bytes: &[u8]) -> Result<Self> {
        // `.map`, not `?`: the closure's error type is then exactly what
        // `load_from` returns, which `contain_panic` boxes. A `?` would make
        // it inferred, and the decoder's parse error has several `From` impls,
        // so inference is ambiguous.
        contain_panic(|| {
            OpLog::load_from(bytes).map(|oplog| {
                let branch = Branch::new_at_tip(&oplog);
                Self { oplog, branch }
            })
        })
    ...
    pub fn checkout(&self, version: &[usize]) -> String {
        branch_text(&self.oplog.checkout(version))
    }
    ...
    pub fn blame_agents(&self) -> Vec<(String, usize)> {
        ...
        positions
            .into_iter()
            .map(|time| {
                let rid = self.oplog.cg.agent_assignment.local_to_remote_version(time);
                (rid.0.to_string(), rid.1)
            })
            .collect()
    }
    ...
    pub fn remote_tip(&self, version: &[usize]) -> Option<(String, u64)> {
        match version {
            &[t] => {
                let rid = self.oplog.cg.agent_assignment.local_to_remote_version(t);
                Some((rid.0.to_string(), rid.1 as u64))
            }
            _ => None,
        }
    }
    ...
    /// `try_remote_to_local_version`, so an unknown id yields `None` rather than a
    ...
    pub fn local_frontier_for(&self, agent: &str, seq: u64) -> Option<usize> {
        self.oplog
            .cg
            .agent_assignment
            .try_remote_to_local_version(RemoteVersion(agent, seq as usize))
            .ok()
    }
    ...
    pub fn merge_bytes(&mut self, bytes: &[u8]) -> Result<()> {
        ...
        contain_panic(|| {
            // `.map`, not `?`: pins the closure's error type — see `load_from`.
            self.oplog.decode_and_add(bytes).map(|_frontier| {
                self.branch
                    .merge(&self.oplog, self.oplog.cg.version.as_ref());
            })
        })
    }
}
...
/// Apply bytes received from a peer with the engine's panics contained.
///
/// `apply` is the decode of the bytes plus the branch merge that
/// materialises the decoded ops (or, on load, the branch built at the
/// tip); a crafted or corrupt blob can trip an assertion in either. Unwinding
/// out of here would take the caller's task with it, so a panic becomes
/// [`Error::EnginePanicked`] instead. `AssertUnwindSafe` is honest because
/// that variant's contract is that the engine is discarded: the closure may
/// have left the operation log half-applied.
///
/// This is also the one site that boxes the decoder's parse error, which
/// diamond-types does not export by name.
fn contain_panic<T, E>(apply: impl FnOnce() -> std::result::Result<T, E>) -> Result<T>
where
    E: std::error::Error + Send + Sync + 'static,
{
    match std::panic::catch_unwind(std::panic::AssertUnwindSafe(apply)) {
        Ok(Ok(value)) => Ok(value),
        Ok(Err(e)) => Err(Error::Decode(Box::new(e))),
        Err(payload) => Err(Error::EnginePanicked {
            message: crate::panic_payload_message(payload.as_ref()),
        }),
    }
}
...
mod tests {
    ...
    /// Enough text to span several rope chunks, so a `String` grown by
    /// doubling would end up with slack capacity.
    const MULTI_CHUNK_TEXT_LEN: usize = 20_000;

    /// `content` and `checkout` must materialise through the plain rope's
    /// size-hinted `to_string`, not the buffered rope's `Display`: the
    /// former allocates exactly once, so capacity equals length.
    #[test]
    fn test_content_and_checkout_allocate_right_sized() {
        let mut engine = Engine::new();
        let agent = engine.get_or_create_agent("test");
        let text: String = "kutl ".repeat(MULTI_CHUNK_TEXT_LEN / 5);
        engine.insert(agent, 0, &text).unwrap();
        let content = engine.content();
        assert_eq!(content, text);
        assert_eq!(content.capacity(), content.len());
        let at_tip = engine.checkout(&engine.local_version());
        assert_eq!(at_tip, text);
        assert_eq!(at_tip.capacity(), at_tip.len());
    }
    ...
    #[derive(Debug)]
    struct FakeParseError;
    impl std::fmt::Display for FakeParseError {
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
            f.write_str("invalid magic")
        }
    }
    impl std::error::Error for FakeParseError {}

    #[test]
    fn test_contain_panic_passes_a_value_through() {
        let value = contain_panic(|| Ok::<_, FakeParseError>(7)).unwrap();
        assert_eq!(value, 7);
    }

    #[test]
    fn test_contain_panic_boxes_the_decoder_error() {
        let err = contain_panic(|| Err::<(), _>(FakeParseError)).unwrap_err();
        assert!(matches!(err, Error::Decode(_)), "got {err:?}");
        assert_eq!(err.to_string(), "decode error: invalid magic");
    }

    #[test]
    fn test_contain_panic_turns_a_panic_into_engine_panicked() {
        let err = contain_panic(|| -> std::result::Result<(), FakeParseError> {
            panic!(
        ...
        .unwrap_err();
        let Error::EnginePanicked { message } = err else {
            panic!("expected EnginePanicked, got {err:?}");
        };
        assert_eq!(
            message,
    ...
    /// Garbage bytes fail as a decode error, not an engine panic: the guard
    /// must not reclassify ordinary rejections.
    #[test]
    fn test_merge_bytes_rejects_garbage_as_a_decode_error() {
        let mut engine = Engine::new();
        let err = engine.merge_bytes(b"not a diamond-types blob").unwrap_err();
        assert!(matches!(err, Error::Decode(_)), "got {err:?}");
        assert!(Engine::load_from(b"nope").is_err());
    }
}
