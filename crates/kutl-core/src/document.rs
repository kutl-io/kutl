...
impl Document {
    ...
    pub fn load_with_env(path: &Path, env: SharedEnv) -> Result<Self> {
        ...
        let sidecar = path.with_extension(sidecar_extension(path));
        let (changes, author_by_agent) = match std::fs::read(&sidecar) {
            Ok(bytes) => {
...
/// [`Engine::blame_agents`](crate::engine::Engine::blame_agents) — for a line
/// with at least one character, or `None` for an empty line (which has no
/// character to attribute). The text excludes the terminating `'\n'`.
///
...
fn segment_blame_lines(content: &str) -> Vec<(usize, Option<usize>, String)> {
    ...
    for ch in content.chars() {
        if ch == '\n' {
            ...
            // newline (at `char_index` right here) — so it attributes to
            // whoever inserted that newline rather than to nobody.
            let first = if line_start < char_index {
                Some(line_start)
            } else {
                Some(char_index)
            };
...
mod tests {
    ...
    fn test_blame_backfills_historical_bindings_from_change_log() {
        ...
        let mut list = <crate::ChangeList as prost::Message>::decode(&bytes[..]).unwrap();
