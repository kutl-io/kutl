...
/// Size-change (chars added or removed) beyond which the bulk path is forced,
/// independent of `SimHash`.
///
/// `TextDiff::from_chars` is O(N·D) in the edit distance D, and the size
/// disparity `|old_len − new_len|` is a lower bound on D — so a large disparity
/// guarantees an expensive diff. `SimHash` cannot catch this: deleting most of a
/// document barely changes its trigram distribution (a 147KB doc shrunk to a
/// 2KB index measured a distance of 9, far under [`BULK_HAMMING_THRESHOLD`]),
/// yet the char diff there is ~145K ops / minutes. This trades op-minimality on
/// large add/deletes for a bounded diff cost. Tunable.
const BULK_DISPARITY_LEN: usize = 2048;
...
fn texts_are_very_different(old: &str, new: &str, old_len: usize, new_len: usize) -> bool {
    // A large size disparity is a large edit distance → the O(N·D) char diff is
    // expensive. Check this FIRST, before the short-doc fast-path: SimHash misses
    // huge add/deletes (deleting most of a doc barely shifts its trigram
    ...
    if old_len.abs_diff(new_len) > BULK_DISPARITY_LEN {
        return true;
    }
...
mod tests {
    ...
    fn test_texts_are_very_different_flags_large_size_disparity() {
        ...
        // through to the slow path. Regression for the 221s scan freeze on
        // parking-lot.md (147KB → 1.9KB, real SimHash distance was 9). The guard
    ...
    fn test_merge_rejects_when_already_at_cap() {
        ...
        /// test fast: `MAX_OPS_PER_DOC / FILL_BLOCK` `edit()` calls instead of
        /// one single-char call per op.
