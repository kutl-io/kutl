...
/// Persist the high-water mark atomically, through `kutl_core::fs::write_atomic`.
///
...
pub fn save_cursor(dir: &Path, hlc: &kutl_proto::sync::Hlc) -> Result<()> {
    ...
    kutl_core::fs::write_atomic(&dest, &serde_json::to_vec(&c)?).map_err(|e| {
        Error::CursorWrite {
            path: dest,
            source: e,
        }
...
mod tests {
    ...
    fn test_save_cursor_no_partial_file_after_success() {
        ...
        assert!(
            leftovers.is_empty(),
            "no scratch file survives a save: {leftovers:?}"
        );
