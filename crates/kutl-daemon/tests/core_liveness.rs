...
//! `DaemonSim`.
//!
//! This is the proof that the daemon's event loop is deadlock-free
...
//! remote registers — the `cp -R` bulk-copy repro. A local create is
...
/// `GuardedPlace(Register)` for each (the untracked fork of the former
/// `materialize_at`), the sim tracks + subscribes both, and the system reaches
...
/// A bulk seed through cap-64 channels. The bulk-load deadlock repro,
/// held as sim state. On the reserve-before-commit (shipped) driver the sim
...
/// A brand-new doc onto an untracked-occupied path
...
/// Mirrors the core's defer-unless-revival-exempt split (`state.exempt_revival`,
/// consumed in `core/reconcile.rs`).
#[test]
...
/// A previously-seen doc revives onto its own
/// orphaned path; it places (exempt), is not deferred. The single-doc analogue
/// of the exemption test's revival half, driven to full quiescence.
#[test]
...
/// A deferred doc gets `RemoteUnregister`, THEN the occupant vacates.
/// Because unregister removes it from `known_records` (tombstone), the next
/// reconcile simply doesn't consider it — no resurrection (the protection
...
fn test_unregistered_deferred_doc_is_not_resurrected() {
    ...
    // THE STRUCTURAL PROTECTION: unregister tombstones the doc in
...
/// A local untracked create (event still in
...
/// Inject a local create between the core's decision and the
/// shell's mutation; `GuardedPlace`'s atomic stat catches it → defer (conflict-
/// copy on the relay's next move), NOT clobber. Same mechanism as the
/// debounce-race test but
...
/// Drive a startup register BEFORE any file
/// scan, with an offline-created untracked file already on disk. The identity
/// maps are empty until `initial_file_scan` (`daemon/startup.rs`) adopts the
/// files, so only the live stat can see the file: the remote doc defers, the
/// scan mints a fresh uuid, both survive.
#[test]
fn test_fs2_startup_offline_create_defers_remote() {
    ...
    // Startup: the identity maps are EMPTY (nothing before `initial_file_scan`
    // populates them), so the offline file is unknown to the core.
    // SpaceDocuments lists the remote doc at
    ...
    // (a deferred register emits no `GuardedPlace`, so it never reaches the
    // `PlaceKind::Register` placement or `register_identity`). Both documents
    // now exist; the collision conflict-copies.
...
/// The shell FAILS a rename (ENOSPC). The shadow must be
...
/// log-and-swallow self-heal (`apply_rename` in `daemon/effects.rs`: a failed
/// rename is logged and yields no `EffectResult`, so the shadow is untouched).
#[test]
fn test_failed_rename_self_heals() {
    ...
    // NOTHING retries while the space is idle — the sim models the real
    // driver faithfully (a failed fs effect schedules no retry; an
    // auto-retry park in the sim would fake a driver behavior the real
...
/// ENOSPC arm: the shell FAILS the conflict-copy
/// DISPLACEMENT write (the loser's `GuardedPlace(Rename)` to its conflict
/// path). The failure leaves the shadow unchanged so a later reconcile
/// re-derives and retries — and the PERIODIC TICK is the guaranteed retry
/// driver. Without it an IDLE space stays diverged forever after a
/// swallowed write: the sim models the real driver faithfully (a failed
/// fs effect schedules NOTHING — an auto-retry park in the sim would fake
/// a driver behavior the real daemon does not have, hiding exactly this gap).
#[test]
...
/// A gamma-computed rename registers the SAME pair-matched suppression the
/// funnel does (the removal half at `old` and the write half at `new`
/// carrying the renamed hash, linked as one pair). gamma emits the rename
/// as a `GuardedPlace(Rename)`; the shell applying it registers the pair
/// into the real `SuppressionSet`, so the pairing is a property of the
/// shared funnel, not a gamma-private choice.
///
...
///   from any write echo), so the file is tracked — no silent strand.
#[test]
fn test_rename_effect_suppression_pairing() {
    ...
    sim.run_one_handle();
    // The shell applied the rename through the funnel: one pair-matched
    // suppression, the removal half at `from.md` and the write half at
    // `to.md` carrying the renamed content hash. The two feeds below are
    // what that registration must and must not swallow.
    ...
    // The registration was ONE pair, not two single entries: the pair-matched
    // consumer fires for exactly (from.md, to.md) while a half is still
    // registered, and a second call finds both halves cleared.
    assert!(
        sim.consume_rename_pair_echo(Path::new("from.md"), Path::new("to.md")),
        "the rename registered a pair-matched suppression for (from.md, to.md)"
    );
    assert!(
        !sim.consume_rename_pair_echo(Path::new("from.md"), Path::new("to.md")),
        "consuming the pair clears both halves"
    );
}

/// A doc deferred behind an untracked occupant eventually
...
/// (`hlc_of` in `client.rs`): the lattice elects a doc's path from the latest of
...
/// Conform vs. concurrent relocation: an out-of-band local `mv`
/// relocated the file; the remote rename is dequeued first. The daemon shadow is
/// BLIND to the user rename (`relocate_out_of_band` does not advance it). The
...
/// orphan, and BOTH renames observed by the relay — a fast deterministic sim
/// guard for a lost-rename divergence otherwise reachable only at e2e scale.
#[test]
fn test_fs12_conform_vs_concurrent_relocation() {
    ...
    // every interleaving — deterministic by construction.
    sim.feed(Event::RemoteRename {
        document_id: doc.into(),
        old_path: "old.md".into(),
    ...
    sim.drain_to_quiescence_or_fail(10_000);
    // The drained local rename reached the relay (no silent drop), even
...
/// A DELETE must vacate the shadow occupant, so a foreign recreate at
...
/// occupant makes the place's stat report "not untracked" (the
/// untracked-occupant guard is
...
fn test_sec42_recreate_after_delete_defers_remote_register() {
    ...
    // The LOCAL delete (the user's `rm`, observed by the watcher):
    // the deleter's own shadow must vacate the occupant. The
...
/// Loser direction of the concurrent-rename drain: the incoming remote
/// rename's origin stamp is OLDER than
...
/// drain edge this scenario silently loses one of the two concurrent renames: the
/// conform relocates the file first and the local rename never reaches the relay,
...
/// The third-observer floored-rename drop:
/// this daemon PLACED a doc whose remote register carried a
...
/// `Occupant::Untracked` marker (the correct half). When the local
...
/// A local create's `Subscribe` rides its place ACK (`place_now` in
/// `daemon/effects.rs`)
/// — without that subscribe the winning doc's edit stream is never
/// delivered and the
/// workers diverge silently (same file keys, different `dup.md` content).
#[test]
fn test_create_create_register_race_subscribes_minted_doc() {
    ...
    // (this daemon's register goes out after the peer's arrived, and wins).
