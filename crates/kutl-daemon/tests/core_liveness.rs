...
/// Mirrors the `defer_if_occupied(exempt_revival)` split (`daemon.rs:505`).
#[test]
...
/// A gamma-computed rename emits the
/// SAME two suppressions in the SAME order the funnel does (removal-half
/// `(old, None)` FIRST, write-half `(new, Some(hash))` SECOND — the pairing
/// now lives in the `apply_rename` funnel). gamma emits the rename as a
