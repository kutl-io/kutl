...
/// eviction (RFD bug 3 / backpressure recovery).
...
/// encodes; tests write/read sidecars directly now that `DocumentManager` is
/// gone).
...
// ---------------------------------------------------------------------------
// Cycle C regression: outbound flood must not starve inbound ack reads
// ---------------------------------------------------------------------------

/// Regression guard for the single-`select!` Cycle C deadlock: a client whose
...
///
/// RED against the un-split `relay_loop` (write arm starves the read arm);
/// GREEN after Task 3 splits send/recv onto independent tasks.
///
/// NOTE: with an in-process relay whose inbound channel capacity is 256 the
/// flood may drain faster than it saturates the socket, so this test may not
/// reliably go red pre-split. It serves as the GREEN regression guard after
/// the split lands; Task 3's dedicated unit test wedges the sink
/// deterministically for the authoritative RED proof.
#[tokio::test]
async fn test_outbound_flood_does_not_starve_inbound_acks() {
    ...
    .await
    .expect("flooder did not receive inbound remote op while flooding (Cycle C starvation)");
    assert!(
        got,
...
// ---------------------------------------------------------------------------
// Bug 3 regression: backpressure eviction must be recoverable
// ---------------------------------------------------------------------------
...
/// the broadcasts it missed while behind. Before the fix, `handle_inbound`'s
/// `_ => {}` catch-all silently dropped the notice, stranding the document.
///
/// RED pre-fix: the victim never observes a `StaleSubscriber` event (it is
/// dropped), so the recv times out. GREEN post-fix: the victim observes the
/// eviction, re-subscribes, and the relay's catch-up replays the FULL current
/// document state — proving every op missed during the stall is recovered.
///
/// This exercises the real production path end-to-end: the relay's eviction,
/// `client.rs::handle_inbound`'s new `StaleSubscriber` arm, and the recovery via
...
async fn test_stale_subscriber_eviction_is_recoverable() {
    ...
    // `SyncEvent::StaleSubscriber` for this document. Pre-fix this is dropped and
    // the recv loop times out.
    ...
    .await
    .expect("victim never observed a StaleSubscriber eviction (notice dropped — bug 3)");
...
/// client and the real relay. Returns the proxy's ws:// URL and the proxy
/// task's `JoinHandle`. Aborting the handle drops both sides of the proxy TCP
/// stream, which causes the client's `ws_stream.next()` to return an error and
/// drives `read_task` → session cancel → `write_task` exit → `run_client`
/// returns. This is the real half-open-socket scenario: the relay vanishes
/// without sending a close frame.
async fn start_relay_abortable() -> (String, tokio::task::JoinHandle<()>) {
    ...
    (format!("ws://{proxy_addr}/ws"), proxy_task)
}
...
/// leaked.
///
/// This is a GREEN regression guard: the teardown wiring landed in Task 3
/// (split `relay_loop`). If someone reverts the cancellation coupling between
/// the two halves this test will time out on the `client_handle` join.
#[tokio::test]
async fn test_half_open_socket_tears_down_both_halves() {
    let (relay_url, proxy_task) = start_relay_abortable().await;

    let (cmd_tx, cmd_rx) = mpsc::unbounded_channel::<SyncCommand>();
    let (event_tx, mut event_rx) = mpsc::channel::<SyncEvent>(TEST_CHANNEL_CAPACITY);

    // Run the real client and keep its JoinHandle to prove it finishes.
...
/// the timeout.
///
/// Reproduces the dogfooding `cp -R` of 226 files into a daemon-watched space:
...
/// well past the old bounded `CHANNEL_CAPACITY` (64) so the burst overruns the
/// `sync_cmd` channel — the condition under which a BLOCKING bounded outbound
/// send (the pre-fix `PollSender::reserve().await` from inside the loop) parks
/// the loop while it should be draining inbound, the architectural defect the
/// unbounded-outbound + intake-gate fix removes.
///
/// Regression guard, NOT a red/green proof of the deadlock on this branch: the
/// prior Phase-1 WS read/write split already closed the sender-side cycle here,
...
/// well under a second). See
/// `commercial/docs/team/design/2026-06-08-daemon-sync-loop-deadlock-findings.md`.
#[tokio::test]
async fn test_bulk_seed_many_documents_does_not_deadlock() {
    /// Number of docs to drop on disk at once; deliberately well past the old
    /// bounded `CHANNEL_CAPACITY` of 64 so the registration burst overruns it.
    const N: usize = 200;

    let relay_url = start_relay().await;
    let (_tmp, space_root) = canonical_tempdir();
    let space_id = "bulk-seed-space";
    ...
    let handle = tokio::time::timeout(PIPELINE_TIMEOUT, spawn_ready_daemon(config))
        .await
...
/// deadlock repro, reinstated as an integration check on the WIRED gamma path).
///
/// With gamma as the live placement
...
/// doc. A timeout here is a stall in the wired gamma path (a regression vs the
/// procedural default), not slowness — a healthy daemon does N tiny docs in well
/// under a second.
#[tokio::test]
async fn test_gamma_bulk_register_confirms_all() {
    /// Number of docs to drop at once; well past the old bounded
    /// `CHANNEL_CAPACITY` (64) so the registration burst overruns it.
    const N: usize = 100;

    let relay_url = start_relay().await;
    let (_dir, root) = canonical_tempdir();
    let space_id = "gamma-bulk-space";
...
///
/// Field repro: a space with 241 docs failed `kutl sync` in prod (the read loop
/// wedged ~64 acks in; an upstream WS keepalive reaper then closed the
...
async fn test_restart_resubscribe_many_documents_does_not_deadlock() {
    ...
    let space_id = "restart-resubscribe-space";
    setup_space(&root, space_id, &relay_url);
    let state_path = root.join(".kutl/state.json");
