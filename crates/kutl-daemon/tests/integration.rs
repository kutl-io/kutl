...
use kutl_daemon::state::DaemonState;
...
/// Boot an in-memory relay app (no data dir, no backends) via the test seam.
/// The OSS binary's `build_app` requires a data dir; a non-durable daemon
/// test that wants in-memory registries constructs this shape explicitly.
/// (Durable tests keep their `data_dir` + `build_app`.)
fn build_in_memory_relay_app(
    config: RelayConfig,
) -> (
    axum::Router,
...
/// Grace for an aborted session's tasks to release the space (watcher,
/// sidecar, relay connection) before the next session boots on it.
const SESSION_TEARDOWN_SETTLE: Duration = Duration::from_millis(300);
...
fn space_root_known_relays(space_root: &Path) -> PathBuf {
    space_root.join(".kutl").join("known_relays.toml")
}
...
async fn start_relay_with_capacity(outbound_capacity: usize) -> String {
    ...
    // In-memory boot: build_app requires a data dir, so construct the
    // no-backend shape via the test seam.
    let (app, _relay_handle, _flush_handle) = build_in_memory_relay_app(config);
    tokio::spawn(async move {
...
/// Copy a tracked document (file + .dt sidecar + the state snapshot UUID) from one
/// space to another.
///
...
fn copy_tracked_doc(src_root: &Path, dst_root: &Path, rel_path: &Path, document_id: &str) {
    ...
    // Copy the state snapshot (UUID mappings) so both daemons use the same
    // document IDs.
    let src_state = DaemonState::load(&src_root.join(".kutl"));
    if !src_state.documents.is_empty() {
        src_state.save(&dst_root.join(".kutl")).unwrap();
    }
}
...
/// by `document_id` (matching what `seed_document_uuid` records in the state snapshot),
...
/// Pre-seed a document UUID in the state snapshot so the daemon uses a
...
fn seed_document_uuid(dir: &Path, rel_path: &str, uuid: &str) {
    ...
    state.set(rel_path.to_owned(), uuid.to_owned(), false);
...
/// Set up a temp space directory with `.kutl/space.toml` (and `.kutl/docs/`).
...
async fn test_two_daemons_converge() {
    ...
    // Pre-seed UUID in A, then copy (the state snapshot is copied by copy_tracked_doc).
...
async fn test_offline_remint_same_path_rejoins_cleanly() {
    ...
    session1.abort();
    tokio::time::sleep(SESSION_TEARDOWN_SETTLE).await;
    ...
    let converged = tokio::time::timeout(PIPELINE_TIMEOUT, async {
        loop {
            ...
            let tracked = DaemonState::load_readonly(&dir_path.join(".kutl"))
                .documents
                .get("notes.md")
                .map(|e| e.id.clone())
                .unwrap_or_default();
    ...
    assert!(
        ...
        DaemonState::load_readonly(&dir_path.join(".kutl")).documents,
    );

    session2.abort();
}

/// A document whose `.dt` sidecar cannot be read at startup is retired, not
/// re-minted: the sidecar moves aside as `.corrupt`, the relay's copy
/// refills a fresh engine under the same id, and an edit made while the
/// sidecar was unreadable folds in exactly once, on top of that content.
#[tokio::test]
async fn test_corrupt_sidecar_is_retired_and_refilled_from_the_relay() {
    init_test_tracing();
    let relay_url = start_relay().await;
    let (_dir, dir_path) = canonical_tempdir();
    let space_id = "4a1c2b7a-52a4-4b06-9c1d-0e8a55aa11ee";
    let doc_id = "cccc1111-2222-4333-8444-555566667777";
    let rel_path = Path::new("notes.md");
    let abs_path = dir_path.join(rel_path);
    ...
    seed_document_uuid(&dir_path, "notes.md", doc_id);

    // Session 1: the daemon registers and pushes the document; the observer's
    // catch-up proves the relay holds it.
    let session1 = spawn_ready_daemon(test_daemon_config(
        dir_path.clone(),
        "did:daemon",
        &relay_url,
    ...
    .await;
    let (_obs_cmd, mut obs_events) =
        connect_and_subscribe(&relay_url, space_id, "did:observer", doc_id).await;
    let caught_up = tokio::time::timeout(PIPELINE_TIMEOUT, async {
        loop {
            match obs_events.recv().await {
                Some(SyncEvent::RemoteOps { ops, .. }) if !ops.is_empty() => return,
    ...
    .await;
    assert!(caught_up.is_ok(), "the document never reached the relay");
    session1.abort();
    tokio::time::sleep(SESSION_TEARDOWN_SETTLE).await;

    // While the daemon is down the sidecar is damaged, and the user edits the
    // file on top of what it held.
    let dt_path = dt_sidecar_path(&dir_path, doc_id);
    std::fs::write(&dt_path, b"not a diamond-types file").unwrap();
    let edited = "original content\nedited while the sidecar was unreadable\n";
    std::fs::write(&abs_path, edited).unwrap();

    // Session 2: the sidecar is retired, the engine refills from the relay,
    // and the edit folds in once.
    let session2 = spawn_ready_daemon(test_daemon_config(
        dir_path.clone(),
        "did:daemon",
        &relay_url,
    ...
    .await;
    let corrupt = kutl_core::envelope::corrupt_path_for(&dt_path);
    let converged = tokio::time::timeout(PIPELINE_TIMEOUT, async {
        loop {
            tokio::time::sleep(POLL_INTERVAL).await;
            let fresh_sidecar = dt_path.exists()
                && kutl_core::Document::load(&dt_path).is_ok_and(|d| d.content() == edited);
            let file_ok = std::fs::read_to_string(&abs_path).is_ok_and(|c| c == edited);
            if fresh_sidecar && file_ok && corrupt.exists() {
                return;
            }
        }
    })
    ...
    assert!(
        converged.is_ok(),
        "retired sidecar never refilled: file = {:?}, fresh sidecar = {}, corrupt = {}",
        std::fs::read_to_string(&abs_path).ok(),
        dt_path.exists(),
        corrupt.exists(),
    );
    assert_eq!(
        DaemonState::load_readonly(&dir_path.join(".kutl"))
            .documents
            .get("notes.md")
            .map(|e| e.id.clone())
            .as_deref(),
        Some(doc_id),
        "the document keeps its id; it was not re-minted"
    );
    session2.abort();
}
...
async fn start_relay_with_auth() -> (String, tempfile::NamedTempFile) {
    ...
    // In-memory boot: see `build_in_memory_relay_app`.
    let (app, _relay_handle, _flush_handle) = build_in_memory_relay_app(config);
    tokio::spawn(async move {
        axum::serve(listener, app).await.unwrap();
    });
...
/// has history to serve a joiner) and return its ws URL plus the
...
async fn test_daemon_without_key_rejected_by_auth_relay() {
    ...
    // found". The relay answers with a refused HandshakeAck and closes, so
    // this is the only event the client can produce here.
...
async fn case_collision_rejects_rename_to_variant() {
    ...
    // (registration writes the state snapshot). Processing is serial, so by then the
    // earlier rename event was already observed (and rejected).
    let kutl_dir = dir_path.join(".kutl");
    tokio::time::timeout(PIPELINE_TIMEOUT, async {
        loop {
            if DaemonState::load_readonly(&kutl_dir)
                .documents
                .contains_key("_sentinel.md")
            {
                return;
            }
            tokio::time::sleep(POLL_INTERVAL).await;
    ...
    // daemon did NOT register a document at the colliding path A.md (the state
    // has no A.md entry), while both original docs' id-keyed sidecars survive.
    let docs_dir = kutl_dir.join("docs");
    let state = DaemonState::load_readonly(&kutl_dir).documents;
    assert!(
        !state.contains_key("A.md"),
...
async fn case_collision_rejects_new_variant_file() {
    ...
    // (which writes the state snapshot) rather than a path-named sidecar. Serial
    ...
    .await
    .expect("daemon should process the sentinel file");
    // what records a path → id mapping in the state, so its absence proves
    // rejection (sidecars are id-keyed, so there is no path-named sidecar to check).
    let state = DaemonState::load_readonly(&kutl_dir).documents;
    assert!(
        !state.contains_key("Foo.md"),
...
/// Count documents marked `confirmed: true` in a space's persisted state.
fn count_confirmed(kutl_dir: &Path) -> usize {
    DaemonState::load_readonly(kutl_dir)
        .documents
        .values()
...
/// it to register them ALL — reaching `confirmed: true` in the state snapshot —
/// within the timeout.
///
...
async fn test_bulk_seed_many_documents_does_not_deadlock() {
    ...
    let kutl_dir = space_root.join(".kutl");
    let synced = tokio::time::timeout(PIPELINE_TIMEOUT, async {
        loop {
            if count_confirmed(&kutl_dir) >= N {
                break;
            }
            tokio::time::sleep(POLL_INTERVAL).await;
        }
...
async fn test_restart_resubscribe_many_documents_does_not_deadlock() {
    ...
    // the relay (and the local state snapshot) end up holding N shared documents.
