...
/// Counter for signal-stream evictions per space — times the relay dropped this
/// daemon's signal subscription under outbound backpressure and it had to
/// re-subscribe.
///
/// Separate from [`ERRORS_TOTAL`] because an eviction is recovered rather than
...
const SIGNAL_STREAM_EVICTIONS: &str = "kutl_daemon_signal_stream_evictions_total";
...
#[allow(clippy::cast_precision_loss)]
// backlog is bounded by SYNC_BACKLOG_HIGH_WATER (256); f64 is exact here.
pub fn record_sync_backlog(space_id: &str, backlog: u64) {
    metrics::gauge!(SYNC_BACKLOG, "space" => space_id.to_owned()).set(backlog as f64);
}
...
/// Increment the signal-stream eviction counter for `space_id`.
pub fn record_signal_stream_eviction(space_id: &str) {
    metrics::counter!(SIGNAL_STREAM_EVICTIONS, "space" => space_id.to_owned()).increment(1);
}
...
mod tests {
    ...
    fn test_facade_emits_named_metrics_with_values() {
        ...
        metrics::with_local_recorder(&recorder, || {
            ...
            record_signal_stream_eviction("space-a");
        ...
        for expected in [
            ...
            "kutl_daemon_signal_stream_evictions_total{space=\"space-a\"} 1",
