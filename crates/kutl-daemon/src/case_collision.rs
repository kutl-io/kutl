...
///
/// Keys are the crate-wide `casefold` (canonical separators + the shared
/// case rule) — sufficient for the ASCII paths that dominate real
/// workspaces. Unicode normalization (NFC vs NFD) is explicitly out of
/// scope.
pub fn detect_case_collisions(root: &Path) -> Result<(), CaseCollisionError> {
    ...
    for entry in WalkDir::new(root).into_iter().filter_map(Result::ok) {
        ...
        let key = crate::core::casefold(rel);
