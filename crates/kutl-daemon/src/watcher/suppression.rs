...
#[derive(Default)]
pub(super) struct SuppressionSet {
...
impl SuppressionSet {
    ...
    pub(super) fn insert(&mut self, path: PathBuf, expected: Option<Vec<u8>>) {
        self.entries.insert(
            path,
            SuppressionEntry {
                payload: expected,
    ...
    pub(super) fn insert_rename_pair(&mut self, old: PathBuf, new: PathBuf, hash: Option<Vec<u8>>) {
        self.entries.insert(
            old.clone(),
            SuppressionEntry {
                payload: None,
    ...
    pub(super) fn consume_rename_pair_echo(&mut self, old: &Path, new: &Path) -> bool {
    ...
    pub(super) fn consume_write_echo(&mut self, path: &Path, current: &[u8]) -> bool {
    ...
    pub(super) fn consume_removal_echo(&mut self, path: &Path) -> bool {
        match self.entries.get(path) {
