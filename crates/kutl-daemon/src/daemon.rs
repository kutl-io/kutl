...
impl SpaceWorker {
    ...
    async fn handle_file_modified(
        ...
        // matches an existing identity whose old path is gone, treat it as a
        // rename. This handles platforms (macOS FSEvents) that don't emit
        // paired rename events. The old-path-gone check guards against inode
        // reuse — see `find_rename_source`.
        if !self.file_identity.contains_key(rel_path)
            && let Some(new_inode) = crate::inode::get_inode(&abs_path)
        {
            let space_root = self.config.space_root.clone();
            let match_found = find_rename_source(&self.file_identity, new_inode, |old_path| {
                space_root.join(old_path).exists()
            });
