...
/// Whether `a` and `b` resolve to the SAME on-disk file (inode equality) —
/// true exactly for a case-only rename's two names on a case-insensitive
/// filesystem, where `exists()` on the new name resolves to the file still at
/// its old casing. `false` when either path is missing or inodes are
/// unavailable (non-Unix), which degrades to the conservative pre-existing
/// occupied-target behavior.
pub fn same_file(a: &Path, b: &Path) -> bool {
    if let (Some(ia), Some(ib)) = (get_inode(a), get_inode(b)) {
        ia == ib
    } else {
        false
    }
