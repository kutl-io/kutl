...
//! a mover and so leaves no stale entry to resurrect — the `37093896` class is
//! structural. A mover whose desired path is held by an [`Occupant::Untracked`]
...
/// deferral: the resurrection bug (`37093896`) is structural.
///
...
pub(crate) fn reconcile_placement(state: &mut SpaceState) -> Vec<Effect> {
    ...
    // entry to resurrect (graft 2 / the `37093896` class).
...
mod tests {
    ...
    fn test_tombstoned_record_emits_nothing() {
        ...
        // a mover — the resurrection bug (37093896) is structural: a non-alive
