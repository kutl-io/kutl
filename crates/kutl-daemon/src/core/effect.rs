...
pub enum Effect {
    ...
    /// The echo suppression is registered by the shell's funnel, keyed by the
    /// hash of `content` — computed there, never carried, so no producer can
    /// mis-key it. Shell ACKs `FileWritten`.
    WriteFile { rel: PathBuf, content: Vec<u8> },
