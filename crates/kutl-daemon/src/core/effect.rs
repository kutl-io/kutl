...
pub enum Effect {
    ...
    /// Port of `rename_doc` (daemon/effects.rs): move `old`→`new`, suppressing
    /// the removal half (`old`, `None`) then the write half (`new`,
    /// `suppress_hash`) in that order (the paired sends in `rename_doc`).
    /// Shell ACKs `RenameApplied`.
    RenameFile {
        old: PathBuf,
        new: PathBuf,
        /// SHA-256 hash to suppress the rename-write echo (the write-half
        /// send in `rename_doc`).
        suppress_hash: Option<Vec<u8>>,
    },
    ...
    /// Delete the `.dt` sidecar for `document_id`. Port of
    /// `DocumentManager::remove`'s disk half (formerly `documents.rs:104`). The
    /// in-memory drop happens in `SpaceState::remove_doc_in_memory`; this is the
    /// sidecar half. Emitted by the new `handle` path (Task 6b); applied by
    /// `SpaceWorker::remove_doc_sidecar`.
    RemoveDoc { document_id: String },
