...
/// implicit coupling behind the `fa883acc` no-Subscribe bug), and re-derive
...
/// repeated bug class (`4bfaa9c9` dropped persists, `fa883acc`). The three
...
pub(super) fn handle_file_removed(
    ...
    // effect here is the `4bfaa9c9` dropped-persistence class).
...
pub(super) fn handle_file_renamed(
    ...
    // `4bfaa9c9` dropped-persistence class; surfaced by the case-only-rename
...
mod tests {
    ...
    fn test_local_modify_of_untracked_text_registers_subscribes_and_folds_shadow() {
        ...
        // (the implicit coupling behind the fa883acc no-Subscribe bug).
        let mut s = st();
