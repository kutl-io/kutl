...
use super::helpers::{
    causal_floor_for, cleanup_document_state, confirm_document, move_identity,
...
/// A document another daemon registered. **Fold-only, NOT freshness-gated** —
/// see the asymmetry note on [`SpaceState::lifecycle_event_is_fresh`].
///
...
pub(super) fn handle_remote_register(
    ...
    if was_seen_before && let Ok(id) = Uuid::parse_str(document_id) {
        // Thread the exemption into the cascade's deferral predicate, for
        // THIS path only. It is spent by the placement pass
        // that emits the doc's place, which may be a later pass than the
        // one this register triggers when that one is gated.
        state.exempt_revival.insert(id, path.to_owned());
    }
    ...
    // Path-arbitration conflict-copy deferral is the cascade's job:
    // `reconcile_placement` derives defer-vs-place from `known_records`, the
    // shadow, and `exempt_revival` (seeded above from `was_seen_before`).
    //
    // IDENTITY-FOLLOWS-PLACEMENT: we do NOT claim identity
    // (`file_identity`/`uuid_to_path`) here — the eventual placement does
    // (the shell's `GuardedPlace(Register)`, which the real daemon claims in
    // `place_now`). Claiming eagerly re-introduces TWO conflations: a DEFERRED
    // register leaves a stale path-claim the startup scan resolves back onto
    // (two authors merged into one), and a PLACING register masks a foreign
    // file racing onto the same path (the atomic stat can no longer tell our
    // own placement's file from a concurrent local create, so the local bytes
    // get clobbered). Seeding ONLY the record keeps the lattice driving the
    // cascade while identity rides the place.
...
/// A document another daemon renamed. **Freshness-gated** (drop via
...
/// do NOT emit a collision-blind imperative rename effect that would race it.
pub(super) fn handle_remote_rename(
    state: &mut SpaceState,
    document_id: &str,
    // The sender's advisory old name. The move resolves the real local source
    // by id from `uuid_to_path`, so the advisory name is unused here — a
    // concurrent rename/rename means our local path has already diverged from it.
    _old_path: &str,
    new_path: &str,
    ...
    if local_old.is_some() {
        ...
        // A subsequent LOCAL create at `new` is then mis-seen as an EDIT of
        // THIS document (its path is tracked) instead of a new document, and
        // the colliding create is silently lost. Leaving identity at the old
        // path until placement keeps `file_identity` consistent with the disk,
        // mirroring `handle_remote_register`. gamma OWNS the disk move — no
        // imperative rename effect here.
        seed_remote_record(
            state,
            document_id,
            &new_rel,
        ...
        //
        // DELIBERATE EXCEPTION to identity-follows-placement (stated at
        // `handle_remote_register`): this claim happens BEFORE any placement.
        // It is safe precisely because the doc is untracked here — there is
        // no local file whose adoption could merge two authors and no
        // placement whose atomic stat the claim could mask; the claim is what
        // lets the streaming content resolve a path at all. Every other
        // identity claim rides the placement ACK. Do not generalize from this
        // site.
        //
        ...
        // The out-of-band claim above mutated the identity maps with no
        // placement ACK downstream to persist them: snapshot now, or a crash
        // before the next state-saving event forgets the binding and the
        // restart re-derives the doc as a phantom.
        let mut effects = reconcile_unless_backlogged(state);
        effects.push(Effect::SaveState);
        return effects;
    }
    reconcile_unless_backlogged(state)
...
/// A document another daemon deleted. **Freshness-gated** — a delete
/// superseded by a newer lifecycle op (a concurrent rename that won, keeping
/// the document alive) is dropped.
///
/// **Edit-revives**, decided by the lattice as the relay decides it: the
/// delete folds as a tombstone beside the content touch this daemon holds,
/// and `DocRecord::is_alive` says whether the document survives. Content
/// whose touch dominates the delete keeps the document and nothing is
/// removed. This is the delete that raced a local edit — drained ahead of
/// it by the driver, or shipped moments before it — which the relay revives
/// on those ops and re-asserts to the space; removing the file here would
/// only recreate it a round-trip later, under the user's editor.
///
...
pub(super) fn handle_remote_unregister(
    ...
    // Read the touch BEFORE any cleanup: `cleanup_document_state` unloads the
    // document it is read from.
    let touched_hlc = state.content_touch(document_id);
    // The file to remove is at the doc's CURRENT on-disk location, which for a
    // DISPLACED doc is its conflict path
    ...
    // ALWAYS tombstone the doc in `known_records`, regardless of whether a file is
    // on disk to remove. A DEFERRED register never claimed a path, so `freed` is
    // None — but the lattice record IS alive (the cascade was deferring it), so
    // the delete MUST fold `deleted_hlc` to make it a tombstone; otherwise the
    // unregistered-but-never-placed doc stays an alive mover and the
    // resurrection guard leaks. The intended path for the tombstone is the
    // record's current one (a delete does not change it; a displaced doc's
    // conflict path is never seeded as its intended path), else the freed path.
    let id = Uuid::parse_str(document_id).ok();
    let tombstone_path = id
        .and_then(|id| state.known_records.get(&id))
        .map(|r| r.current_path().to_owned())
        .or_else(|| freed.as_ref().map(|rel| rel_path_to_string(rel)));
    ...
    // Edit-revives is decided by the record's liveness only once a delete
    // stamp has been folded: with none, `is_alive` is a constant `true`, and
    // a delete carrying no origin HLC applies unconditionally (the pre-HLC
    // fallback every lifecycle door shares).
    if deleted_hlc.is_some()
        && id
            .and_then(|id| state.known_records.get(&id))
            .is_some_and(kutl_core::lattice::DocRecord::is_alive)
    {
        debug!(
            ...
            ?deleted_hlc,
            ?touched_hlc,
            "remote delete loses to the record's liveness; keeping the document (edit revives)"
        );
        return reconcile_unless_backlogged(state);
    }
    let mut effects = Vec::new();
    if let Some(rel) = &freed {
        ...
        // occupant + shadow_path + inode). Folding on EMIT here would be the
        // one exception to the "shadow updates only on shell-ACK" invariant,
        // and it buys nothing: the tombstoned doc is not a mover, a winner
    ...
    // coalesced persist so the snapshot is not stale across a crash (an observer
    // that crashes between the drop and the next save would otherwise read a
    // stale on-disk map). Gated on the doc actually having been tracked so a
    // stale duplicate echo does not churn disk writes.
    if state.uuid_to_path.contains_key(document_id) {
        effects.push(Effect::SaveState);
    }
...
/// The relay acknowledged THIS daemon's own register/rename (the typed
/// lifecycle-ack rail). Confirms the document, then — only when the ack
/// signals we LOST a path collision (the effective path is a conflict path) —
/// self-corrects by reapplying the rename to that conflict path.
/// **Freshness-gated** (after classification, before the mutating apply): a
...
pub(super) fn handle_lifecycle_ack(
    ...
    // confirm survives a restart, on EVERY return path below.
    let mut effects = Vec::new();
    if confirm_document(state, document_id) {
        effects.push(Effect::SaveState);
    }
    ...
    // Self-correct: move our document to the arbitrated conflict path. This is
    // the same fold as a remote rename; the freshness gate already advanced
    // the watermark, so seed the record with the ack's stamp as the rename
    // hlc. gamma OWNS the disk move: the cascade derives the
    // `GuardedPlace(Rename)` onto the conflict path below (no separate
    // imperative rename effect, which would race the arbitrated order).
    let renamed_hlc = stamp.origin_hlc;
...
///
/// Returns only the `new` target — the `old` source is resolved from
/// `uuid_to_path` by the caller (the local move is reconciled by id, not the
/// advisory old path).
...
mod tests {
    ...
    fn test_fresh_remote_rename_of_tracked_doc_emits_guarded_rename() {
        ...
        // Reflect the placement in the shadow (the shell's place ACK) so the doc sits at "a.md".
        s.shadow
    ...
    /// Edit-revives at the daemon: a delete stamped below the touch of
    /// content this daemon holds keeps the document (the relay revives it on
    /// the same content); a delete stamped above the touch removes it.
    #[test]
    fn test_remote_unregister_loses_to_content_this_daemon_holds() {
        let mut s = st();
        let actor = s.hlc.last().actor;
        let id = doc_id();
        track_remote(&mut s, &id, "a.md", 10);
        let touch_ms = {
            ...
            let agent = doc.register_agent("editor").unwrap();
            doc.edit(
                agent,
                "did:editor",
                "insert",
                kutl_core::Boundary::Explicit,
                |ctx| ctx.insert(0, "kept"),
            )
            .unwrap();
            u64::try_from(doc.changes().last().unwrap().timestamp).unwrap()
        };
        let touch = kutl_core::Hlc::physical_touch(touch_ms);
        assert_eq!(s.content_touch(&id), Some(touch));

        // Same millisecond as the content: the touch's logical ceiling wins.
        let effects = DaemonCore::handle(
            &mut s,
            Event::RemoteUnregister {
                document_id: id.clone(),
                stamp: EventStamp {
                    wall_ms: touch_ms,
                    origin_hlc: Some(hlc(actor, touch_ms)),
                },
            },
        );
        assert!(
            ...
            "a delete the content outranks removes nothing, got {effects:?}"
        );
        assert!(
            s.uuid_to_path.contains_key(&id),
            "the document stays tracked"
        );
        assert!(
            s.known_records
                .get(&Uuid::parse_str(&id).unwrap())
                .is_some_and(kutl_core::lattice::DocRecord::is_alive),
            "the tombstone folded beside the touch leaves the record alive"
        );

        // One millisecond later than the content: the delete wins.
        let effects = DaemonCore::handle(
            &mut s,
            Event::RemoteUnregister {
                document_id: id.clone(),
                stamp: EventStamp {
                    wall_ms: touch_ms + 1,
                    origin_hlc: Some(hlc(actor, touch_ms + 1)),
                },
            },
        );
        assert!(
            ...
            "a causally-later delete removes the file, got {effects:?}"
        );
        assert!(!s.uuid_to_path.contains_key(&id));
    }
    ...
    #[test]
    fn test_remote_rename_of_an_untracked_doc_persists_the_claim() {
        // The untracked arm claims identity outside any placement ACK (the
        // deliberate exception) — the claim mutates the maps in memory only,
        // so the fold itself must ship a SaveState: a crash before the next
        // state-saving event would otherwise forget the binding and the
        // restart re-derives the doc as a phantom.
        let mut s = st();
        let id = doc_id();
        let effects = DaemonCore::handle(
        ...
        assert_eq!(
            s.uuid_to_path.get(&id),
            Some(&PathBuf::from("new.md")),
            "the untracked arm claims the identity"
        );
        assert!(
            effects.iter().any(|e| matches!(e, Effect::SaveState)),
            "the out-of-band identity claim must persist via SaveState, got {effects:?}"
        );
    }

    /// A delete carrying no origin HLC is the pre-HLC fallback and applies
    /// unconditionally: with no delete stamp folded, the record's liveness
    /// says nothing, and edit-revives must not consult it.
    #[test]
    fn test_remote_unregister_without_origin_hlc_removes_the_file() {
        let mut s = st();
        let id = doc_id();
        track_remote(&mut s, &id, "a.md", 10);
        let effects = DaemonCore::handle(
        ...
        assert!(
            ...
            "an HLC-less remote unregister must emit RemoveFile, got {effects:?}"
        );
        assert!(
            !s.uuid_to_path.contains_key(&id),
            "the unregistered doc's identity must be dropped"
