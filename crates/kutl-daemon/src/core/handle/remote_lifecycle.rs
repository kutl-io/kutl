...
pub(super) fn handle_remote_register(
    ...
    if was_seen_before && let Ok(id) = Uuid::parse_str(document_id) {
        ...
        // The set is per-pass — cleared at the top of `handle` — so this only
        // exempts the reconcile THIS register triggers below.
...
pub(super) fn handle_remote_rename(
    ...
    if local_old.is_some() {
        ...
        let mut effects = reconcile_placement(state);
        effects.push(Effect::SaveState);
        return effects;
    }
    reconcile_placement(state)
...
pub(super) fn handle_remote_unregister(
    ...
    // on disk to remove. A DEFERRED register never claimed a
    ...
    // resurrection guard leaks. The intended path for the tombstone is
    ...
    // coalesced persist so state.json is not stale across a crash (an observer
