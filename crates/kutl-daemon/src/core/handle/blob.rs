...
pub(super) fn merge_remote_blob(
    ...
    state.blob_state.insert(
        ...
        BlobState {
            ...
            size: i64::try_from(ops.len()).unwrap_or(i64::MAX),
        },
    );
    vec![
        Effect::WriteFile {
            suppress_hash: Some(sha256_bytes(ops)),
            rel,
            content: ops.to_vec(),
        },
        Effect::SaveBlobState,
        ...
        assert!(
            effects.iter().any(|e| matches!(
                ...
                Effect::WriteFile { rel, content, suppress_hash }
                    if rel.as_path() == Path::new("img.png")
                        && *content == bytes
                        && suppress_hash.as_deref() == Some(sha256_bytes(&bytes).as_slice())
            )),
            "accepted blob writes via the shared funnel with the suppress hash, got {effects:?}"
        );
        assert!(
