...
pub struct SpaceState {
    ...
    /// `Effect::SaveDoc` / `Effect::RemoveDoc` (plus the imperative
    /// `SpaceWorker::save_doc` / `remove_doc_sidecar`) persist mutations. The
    ...
    /// resurrection bug (`37093896`) is structural, not a guard to remember.
    ///
...
impl SpaceState {
    ...
    /// Called for a local register/mint and every remote register observed, in
    /// both configs; a revival's higher register stamp advances the floor.
    ...
    /// the WINNER's file at the contested path (carried concern 2). Every
