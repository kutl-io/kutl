...
impl SpaceWorker {
    ...
    pub(super) fn register_identity(
        ...
        // live inode, the disk-backed shadow claim, and persistence.
        ...
        if let Some(id) = self.state.file_identity.get_mut(&rel_path) {
            id.inode = inode;
        }
        if abs_path.exists() {
    ...
    /// only the inline save, which is the sole thing the pure core cannot do
    /// (it emits `Effect::SaveState` instead).
    ...
    /// pure core cannot touch — the live inode read and persistence.
    pub(super) fn move_identity(
        ...
        if let Some(id) = self.state.file_identity.get_mut(&dest) {
            id.inode = moved_inode(crate::inode::get_inode(&abs_new), id.inode);
        }
...
mod tests {
    ...
    fn test_offline_rename_inode_persists_across_restart() {
        ...
        // ── session 1: register an absent remote doc, then materialize it ──
        {
            ...
            // is the save-coalescing step that writes the just-recorded inode to state.json.
