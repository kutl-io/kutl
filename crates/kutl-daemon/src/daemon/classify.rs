...
impl SpaceWorker {
    ...
    /// disk, record it `Tracked` by its doc.
    pub(super) fn seed_shadow(&mut self) {
        let entries: Vec<(PathBuf, String)> = self
            .state
            .file_identity
            .iter()
            .map(|(rel, id)| (rel.clone(), id.document_uuid.clone()))
            .collect();
    ...
    fn is_local_rename_candidate(&self, rel_path: &Path, abs_path: &Path) -> bool {
        ...
        if tracked {
            ...
            self.overwrite_rename_source(inode, rel_path)
            ...
            self.live_rename_source(inode).is_some()
        }
    }
    ...
    pub(super) fn classify_blob_bytes(
        ...
        if self.state.file_identity.contains_key(&rel_path) {
            self.state
                .identity_set_inode(&rel_path, crate::inode::get_inode(abs_path));
        }
        LoopInput::Core {
    ...
    fn classify_modified(&mut self, rel_path: PathBuf) -> LoopInput {
        ...
        match self.classify_modified_content(rel_path.clone(), &abs_path) {
            Ok(input) => input,
            Err(e) => {
                error!(
                    rel_path = %rel_path.display(),
                    error = %e,
                    "failed to read a modified file"
                );
                crate::metrics_calls::record_error(
    ...
    /// The content route of a `Modified` on `rel_path`, after the rename
    /// probe declined or ran elsewhere: the stability gate, the read, and the
    /// UTF-8 fork into the core's text or blob path. The ONE door for a
    /// modified file's bytes — the watcher's events and the imperative rename
    /// detectors' fall-through (the startup scan included) both take it, so
    /// every modified file meets the same rules. A read error other than
    /// `NotFound` is the caller's to report; `NotFound` is a departed file, or a
    /// departed directory that still prefixes tracked documents, which routes
    /// to the removal expansion.
    pub(super) fn classify_modified_content(
        &mut self,
        rel_path: PathBuf,
        abs_path: &Path,
    ) -> std::io::Result<LoopInput> {
        ...
        match std::fs::read(abs_path) {
            Ok(bytes) => {
                    ...
                    && before != after
                {
                    ...
                    return Ok(LoopInput::Activity);
                }
                Ok(self.classify_read_bytes(rel_path, bytes, abs_path))
            }
            // Gone before we read it. A rename's old half or a transient
            // file is a no-op — but a vanished DIRECTORY that still PREFIXES
            ...
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
                Ok(if self.has_tracked_children(&rel_path) {
    ...
    /// The UTF-8 fork for bytes the edge just read at `rel_path`: text goes
    /// to the core's text path (with the empty-snapshot deferral and the
    /// tracked doc's inode refresh, the edge work the core cannot do), and
    /// anything else to the core's blob path through the size cap.
    fn classify_read_bytes(
        &mut self,
        rel_path: PathBuf,
        bytes: Vec<u8>,
        abs_path: &Path,
        ...
        match String::from_utf8(bytes) {
            ...
            Err(not_text) => self.classify_blob_bytes(rel_path, not_text.into_bytes(), abs_path),
        }
    }
