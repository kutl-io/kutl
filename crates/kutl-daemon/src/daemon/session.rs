...
fn bump_idle_deadline(one_shot: bool, idle_timeout: std::pin::Pin<&mut tokio::time::Sleep>) {
    if one_shot {
        idle_timeout.reset(tokio::time::Instant::now() + ONE_SHOT_IDLE_TIMEOUT);
    }
}
...
impl SpaceWorker {
    ...
    async fn run_session(
        ...
        // The client task moves the token; keep a clone so the post-connect
        // signal catch-up can present the SAME bearer to the HTTP surface.
        ...
        let supports_signal_records = match sync_event_rx.recv().await {
        ...
        // Signal catch-up happens AFTER discovery and the initial scan, just
        // before the event loop — see the block down there for why. Subscribing
        // here would put the relay's answer — and, for an
        // unauthorized DID, its refusal — into the event stream that
        // `startup_reconciliation` drains for its own replies, which would both
        // eat the first backlog page and turn a signal-stream refusal into a
        // bogus "relay rejected document discovery" error.
        ...
        // Join the signal stream LAST, immediately before the loop that reads
        // its pages.
        //
        // Ordering is load-bearing: `startup_
        // reconciliation` and `initial_file_scan` both drain `sync_event_rx`
        // looking for their own replies, so a `SignalPage` answered before them
        // is consumed and discarded — catch-up then silently delivers nothing
        // (`test_signal_catch_up_converges_two_daemons` pins this).
        // Subscribing here means the first page cannot arrive before there is
        // something to receive it.
        if supports_signal_records && let Err(e) = self.start_signal_catch_up(&sync_cmd_tx) {
            warn!(error = %e, "signal catch-up on connect failed (advisory, continuing)");
        }
        ...
        // the clock — so without this save, a one-shot `kutl sync` would leave
        // the persisted floor at its pre-session value (`{0,0}` on first run),
        // and the next sync would stamp its offline delete/rename at zero and
        // lose arbitration at the relay.
        self.save_state();
    ...
    /// record at `$KUTL_HOME/known_relays.json`, and adopt the resulting trust
    /// set.
    ///
        ...
        loop {
            ...
            match input {
                ...
                LoopInput::SignalAck {
                    ...
                    // re-seed walk. Without this bump the ENTIRE multi-chunk
                    // walk would have to fit inside one idle window — fine at
                    // local round-trip speeds, but over a real network N
                    // chunks of latency can exceed it, and the one-shot would
                    // exit mid-push looking like an ordinary idle completion.
                    // With the bump, each ack grants a fresh window, so walk
                    // length does not bound completion.
                    bump_idle_deadline(one_shot, idle_timeout.as_mut());
    ...
    /// local rename first, run the PURE [`DaemonCore::handle`] decision,
    ...
    pub(super) fn apply_core_input(
        ...
        // Surface an undrained concurrent LOCAL rename FIRST for any core
        // event whose apply would destroy its evidence (`drain_before_core_event`).
