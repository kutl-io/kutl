...
impl SpaceWorker {
    ...
    pub(super) fn apply_effect(
        ...
        match effect {
            ...
            // new document at the move target. Mirrors `register_identity`
    ...
    /// reads the birthtime in [`Self::to_sync_command`] — mirrors
    ...
    /// Funnel: move `old`→`new` (the `GuardedPlace(Rename)` arms), suppressing the
    ...
    ///   wrong-winner, as the former imperative conform did). A GENUINE concurrent
    ...
    ///
    /// Uses the shared funnels (`apply_rename`/`apply_write` + `apply_effect_result`)
...
mod tests {
    ...
    /// Seed the state a LOCAL DELETE leaves behind: the CRDT sidecar still holds
    /// the content, while the placement state that pointed at a path is gone.
    fn seed_deleted_but_held(worker: &mut SpaceWorker, doc: &str, content: &str) {
    ...
    async fn test_register_place_materializes_revived_doc_from_its_sidecar() {
        ...
        seed_deleted_but_held(&mut worker, DOC, "payload\n");
        assert!(
