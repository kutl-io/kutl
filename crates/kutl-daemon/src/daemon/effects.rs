...
//! (graft 1 / FS-1), and the periodic metrics/watchdog emission — plus the
//! free-standing disk-mutation funnel (`write_doc`/`rename_doc`/`remove_doc`)
//! and `encode_delta`.
...
use crate::blob_state::{sha256_bytes, sha256_hex};
use crate::client::SyncCommand;
use crate::core::{DaemonCore, Effect, EffectResult, Event, FileIdentity, rel_path_to_string};
use crate::watcher::Suppression;

use super::SpaceWorker;
use super::identity::refresh_inode;
...
impl SpaceWorker {
    ...
    /// into the shadow via [`DaemonCore::apply_effect_result`] (graft 3, success
    /// only). `GuardedPlace` is a gamma cascade effect: gamma's `reconcile_placement`
    /// emits it, but no event the live driver routes through `handle` reaches the
    /// cascade with the feature OFF (see `classify_*`), so that arm is a guarded
    /// no-op until the gamma cutover.
    pub(super) fn apply_effect(
        ...
        match effect {
            ...
            // local move of this file is detected by `find_rename_source` (which
            // matches on the recorded inode) instead of being minted as a spurious
            // new document at the move target. Mirrors procedural's `register_identity`
            // (daemon.rs:474-487) reading + recording the inode synchronously.
            ...
            Effect::WriteFile {
                ...
                if let Some(res) = self.apply_write(&rel, &content, suppress_hash, suppress_tx) {
            ...
            Effect::RenameFile {
                ...
                if let Some(res) = self.apply_rename(&old, &new, suppress_hash, suppress_tx) {
                    DaemonCore::apply_effect_result(&mut self.state, res);
                }
            }
            ...
            Effect::SaveDoc { document_id } => {
                if let Err(e) = self.save_doc(&document_id) {
                    error!(error = %e, "failed to save document sidecar");
                }
            }
            Effect::RemoveDoc { document_id } => {
                self.state.remove_doc_in_memory(&document_id);
                self.remove_doc_sidecar(&document_id);
            }
            // Coalesced: mark a persist pending rather than rewriting the whole
            // `state.json` inline. The loop flushes it once the intake is drained
    ...
    /// Without this, `find_rename_source` (which matches a moved file to its origin
    /// via the recorded inode) could never match the document on a later local move,
    /// and the relocated file would be minted as a spurious new document at the move
    /// target. Reading + recording the inode here — at the same driver edge that
    /// reads the birthtime in [`Self::to_sync_command`] — mirrors procedural's
    /// `register_identity` (`daemon.rs:474`), which records it synchronously.
    ///
    /// No-op when the file is not on disk (a register whose content has not landed)
    /// or the inode cannot be read; the tracked-edit refresh (`daemon.rs:1486`)
    /// remains a backstop for any path that gets a later edit first.
    fn record_local_register_inode(&mut self, rel: &str) {
        ...
        if let Some(identity) = self.state.file_identity.get_mut(&rel_path) {
            identity.inode = Some(inode);
        }
    ...
    /// Funnel: write `content` to `rel` from an [`Effect::WriteFile`], suppressing
    /// the resulting echo by `suppress_hash` through the watcher's `Suppression`
    /// channel (preserving the funnel's order, `daemon.rs:3142`). Returns the
    /// post-op [`EffectResult`] (with the real inode) on success; `None` on failure
    /// (graft 3: a failed write leaves the shadow unchanged for the next reconcile).
    fn apply_write(
        &mut self,
        rel: &Path,
        content: &[u8],
        ...
        {
            error!(path = %abs_path.display(), error = %e, "failed to create parent dir for write");
            return None;
        }
        let _ = suppress_tx.send(Suppression::Single(rel.to_path_buf(), suppress_hash));
        ...
        refresh_inode(&mut self.state.file_identity, rel, inode);
        Some(EffectResult::FileWritten {
            rel: rel.to_path_buf(),
            inode,
        })
    }

    /// Funnel: move `old`→`new` from an [`Effect::RenameFile`], suppressing the
    /// removal half `(old, None)` then the write half `(new, suppress_hash)` in that
    /// order (`daemon.rs:3121-3124`). The write-half hash is computed HERE from the
    /// moved bytes (the core sends `suppress_hash: None`; the content lives at `old`,
    /// not in `SpaceState`). Returns the post-op [`EffectResult`] on success.
    fn apply_rename(
        &mut self,
        old: &Path,
        new: &Path,
            ...
            && let Err(e) = std::fs::create_dir_all(parent)
        {
            error!(
                ...
                to = %new.display(),
                error = %e,
                "failed to create parent dir for rename"
            );
            return None;
        }
        // Compute the write-half suppress hash from the moved bytes when the core
        // did not supply one (it can't read disk); matches old `rename_doc`.
        let write_hash =
            suppress_hash.or_else(|| std::fs::read(&old_abs).ok().map(|b| sha256_bytes(&b)));
        // ONE pair-matched suppression: the watcher links the halves so its OS
        // rename-pair consumer fires only for this exact pair.
        ...
        refresh_inode(&mut self.state.file_identity, new, inode);
        Some(EffectResult::RenameApplied {
            old: old.to_path_buf(),
            new: new.to_path_buf(),
            inode,
        })
    ...
    /// echo `(rel, None)` (`daemon.rs:3142`). Best-effort: a missing file is fine
    ...
    /// section (graft 1). Real-driver twin of `DaemonSim::apply_guarded_place`.
    ///
    /// The stat (`stat_untracked`) and the place run ADJACENTLY in this one
    /// non-suspending method — the same shape `reconcile_disk_for_rename` uses
    /// (`exists()` then rename, `daemon.rs` rename reconcile) — so a file appearing
    /// in the TOCTOU window IS seen by the stat:
    /// - DISAGREEMENT (`expected_free` but the live stat found an untracked
    ///   occupant): the FS-1 race. Do NOT place — inject `Event::UntrackedFileObserved`
    ...
    fn apply_guarded_place(
        ...
        // core's shadow never saw it (that is the TOCTOU window, graft 1).
        ...
        // AGREEMENT: place.
        self.place_now(uid, id, target, place_kind, sync_cmd_tx, suppress_tx)
    }
    ...
    /// genuine FS-1 race — a foreign create no document is arbitrated onto.
    ///
    ...
    fn stat_untracked(&self, target: &Path) -> bool {
        ...
        // shadow has not folded yet (the graft-1 TOCTOU window the core
        // cannot see).
        ...
        crate::core::shadow::held_by_foreign_untracked(
            self.state
                .shadow
                .shadow_occupant
                .get(&crate::core::shadow::casefold(target)),
            abs.exists(),
            self.state.file_identity.contains_key(target),
        )
    }

    /// The place half of [`Self::apply_guarded_place`] (the AGREEMENT branch): commit
    /// the placement to disk + identity and fold the post-op inode into the shadow.
    ///
    /// `Register` mirrors `place_register` (`daemon.rs:2918`): claim identity at the
    /// target (so a later content op resolves the path), `Subscribe` so the content
    /// streams in, and mark the shadow `Tracked` so the recompute is idempotent (the
    /// register places NO bytes — the disk stays empty until content arrives). When
    /// the file already exists (a revival adopting its own orphan), fold its inode.
    ///
    /// `Rename` moves the held file via the shared `apply_rename` funnel (the same
    /// suppress-pairing/order the imperative `rename_doc` uses) and folds the result
    /// via `apply_effect_result` (graft 3 — vacate old, track new, record inode).
    fn place_now(
        &mut self,
        uid: uuid::Uuid,
        id: &str,
        ...
        match place_kind {
            crate::core::PlaceKind::Register => {
                ...
                // (carried concern 1 / FS-2). A doc ALREADY in `uuid_to_path` is a
                ...
                let abs = self.config.space_root.join(target);
                if let Some(inode) = crate::inode::get_inode(&abs) {
                    self.state.shadow.set_inode(inode, target);
                }
                ...
                // (matching the startup-path `register_and_subscribe`, which
                // registers AND subscribes; a self-subscribe is harmless).
                self.send_cmd(
                    sync_cmd_tx,
            ...
            crate::core::PlaceKind::Rename { old_rel } => {
                ...
                if is_displacement {
                    ...
                    if let Some(blob) = self.state.blob_state.get(&old_rel) {
                        ...
                        if intact {
                            if let Some(res) =
                                self.apply_rename(&old_rel, target, None, suppress_tx)
                            {
                    ...
                    let content = self.state.load_or_create_doc(id).content().into_bytes();
                    if let Some(res) = self.apply_write(target, &content, None, suppress_tx) {
                        // Claim identity at the conflict path BEFORE folding so the
                        // `FileWritten` fold resolves the id and marks the shadow
                        // `Tracked(id)` at `target` (idempotent fixpoint), then vacate
                        // the stale source so it leaves no orphan.
                        self.move_identity(&old_rel, target.to_path_buf(), id);
                        DaemonCore::apply_effect_result(&mut self.state, res);
                        let abs_old = self.config.space_root.join(&old_rel);
                ...
                // Occupied-target guard (mirrors procedural `reconcile_disk_for_rename`,
                // daemon.rs: `abs_old.exists() && !abs_new.exists()`): move a WINNER
                // onto a clean target ONLY when the target is free on LIVE disk.
                //
                // A concurrent LOCAL rename onto the SAME target (the §2.2/§2.4
                // rename/rename collision) can put a DIFFERENT live file at `target`
                ...
                // handles a foreign UNTRACKED occupant (FS-1); this also covers a target
                ...
                // authoritative target by inode (or MATERIALIZE from the CRDT) — the gamma
                // (the former `conform_or_materialize_at`'s contract). Without
                ...
                // peer's injected clock skew, beats the authoritative winner (the §7.1
                ...
                if !abs_old.exists() {
                    self.conform_relocated_or_materialize(id, &old_rel, target, suppress_tx);
                    return Ok(());
                }
                // Move the held file onto the target through the shared funnel
                // (suppress pairing/order preserved) and fold the post-op inode.
                if let Some(res) = self.apply_rename(&old_rel, target, None, suppress_tx) {
                    // Claim the new path's identity, mirroring procedural's
                    // `place_rename` (`move_identity` before the disk reconcile,
                    // daemon.rs). The cascade derives a DISPLACEMENT move (a loser to
    ...
    /// for the doc) holds no file on LIVE disk.
    ///
    ...
    ///   its local name and emits no competing `RenameDocument` (the §7.1 skew
    ///   wrong-winner, mirroring procedural's conform). A GENUINE concurrent
    ...
    ///   into the core (§1.3), so the silence here only ever swallows echoes of
    ///   an already-emitted rename.
    /// - not found (genuinely deleted) → `apply_write` the CRDT content at `target`.
    ///
    /// Uses the gamma funnels (`apply_rename`/`apply_write` + `apply_effect_result`)
    /// and `move_identity`, NOT the procedural `rename_doc`/`fold_shadow_*`, so the
    /// gamma path owns its own identity-follows-placement shadow bookkeeping. The
    /// inode `WalkDir` is O(files) but runs only on this rare missing-source path
    /// (never per content op), the same cost asymmetry procedural keeps. Best-effort
    /// like the former `conform_or_materialize_at` did: the funnels log their
    /// own IO errors.
    fn conform_relocated_or_materialize(
        ...
        match self.space_file_with_inode(self.recorded_inode(old_rel)) {
            ...
            Some(relocated) if relocated.as_path() != target => {
                if let Some(res) = self.apply_rename(&relocated, target, None, suppress_tx) {
                    self.move_identity(old_rel, target.to_path_buf(), id);
                    DaemonCore::apply_effect_result(&mut self.state, res);
                }
            }
...
mod tests {
    ...
    use crate::daemon::identity::find_rename_source;
    ...
    async fn test_write_doc_records_inode_and_suppresses_by_hash() {
        ...
        write_doc(&mut map, &rel, &abs, b"hello", &tx).unwrap();
    ...
    /// file on a case-insensitive filesystem. The occupied-target guards
    /// (gamma `place_now`, procedural `reconcile_disk_for_rename`) test the
    /// target with `exists()`, which resolves case-insensitively to the SAME
    ...
    // ── TWIN deferral predicate: stage-1 equivalence pin (graft-1 agreement) ──
    ...
    fn core_held_by_untracked(shadow: RowShadow, identity_claimed: bool) -> bool {
        ...
        s.known_records.observe(kutl_core::lattice::DocRecord {
            document_id: doc,
            path: "p.md".into(),
            registered_hlc: Some(kutl_core::Hlc {
                physical_ms: 1,
                logical: 0,
        ...
        let effects = DaemonCore::reconcile(&mut s);
    ...
    /// Stage-1 pin of the TWIN deferral predicate across the IO boundary —
    /// graft 1's agreement point. The driver's `stat_untracked` (live disk +
    ...
    /// target beats the shadow's `Untracked` marker, in BOTH. Until the wave-3
    /// consolidation to one shared predicate, this contract is otherwise
    /// comment-only — this test is the regression sensor for a one-sided edit.
    #[test]
    fn test_twin_deferral_predicate_equivalence() {
        ...
        // to the driver's live stat — graft 1 exists precisely because the
