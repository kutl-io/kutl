...
impl SpaceWorker {
    ...
    /// success; `None` on failure (graft 3: a failed write leaves the shadow
    ...
    fn apply_write(
        ...
        match write_doc(
            ...
            Ok(inode) => Some(EffectResult::FileWritten {
                rel: rel.to_path_buf(),
                inode,
            }),
            Err(e) => {
    ...
    fn apply_rename(
        ...
        match rename_doc(
            ...
            Ok(inode) => Some(EffectResult::RenameApplied {
                old: old.to_path_buf(),
                new: new.to_path_buf(),
                inode,
            }),
    ...
    fn place_by_register(
        ...
        if let Some(inode) = crate::inode::get_inode(&abs) {
            ...
            if let Some(content) = held {
                ...
                if let Some(res) = self.apply_write(target, &content, suppress_tx) {
                    DaemonCore::apply_effect_result(&mut self.state, res);
                }
            }
        }
            ...
            crate::core::PlaceKind::Rename { old_rel } => {
                ...
                if is_displacement {
                    ...
                    if let Some(blob) = self.state.blob_state.get(&old_rel) {
                        ...
                        if intact {
                            if let Some(res) = self.apply_rename(&old_rel, target, suppress_tx) {
    ...
    fn conform_relocated_or_materialize(
        ...
        match self.space_file_with_inode(self.recorded_inode(old_rel)) {
            ...
            Some(relocated) if relocated.as_path() != target => {
                if let Some(res) = self.apply_rename(&relocated, target, suppress_tx) {
                    self.move_identity(old_rel, target.to_path_buf(), id);
                    DaemonCore::apply_effect_result(&mut self.state, res);
                }
            }
...
// the hash COMPUTED FROM THE BYTES BEING WRITTEN (the watcher then recognizes
// its own echo, while a genuine concurrent edit — different bytes — is never
// swallowed); a removal registers `(path, None)`. The hash is never a
// parameter: a caller that could pass one could pass `None` and silently
// register a removal suppression for a write, which both poisons a later real
// removal echo at that path and lets the write echo surface as a spurious
// local edit. Each write/rename also refreshes the recorded inode, keeping
...
/// its content hash and refreshing the recorded inode. Returns the recorded
/// inode so the effect path can fold it into the shadow.
pub(super) fn write_doc(
    ...
    refresh_inode(file_identity, rel_path, inode);
    Ok(inode)
}
...
pub(super) fn rename_doc(
    ...
    refresh_inode(file_identity, new_rel, inode);
    Ok(inode)
}
