...
//! the §1.3/A1/A4i fix cluster — metadata minting (fresh and explicit-stamp),
...
impl SpaceWorker {
    ...
    ///
    /// The source is the per-doc REGISTER HLC (`register_hlc`), populated for every
    /// doc this daemon created or saw registered, and PERSISTED
    /// across restarts (`state.json`): an OFFLINE rename is re-emitted at the next
    /// startup stamped at the pre-offline floor — `{0,0}` for a pure observer that
    /// never emitted a stamp — so the recorded register is its only causal proof
    /// over the registration (the offline-ingest rejoin bug: a `None` floor there
    /// loses to the registration at the relay and the rename is dropped
    /// cluster-wide). It MUST be the register specifically — NOT the lifecycle
    /// watermark, which also absorbs renames: in §7.1 (A and B both rename one
    /// doc), if B's watermark had folded A's concurrently-observed rename, B's
    /// floor would reach A's rename HLC and TIE it, and the lexicographic-path
    /// tiebreak would flip the skew winner run-to-run (a non-deterministic §7.1
    /// break). Under gamma the placement lattice's `registered_hlc` is an
    /// equivalent in-session source (kept as a fallback). `None` only if the
    /// register was never recorded, which degrades to the original lost-rename
    /// behavior — never to a wrong-high floor.
    ...
    /// scan, `sync_local_document`, and the blob path all register through
    ...
    pub(super) fn fold_local_register_record(
        ...
        self.state
            .known_records
            .observe(kutl_core::lattice::DocRecord {
                document_id: id,
                path: path.to_owned(),
                registered_hlc,
                renamed_hlc: None,
    ...
    pub(super) fn fold_local_rename_record(
        ...
        // `registered_hlc: None` + `renamed_hlc: Some` is a rename fragment: merge
        // takes the max per HLC field (preserving the existing registration) and the
        // rename's path wins (`path_priority`, rename-beats-register). The causal
        // floor lets the rename supersede even a future-stamped (skewed) register.
        // Identical to the core's
        // `seed_remote_record(state, id, new, None, renamed_hlc, floor, None)`.
        self.state
            .known_records
            .observe(kutl_core::lattice::DocRecord {
                document_id: id,
                path: rel_path_to_string(new_rel),
                registered_hlc: None,
                renamed_hlc,
                rename_causal_floor,
                touched_hlc: None,
...
mod tests {
    ...
    /// The offline-ingest rejoin rename bug (A1, 2026-06-11): a later OFFLINE
    ...
    ///
    /// Drives the REAL per-config remote-register entry (the gamma core's
    /// `Event::RemoteRegister` / the procedural `handle_remote_register`), then
