...
//! registered here) and `initial_file_scan` AFTER it (the watcher must be
...
impl SpaceWorker {
        ...
        for action in actions {
            match action {
                ...
                StartupAction::RegisterLocal { document_id, path } => {
                    self.register_local_document(document_id, path, sync_cmd_tx)?;
                }
    ...
    fn register_local_document(
        ...
        self.register_and_subscribe(sync_cmd_tx, document_id, &register_path, "startup sync")?;
    ...
    pub(super) fn initial_file_scan(
        ...
        for rel_path in paths {
            ...
            // String materialization during the scan.
            ...
            if let Some(doc) = doc_id.as_deref().and_then(|id| self.state.get_doc(id)) {
                ...
                match std::fs::read_to_string(&abs_path) {
                    ...
                    // File differs or can't be read — process it. Logged
                    // because this is the door that turns disk bytes into new
                    // CRDT ops for an ALREADY-TRACKED doc: if the difference
                    // is not a genuine offline edit (e.g. the CRDT is behind
                    // because remote catch-up for this doc is still in
                    // flight), the diff re-inserts content the incoming ops
                    // also carry, and every replica converges on the
                    // duplicated result.
                    _ => {
                        debug!(
...
mod tests {
    use super::*;
    use crate::daemon::SpaceWorkerConfig;

    fn test_worker(space_root: std::path::PathBuf) -> SpaceWorker {
        SpaceWorker::new(SpaceWorkerConfig {
            poll_watcher: true,
            space_root,
            author_did: "did:test".into(),
            relay_url: "ws://test".into(),
