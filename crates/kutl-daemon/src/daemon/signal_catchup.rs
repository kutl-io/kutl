...
//! [`SpaceWorker`]: the driver edge that, first thing in every session,
//! listens to the space (which is what makes the daemon present in it) and
//! pulls the relay's durable records this daemon lacks, then pushes back any
//! this daemon holds that the relay lacks.
//!
//! Direction:
...
impl SpaceWorker {
    ...
    ///
    /// **Subscribing is what makes this daemon present in the space.** The
    /// relay keeps one listener set per space, entered by this frame: it is
    /// the recipient set for space-audience signals and for lifecycle
    /// broadcasts, and the presence `list_participants` reports. A connection
    /// that never sends it receives none of those, so this runs first thing
    /// in every session, before the document snapshot is even asked for.
    ///
    ...
    ///
    /// Infallible on purpose: the subscribe goes out whatever the local
    /// store's state, because presence and lifecycle delivery ride it. A
    /// store that cannot be opened or a cursor that cannot be read is logged
    /// and costs only the floor (the walk starts from zero).
    pub(super) fn start_signal_catch_up(
        &mut self,
        sync_cmd_tx: &tokio::sync::mpsc::UnboundedSender<crate::client::SyncCommand>,
    ) {
        let since = match self.catch_up_floor() {
            Ok(since) => since,
            Err(e) => {
                warn!(error = %e, "signal store unavailable; listening without a catch-up floor");
                None
            }
        };
    ...
    /// The cursor to resume the backlog walk from, opening the store on the
    /// way (idempotent) so the read and every later ingest run under its held
    /// flock.
    ///
    /// Resumes one millisecond BEFORE the persisted cursor. Live ingest
    ...
    fn catch_up_floor(&mut self) -> Result<Option<kutl_proto::sync::Hlc>> {
        ...
        let dir = self
            ...
            .dir()
            .to_path_buf();
        Ok(kutl_signals::catchup::load_cursor(&dir)
    ...
    /// # Errors
    ///
    /// Returns an error if the local signal store cannot be opened, the local
    /// segments cannot be loaded, or the push fails.
    ...
    ///
    /// Returns an error if the local signal store cannot be opened, the local
    /// segments cannot be loaded, or the re-seed POST fails.
    fn reseed_surplus(
        &mut self,
        sync_cmd_tx: &tokio::sync::mpsc::UnboundedSender<crate::client::SyncCommand>,
        space_id: &str,
        ...
        // The subscribe that led here goes out even when the store failed to
        // open (presence rides it), so the store is not a given: open it now,
        // and let one still unavailable surface as the advisory re-seed
        // failure rather than a panic on the relay's first page.
        self.ensure_signal_store()?;
        let loaded = self
            .signal_store
            .as_ref()
