...
pub struct DaemonState {
    /// Maps relative file paths to their document entry. Defaulted so a state
    /// file carrying only `device_id` (the e2e harness's device pin, written
    /// before first start) parses as the current format instead of falling
    /// through to the "corrupt" branch — which silently discarded the pinned
    /// device actor and re-randomized the HLC tiebreaker every run.
    #[serde(default)]
    ...
    /// live map on every persist; pruned on unregister. Defaulted on legacy
    /// state files.
    #[serde(default)]
    ...
    /// `BTreeSet` for stable serialization order. Defaulted on legacy state
    /// files.
    #[serde(default)]
    ...
    /// one. Defaulted on legacy state files.
    #[serde(default)]
