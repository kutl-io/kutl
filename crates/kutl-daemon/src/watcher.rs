...
/// watcher event is recognised as its own echo and dropped. A TTL ensures that
/// if the echo event never arrives (coalesced or dropped by the OS) the
/// suppression expires instead of silently swallowing a later genuine edit, and
/// bounds the set's growth.
#[derive(Default)]
struct SuppressionSet {
    /// `path -> poll cycles remaining before expiry`.
    entries: HashMap<PathBuf, u8>,
}

impl SuppressionSet {
    /// Register `path` for echo suppression.
    fn insert(&mut self, path: PathBuf) {
        self.entries.insert(path, SUPPRESS_TTL_POLLS);
    }

    /// Consume a pending suppression for `path`; returns true if one existed.
...
impl FileWatcher {
    ...
    async fn flush_pending(
        ...
        for (old_path, new_path) in rename_pairs.drain(..) {
            ...
            let suppressed_old = self.suppressed.consume(&old_path);
            let suppressed_new = self.suppressed.consume(&new_path);
        ...
        for (rel_path, kind) in events {
            if self.suppressed.consume(&rel_path) {
...
mod tests {
    ...
    /// Suppressions are consumed independently (no short-circuit) so a
    /// rename suppressing both halves clears both.
    #[test]
    fn test_suppression_set_consumes_each_entry_independently() {
        ...
        assert!(s.consume(Path::new("old.md")));
        assert!(s.consume(Path::new("new.md")));
        assert!(!s.consume(Path::new("old.md"))); // already consumed
        assert_eq!(s.len(), 0);
    }
    ...
    fn test_suppression_set_expires_after_ttl() {
        ...
        s.tick();
        assert!(s.consume(Path::new("x.md")));
        ...
        assert!(!s.consume(Path::new("y.md")));
        assert_eq!(s.len(), 0);
    }
}
